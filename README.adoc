...
* The GitHub CLI `gh` (version 2.68 or higher) is used if it is installed;
  ...
  needed.* Setting `GH_TOKEN` to any GitHub token is optional and only makes
  the run use your own API quota instead of the anonymous one (60 requests per
  hour per IP address).
...
incident). The report of a mismatching rebuild therefore ends by saying
whether the provenance of the CDN files was confirmed.

Consequently:

* An attestation that *does not match* aborts the run, within seconds and
  before the build starts. This is what a substituted file that carries
  someone else's attestation — another commit's or another directory's
  checksum file — runs into.
* An attestation that *cannot be obtained* only prints a warning, and the
  verification continues to the local build. This covers GitHub or Sigstore
  being unreachable, the API rate limit being exhausted, a commit built before
  attestations were introduced, and a security hotfix built on the private
  `dfinity/ic-private` repository: those builds are not attested, and the
  commit gains an attestation only once its `hotfix-*` branch is published to
  `dfinity/ic` and rebuilt there. It also covers a substituted file that
  nobody attested at all: attestations are looked up by the file's digest, so
  up front such a file cannot be told from an unattested build. The local
  build catches it at the end, and the mismatch report then states that the
  provenance was never confirmed, so that the mismatch is not taken for a
  reproducibility bug.
...
`--skip-attestation-check` turns the check off entirely. It is a developer
escape hatch and should never be used when verifying an NNS proposal. It is
also not the way around an outage, since an unreachable attestation already
only warns; and verifying an older release is done by running that release's
own version of the script, as described above, not by disabling the check.
