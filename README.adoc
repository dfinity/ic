...
than read from the download cache, and a cached image is revalidated against
the CDN (a `HEAD` request) before it is reused, so rerunning with a warm cache
still reports on what the CDN serves now. Artifacts under `ic/{COMMIT_ID}/`
are immutable by policy, so a revalidation mismatch is worth reporting.
