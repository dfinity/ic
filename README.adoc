...
`HEAD` request confirms that the CDN still serves the same bytes; otherwise it
is downloaded again. So rerunning with a warm cache still reports on what the
CDN serves now. Artifacts under `ic/{COMMIT_ID}/` are immutable by policy, so
a revalidation mismatch is worth reporting. Pass `--clean`, or use a fresh
