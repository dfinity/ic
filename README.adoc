...
* About 100 GB of free disk space in `$HOME` (or another volume, specifed via
  option `--cache-dir`)
* Network access to `download.dfinity.systems` (and `download.dfinity.network`
  with `--download-source network` or `both`), `ic-api.internetcomputer.org`
  (for `-p`), `github.com`, `raw.githubusercontent.com`,
  `release-assets.githubusercontent.com`, `api.github.com`,
  `tuf-repo-cdn.sigstore.dev` and `ghcr.io` (the build container image), plus
  an Ubuntu package archive if `git` or `podman` has to be installed. The
  GitHub hosts and Sigstore are not optional: the run aborts when the
  attestation of a CDN checksum file cannot be fetched and verified.
* A checksum-pinned GitHub CLI `gh` is downloaded, kept in the download cache
  and unpacked into the run's temporary directory; a `gh` installed on the
  machine is not used. *No GitHub account or login is needed.* Setting
  `GH_TOKEN` to any GitHub token is optional: it makes the run use your own
  API quota instead of the anonymous one (60 requests per hour per IP address;
  a run normally needs at most eight), which matters behind a shared address
  because an exhausted quota aborts the run.
...
Besides rebuilding the images, the script checks that the `SHA256SUMS` files it
downloads from the CDN are covered by a https://docs.github.com/actions/security-for-github-actions/using-artifact-attestations[GitHub
build-provenance attestation]: a Sigstore-signed statement that they were
produced by a `dfinity/ic` GitHub Actions build of exactly the commit being
verified. The images and `launch-measurements.json` are then checked against
those verified checksums, so one attestation per directory covers everything
downloaded from it.

Which build counts depends on the mode. Verifying a proposal (`-p`) accepts
only a release build — `release-testing.yml` running on an `rc--` or
`hotfix-` branch — because that is what an elected version comes from.
Verifying a bare commit (`-c`) additionally accepts a `ci-kickoff.yml` build
of `master`, since a commit need not have been through a release branch. Use
`-p` when checking an NNS proposal. Each verified file is reported as:

  Attested: ic/{COMMIT_ID}/guest-os/update-img/SHA256SUMS (sha256 ...) by dfinity/ic CI run ... (release-testing.yml from refs/heads/rc--...)

*The local build remains the authoritative check.* Rebuilding the images from
source proves strictly more than an attestation does — it does not require
trusting GitHub's runners at all. The attestation is there to tell two very
different failures apart: if the attestation verifies but the rebuild does not
match the CDN, the build is not reproducible (a bug); if the CDN file carries
an attestation of another commit, directory or signer, the CDN is not serving
what CI built (an incident). Anything else — no attestation on GitHub, or
GitHub or Sigstore unreachable — tells the run only that the provenance is
unavailable, not which of the two it faces, and it stops rather than guess.
The mismatch report of a rebuild therefore always says that the provenance of
the CDN files was confirmed, unless the check was skipped.

Consequently, any attestation that does not verify aborts the run before any
image is downloaded or the build starts: within seconds, or within a few
minutes when GitHub or Sigstore has to time out first — whether it
*mismatches* (a substituted file carrying someone else's attestation: another
commit's or another directory's checksum file) or *cannot be obtained* at all
(bytes nobody attested, a commit built before attestations were introduced on
2026-08-31, a security hotfix still private on `dfinity/ic-private`, GitHub or
Sigstore unreachable, the API rate limit exhausted). Attestations are looked up
by the file's digest, so from here a substituted file nobody attested looks
exactly like an unattested build; the run stops rather than rebuild against
bytes whose provenance it cannot establish. A hotfix built on
`dfinity/ic-private` gains its attestation once its `hotfix-*` branch is
published to `dfinity/ic` and rebuilt there.

The checksum files and launch measurements are always fetched fresh rather
than read from the download cache, and a cached image is reused only after a
`HEAD` request shows the CDN still serving it under the same strong `ETag`,
size and modification time; otherwise it is downloaded again. So rerunning
with a warm cache still reports on what the CDN serves now. Artifacts under
`ic/{COMMIT_ID}/` are immutable by policy, so a revalidation mismatch is worth
reporting. Pass `--clean`, or use a fresh `--cache-dir`, for a verification
that reads nothing from the cache at all.

`--skip-attestation-check` turns the check off. It is an escape hatch that
DFINITY's CI does not use and a verifier never needs: a mismatch report then
cannot tell a reproducibility problem from a substituted artifact. Never use it
when verifying an NNS proposal: a release predating the check is verified with
that release's own version of this script, as described above, which has no
check to skip.
