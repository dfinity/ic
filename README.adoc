...
* Network access to `download.dfinity.systems` (and `download.dfinity.network`
  ...
  `tuf-repo-cdn.sigstore.dev` (the GitHub hosts and Sigstore are not optional:
  the run aborts when the attestation of a CDN checksum file cannot be fetched
  and verified)
...
seconds, before any image is downloaded or the build starts — whether it
