...
if ! podman "${PODMAN_ARGS[@]}" image exists $IMAGE; then
    if ! podman "${PODMAN_ARGS[@]}" pull $IMAGE; then
        ...
        # echo "Error building image $IMAGE, attempting to build it locally."
        # exit 1
        docker() {
            # Preserve "${PODMAN_ARGS[@]}" in the exported function by passing
            # them through a single variable, and unpacking them here.
            PODMAN_ARGS=(${PODMAN_ARGS})
            podman "${PODMAN_ARGS[@]}" "$@" --network=host
        }
        export -f docker
        PODMAN_ARGS="${PODMAN_ARGS[@]}" "$REPO_ROOT"/ci/container/build-image-macos.sh
        unset -f docker
    fi
fi
...
if [ $# -eq 0 ]; then
    set -x
    exec podman "${PODMAN_ARGS[@]}" run $other_args "${PODMAN_RUN_ARGS[@]}" ${PODMAN_RUN_USR_ARGS[@]} -w "$WORKDIR" "$IMAGE" "${USHELL:-/usr/bin/bash}"
else
    set -x
    exec podman "${PODMAN_ARGS[@]}" run $other_args "${PODMAN_RUN_ARGS[@]}" ${PODMAN_RUN_USR_ARGS[@]} -w "$WORKDIR" "$IMAGE" "$@"
fi
