...
class FakeDependencyManager(DependencyManager):
...
def test_on_periodic_job_no_findings(jira_lib_mock):
    ...
    sub1 = Mock()
    sub2 = Mock()
    scanner_job = DependencyScanner(FakeDependencyManager(1), jira_lib_mock, [sub1, sub2])
...
def test_on_periodic_job_delete_finding(jira_lib_mock):
    ...
    scanner_job = DependencyScanner(FakeDependencyManager(1), jira_lib_mock, [])
...
def test_on_periodic_job_one_finding(jira_lib_mock):
    ...
    sub1 = Mock()
    sub2 = Mock()
    fake_manager = FakeDependencyManager(2)
    scanner_job = DependencyScanner(fake_manager, jira_lib_mock, [sub1, sub2])
    ...
    finding = fake_manager.get_findings(repository.name, repository.projects[0], repository.engine_version)[0]
...
def test_on_periodic_job_one_finding_in_jira(jira_lib_mock):
    ...
    finding = fake_manager.get_findings(repos[0].name, repos[0].projects[0], repository.engine_version)[0]
...
def test_on_periodic_job_one_finding_in_jira_transition_to_failover(jira_lib_mock):
    ...
    scanner_job = DependencyScanner(fake_manager, jira_lib_mock, [sub1, sub2], failover_mock)
    repos = [
        Repository(
            "ic",
            "https://github.com/dfinity/ic",
    ...
    expected_finding = fake_manager.get_findings(repos[0].name, repos[0].projects[0], repository.engine_version)[0]
    ...
    failover_mock.store_findings.assert_called_once_with(repository.name, SCANNER_ID, [expected_finding])
...
def test_on_periodic_job_one_finding_in_jira_clear_risk_and_keep_risk_note(jira_lib_mock):
    ...
    repository = Repository("ic", "https://github.com/dfinity/ic", [Project("ic", __test_get_ic_path())])
    fake_manager = FakeDependencyManager(2)
    jira_finding = fake_manager.get_findings(repository.name, repository.projects[0], repository.engine_version)[0]
    ...
    scanner_job = DependencyScanner(fake_manager, jira_lib_mock, [])
...
def test_on_periodic_job_failure(jira_lib_mock):
    ...
    fake_manager = Mock()
    fake_manager.get_findings.side_effect = OSError("Call failed")
    fake_manager.get_scanner_id.return_value = SCANNER_ID
...
def test_get_findings_ic_dir(shutil, clone_repo, jira_lib_mock):
    ...
    fake_manager.get_findings.assert_called_once_with("ic", project, repo.engine_version)
...
def test_get_findings_external_dir(shutil, clone_repo, jira_lib_mock):
    with patch("pathlib.Path.is_dir", return_value=True):
        ...
        fake_manager.get_findings.assert_called_once_with("cycles-wallet", project, repo.engine_version)
...
def test_on_merge_request_no_changes_to_dependency_files(jira_lib_mock):
    ...
    sub2 = Mock()
    fake_manager = Mock()
    fake_manager.has_dependencies_changed.return_value = {"external_crates_bzl": False, "cargo_lock_toml": False}
    scanner_job = DependencyScanner(fake_manager, jira_lib_mock, [sub1, sub2])

    scanner_job.do_merge_request_scan("ic")
    fake_manager.get_modified_packages.assert_not_called()
    fake_manager.get_dependency_diff.assert_not_called()
    fake_manager.get_findings.assert_not_called()
    sub1.on_scan_job_succeeded.assert_called_once()
    sub2.on_scan_job_succeeded.assert_called_once()
    sub1.on_scan_job_failed.assert_not_called()
    sub2.on_scan_job_failed.assert_not_called()
...
def test_on_merge_request_changes_to_dependency_files_but_not_external_crates(jira_lib_mock):
    ...
    sub2 = Mock()
    fake_manager = Mock()
    fake_manager.has_dependencies_changed.return_value = {"external_crates_bzl": False, "cargo_lock_toml": True}
    scanner_job = DependencyScanner(fake_manager, jira_lib_mock, [sub1, sub2])

    scanner_job.do_merge_request_scan("ic")
    fake_manager.get_modified_packages.assert_called_once()
    fake_manager.get_dependency_diff.assert_called_once()
    fake_manager.get_findings.assert_not_called()
    sub1.on_scan_job_succeeded.assert_called_once()
    sub2.on_scan_job_succeeded.assert_called_once()
    sub1.on_scan_job_failed.assert_not_called()
...
def test_on_merge_request_changes_no_findings(jira_lib_mock):
    ...
    sub2 = Mock()
    fake_manager = Mock()
    fake_manager.has_dependencies_changed.return_value = {"external_crates_bzl": True, "cargo_lock_toml": True}
    fake_manager.get_modified_packages.return_value = ["package1", "package2"]
    fake_manager.get_findings.return_value = []
    scanner_job = DependencyScanner(fake_manager, jira_lib_mock, [sub1, sub2])
    ...
    fake_manager.get_modified_packages.assert_called_once()
    fake_manager.get_dependency_diff.assert_called_once()
    fake_manager.get_findings.assert_called_once()
    jira_lib_mock.get_open_finding.assert_not_called()
    sub1.on_scan_job_succeeded.assert_called_once()
    sub2.on_scan_job_succeeded.assert_called_once()
    sub1.on_scan_job_failed.assert_not_called()
...
def test_on_merge_request_changes_with_findings_to_flag_no_commit_exception(github_comment_mock, jira_lib_mock):
    ...
    fake_manager.get_modified_packages.return_value = ["foo"]
    fake_manager.get_dependency_diff.return_value = [Dependency("VDID3", "fl dep", "0.1 beta", {"VID1": ["3.0 alpha"]})]
    fake_manager.get_findings.return_value = [
        Finding(
            repository=repository,
            scanner=SCANNER_ID,
...
def test_on_merge_request_job_failed(jira_lib_mock):
    ...
    fake_manager = Mock()
    fake_manager.has_dependencies_changed.side_effect = OSError("Call failed")
