...
fn random_key<R: rand::CryptoRng + rand::Rng>(rng: &mut R) -> PrivateKey {
    PrivateKey::generate_from_seed(&rng.random::<[u8; 32]>())
}
...
fn batch_verification_works() {
    ...
    fn two_positions<R: Rng>(max: usize, rng: &mut R) -> (usize, usize) {
        ...
        let pos0 = rng.random::<usize>() % max;

        loop {
            let pos1 = rng.random::<usize>() % max;
            if pos0 != pos1 {
                return (pos0, pos1);
            }
        }
    ...
    for batch_size in (1..30).chain([50, 75, 100]) {
        ...
        let corrupted_sig_idx = rng.random::<usize>() % batch_size;
        let corrupted_sig_byte = rng.random::<usize>() % 64;
        let corrupted_sig_mask = std::cmp::max(1, rng.random::<u8>());
        ...
        let corrupted_msg_idx = rng.random::<usize>() % batch_size;
        let corrupted_msg_byte = rng.random::<usize>() % 32;
        let corrupted_msg_mask = std::cmp::max(1, rng.random::<u8>());
        ...
        let corrupted_pk_idx = rng.random::<usize>() % batch_size;
    ...
    for n in 1..testcases.len() {
        ...
        let (bkeys, bsigs) = {
            ...
            for _ in 0..n {
                ...
                let idx = rng.random::<usize>() % testcases.len();
