...
Browser-side library that bridges the [WebMCP](https://webmcp.link/) standard to Internet Computer canisters via [`@dfinity/agent`](https://github.com/dfinity/agent-js).
...
@dfinity/webmcp (this package)
  ...
  ├── Maps tool calls → @dfinity/agent query/call
...
Returns the underlying `@dfinity/agent` HttpAgent (available after `registerAll()`).
...
- [`@dfinity/agent`](https://github.com/dfinity/agent-js)
