...
### Added
- If the `registry` ICP feature is enabled, then a new subnet is created in PocketIC (enhancing its topology) for every new subnet record added to the registry, e.g., by calling the registry canister's `create_subnet` endpoint.
  All settings of such a subnet record are preserved, except for the subnet's membership, DKG interval length, and chain key config which must be generated by PocketIC itself.

### Fixed
- PocketIC stores the whole routing table in a single canister ranges registry shard, but did not delete the remaining shards (e.g., created by the registry canister which splits the routing table into multiple shards) resulting in duplicate canister ranges.
