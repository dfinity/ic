...
use crate::icrc::generic_value_predicate::is_account;
...
///
/// Stricter than [`validate_freeze_account`]: `caller`, `mthd`, and `ts` are required.
...
///
/// Stricter than [`validate_unfreeze_account`]: `caller`, `mthd`, and `ts` are required.
...
///
/// Stricter than [`validate_freeze_principal`]: `caller`, `mthd`, and `ts` are required.
...
///
/// Stricter than [`validate_unfreeze_principal`]: `caller`, `mthd`, and `ts` are required.
...
pub fn validate(block: &Value) -> Result<(), ValuePredicateFailures> {
    or(vec![
        Arc::new(|v| validate_freeze_account(&v)),
        Arc::new(|v| validate_unfreeze_account(&v)),
        Arc::new(|v| validate_freeze_principal(&v)),
        Arc::new(|v| validate_unfreeze_principal(&v)),
    ])(Cow::Borrowed(block))
}
...
mod tests {
    ...
    fn test_validate_freeze_account_full() {
        assert!(validate_freeze_account(&full_freeze_account_block()).is_ok());
    }

    #[test]
    fn test_validate_unfreeze_account_full() {
        assert!(validate_unfreeze_account(&full_unfreeze_account_block()).is_ok());
    }

    #[test]
    fn test_validate_freeze_principal_full() {
        assert!(validate_freeze_principal(&full_freeze_principal_block()).is_ok());
    }

    #[test]
    fn test_validate_unfreeze_principal_full() {
        assert!(validate_unfreeze_principal(&full_unfreeze_principal_block()).is_ok());
    }
    ...
    fn test_validate_freeze_account_minimal() {
        assert!(validate_freeze_account(&minimal_freeze_account_block()).is_ok());
    }

    #[test]
    fn test_validate_unfreeze_account_minimal() {
        assert!(validate_unfreeze_account(&minimal_unfreeze_account_block()).is_ok());
    }

    #[test]
    fn test_validate_freeze_principal_minimal() {
        assert!(validate_freeze_principal(&minimal_freeze_principal_block()).is_ok());
    }

    #[test]
    fn test_validate_unfreeze_principal_minimal() {
        assert!(validate_unfreeze_principal(&minimal_unfreeze_principal_block()).is_ok());
    }
    ...
    #[test]
    fn test_validate_freeze_account_rejects_missing_account() {
        let block = Value::map([
            ("btype", Value::text(BTYPE_123_FREEZE_ACCOUNT)),
            ("ts", Value::Nat(1_000_000_000_u64.into())),
    ...
    #[test]
    fn test_validate_unfreeze_account_rejects_missing_account() {
        let block = Value::map([
            ("btype", Value::text(BTYPE_123_UNFREEZE_ACCOUNT)),
            ("ts", Value::Nat(1_000_000_000_u64.into())),
            ("tx", Value::map::<&str, [(&str, Value); 0]>([])),
        ]);
        assert!(validate_unfreeze_account(&block).is_err());
    }

    #[test]
    fn test_validate_freeze_principal_rejects_missing_principal() {
        let block = Value::map([
            ("btype", Value::text(BTYPE_123_FREEZE_PRINCIPAL)),
            ("ts", Value::Nat(1_000_000_000_u64.into())),
            ("tx", Value::map::<&str, [(&str, Value); 0]>([])),
    ...
    #[test]
    fn test_validate_unfreeze_principal_rejects_missing_principal() {
        let block = Value::map([
            ("btype", Value::text(BTYPE_123_UNFREEZE_PRINCIPAL)),
            ("ts", Value::Nat(1_000_000_000_u64.into())),
            ("tx", Value::map::<&str, [(&str, Value); 0]>([])),
        ]);
        assert!(validate_unfreeze_principal(&block).is_err());
    }
    ...
    fn test_validate_accepts_all_block_types() {
        assert!(validate(&full_freeze_account_block()).is_ok());
        assert!(validate(&full_unfreeze_account_block()).is_ok());
        assert!(validate(&full_freeze_principal_block()).is_ok());
        assert!(validate(&full_unfreeze_principal_block()).is_ok());
    }

    #[test]
    fn test_validate_accepts_minimal_blocks() {
        assert!(validate(&minimal_freeze_account_block()).is_ok());
        assert!(validate(&minimal_unfreeze_account_block()).is_ok());
        assert!(validate(&minimal_freeze_principal_block()).is_ok());
        assert!(validate(&minimal_unfreeze_principal_block()).is_ok());
    }
