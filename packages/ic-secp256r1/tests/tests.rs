...
fn private_derivation_is_compatible_with_public_derivation() {
    ...
    fn random_path<R: Rng>(rng: &mut R) -> DerivationPath {
        let l = 1 + rng.random::<usize>() % 9;
        let path = (0..l).map(|_| rng.random::<u32>()).collect::<Vec<u32>>();
    ...
    for _ in 0..100 {
        ...
        let chain_code = rng.random::<[u8; 32]>();
...
fn private_derivation_also_works_for_derived_keys() {
    ...
    for _ in 0..100 {
        ...
        let chain_code = rng.random::<[u8; 32]>();
        let path_len = 2 + rng.random::<usize>() % 32;
        let path = (0..path_len)
        ...
        let split = rng.random::<usize>() % (path_len - 1);
