...
if [[ $((size_level)) -eq 0 ]]; then
    ...
    deploy_with_timeout "$testnet" --no-boundary-nodes --no-api-nodes --dkg-interval-length 20 --git-revision "$GIT_REVISION" "${HOSTS_INI_ARGUMENTS[@]}"
else
    # Deploy to testnet
    deploy_with_timeout "$testnet" --no-boundary-nodes --no-api-nodes \
...
curl -XPOST -G "https://ic-metrics-prometheus-staging.ch1-obsstage1.dfinity.network/api/v1/query_range" \
