...
Title:: Subnet handles cow safety workloads

Goal:: Ensure IC responds to cow safety exercising calls of a given size in a timely manner.

Runbook::
. set up the testnet (nns + subnet installation)
. start the workload generator with calls exercising cow safety
. wait (configurable)
...
function exit_usage() {
    ...
    echo >&2 "$0 p2p_15_28 30 40 250b normal boundary_nodes ./results/"
    exit 1
}
...
experiment_dir="$results_dir/subnet_cow_safety_workload_test_${testnet}-rt_${exec_time}-rate_${rate}-payload_${payload_size}-subnet_${subnet_type}-dest_${load_dest}-$(date +%s)"

# shellcheck disable=SC1090
source "${HELPERS:-$(dirname "${BASH_SOURCE[0]}")/include/helpers.sh}"

if [[ "$subnet_type" != "normal" ]] && [[ "$subnet_type" != "large" ]]; then
    echo >&2 "Invalid subnet type '$subnet_type' specified, choose between 'normal' and 'large'."
    exit_usage
fi
...
echo "Starting Testcase subnet_cow_safety_workload"
...
(
    {
        ...
        timeout -k 300 $((exec_time + 600)) ic-workload-generator \
            "$loadhosts" -m CowSafetyA -p 9090 \
            --canister="$MEDIA_CANISTERS_PATH/cow_safety.wasm" \
            -r "$rate" \
