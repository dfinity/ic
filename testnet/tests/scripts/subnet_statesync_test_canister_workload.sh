...
Title:: Subnet handles the statesync-test-canister.wasm workload

Goal:: Ensure IC responds to the calls in statesync-test-canister.wasm of a given size in a timely manner.

Runbook::
. set up the testnet (nns + subnet installation)
. start the workload generator with statesync-test-canister.wasm
. wait (configurable)
...
function exit_usage() {
    echo >&2 "Usage: <testnet_identifier> <exec_time_in_seconds> <rate> <subnet_type> <load_dest> <results_dir>"
    echo >&2 "$0 p2p_15_28 30 40 normal boundary_nodes ./results/"
    exit 1
}
...
experiment_dir="$results_dir/subnet_statesync_test_canister_workload_${testnet}-rt_${exec_time}-rate_${rate}-subnet_${subnet_type}-dest_${load_dest}-$(date +%s)"

# shellcheck disable=SC1090
source "${HELPERS:-$(dirname "${BASH_SOURCE[0]}")/include/helpers.sh}"

if [[ "$subnet_type" != "normal" ]] && [[ "$subnet_type" != "large" ]]; then
    echo >&2 "Invalid subnet type specified, choose between normal and large."
    exit_usage
fi
...
(
    {
        ...
        timeout -k 300 $((exec_time + 600)) ic-workload-generator \
            "$loadhosts" -m StateSyncA -p 9090 \
            --canister="$MEDIA_CANISTERS_PATH/statesync-test-canister.wasm" \
            -r "$rate" \
            -n "$exec_time" \
            --periodic-output \
...
# 200/202 are the status of good message, everything else is a faulty one
...
status_bad_and_unknown="$((status_total - status_200_good - status_202_good))"
...
has_http_failure="$(jq -r '.[0].status_counts | keys | inside(["0", "11", "33", "44", "200", "202", "503"]) | not' <"$experiment_dir/workload-summary.json")"

sed -i "s/failed_requests_percentage/$bad_percentage/g" "$experiment_dir/data_to_upload/FailedRequestsPercentage.json"

if [[ $status_200_good == "0" && $status_202_good == "0" ]]; then
    failure "There were no good requests, check '$experiment_dir/workload-summary.json'"
elif [[ $status_200_good == "0" ]]; then
    failure "There were no good read requests, check '$experiment_dir/workload-summary.json'"
elif [[ $status_202_good == "0" ]]; then
    failure "There were no good write requests, check '$experiment_dir/workload-summary.json'"
