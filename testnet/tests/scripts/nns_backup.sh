...
pull_artifacts_result=0
function pull_artifacts() {
    NODE_IP=$1
    DIR=$2
    HEIGHT=$3
    STOP_AFTER_HEIGHTS=$4
    echo "Pulling artifacts from node $NODE_IP..."
    # set +x
    for (( ; STOP_AFTER_HEIGHTS > 0; STOP_AFTER_HEIGHTS--)); do
        TAR="$DIR/$HEIGHT.tar.gz"
        CODE=$(curl -s -o "$TAR" -w "%{http_code}" "http://[$NODE_IP]:8080/_/artifacts/$HEIGHT")
        if [ ! "$CODE" -eq 200 ]; then
            echo "No artifacts for height $HEIGHT"
            rm -f "$TAR"
            break
        fi
        tar xf "$TAR" -C "$DIR"
        rm "$TAR"
        pull_artifacts_result="$HEIGHT"
        HEIGHT=$((HEIGHT + 1))
    done
    # set -x
}
...
ssh-keygen -R "$nns_ip"
scp -o StrictHostKeyChecking=no -r "readonly@[${nns_ip}]:/run/ic-node/config/ic.json5" "${results_dir}"
rsync -az "readonly@[${nns_ip}]:/var/lib/ic/data/ic_registry_local_store/" "${results_dir}/ic_registry_local_store/"

backup_dir="$results_dir/backup"
mkdir "$backup_dir"

pull_artifacts "$nns_ip" "$backup_dir" 0 210
...
SUCCESS=0
retries=25
last_pulled_height=$((POST_UPGRADE_HEIGHT - 1))
