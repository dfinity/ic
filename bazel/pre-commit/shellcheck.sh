...
find testnet/tests/scripts/ -maxdepth 1 -type f -name "*.sh" -exec "$SHELLCHECK" --source-path=include {} \+

# Run shellcheck in for testnet/tests/scripts/test_modules/*.sh
find testnet/tests/scripts/test_modules/ -maxdepth 1 -type f -name "*.sh" -exec "$SHELLCHECK" --source-path=include {} \+
