...
The crates used in our build are read from the Cargo build files (`Cargo.toml`s, `Cargo.lock`) and may include some overrides defined in `bazel/external_crates.bzl`.

Changes to the Cargo build require regeneration of `Cargo.Bazel.*.lock` with `./bin/bazel-pin.sh`:

Below is an example of adding a new third-party crate.

1.  Use e.g. `cargo add` to add a new dependency
2.  `cargo update --workspace` to regenerate the `Cargo.lock`
2.  `./bin/bazel-pin.sh` to regenerate the `Cargo.Bazel.*.json` files

Make sure the commit the changes to the `Cargo.Bazel.*.json` files.
