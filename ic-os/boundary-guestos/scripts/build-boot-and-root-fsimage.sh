...
if [ "${BOOT_FILE_CONTEXTS}" != "" ]; then
    ...
    # It appears the context of the root inode is not set correctly. Fix it up.
    ROOT_CONTEXT=$(sed -e 's#^/boot[ \t]\+-d[ \t]\+##' -e 't' -e 'd' <"${FILE_CONTEXTS}")
    echo Setting root inode context of 'boot' image to "${ROOT_CONTEXT}"
    echo 'ea_set / security.selinux "'"${ROOT_CONTEXT}"'\000"' | debugfs -w "${BOOT_IMAGE}"
    echo
else
...
if [ "${ROOT_FILE_CONTEXTS}" != "" ]; then
    ...
    # It appears the context of the root inode is not set correctly. Fix it up.
    ROOT_CONTEXT=$(sed -e 's#^/[ \t]\+-d[ \t]\+##' -e 't' -e 'd' <"${FILE_CONTEXTS}")
    echo Setting root inode context of 'root' image to "${ROOT_CONTEXT}"
    echo 'ea_set / security.selinux "'"${ROOT_CONTEXT}"'\000"' | debugfs -w "${ROOT_IMAGE}"
    echo
else
