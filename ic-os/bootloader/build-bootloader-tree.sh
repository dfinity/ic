...
trap 'rm -rf "${TMPDIR}"; podman rm -f "${CONTAINER}"' exit
...
podman build --iidfile ${TMPDIR}/iidfile - <<<"
    FROM $BASE_IMAGE
    USER root:root
...
CONTAINER=$(podman run -d "$IMAGE")

podman export "$CONTAINER" | tar -C "$TMPDIR" -x build --strip-components=1
