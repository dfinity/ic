...
To perform this exchange, the HostOS launches a temporary "upgrade VM" alongside the active GuestOS on the same physical CPU. The full sequence is:
...
4. The active GuestOS derives the disk encryption key and sends it to the upgrade VM over the attested channel. The upgrade VM writes the key to disk and shuts down.
