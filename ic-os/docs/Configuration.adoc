...
This process is controlled by the (link:../../rs/ic_os/config/tool/README.md[ic-os config tool]) and an assortment of bash scripts.
...
Refer to link:../../rs/ic_os/config/tool/README.md[rs/ic_os/config/tool] and link:../components/setupos/setup-hostos-config.sh[setup-hostos-config.sh] for more details.

== HostOS -> GuestOS

HostOS creates a bootstrap config image containing the GuestOS configuration files.

Refer to link:../../rs/ic_os/os_tools/guest_vm_runner/src/guest_vm_config.rs[guest_vm_config.rs] and link:../../rs/ic_os/config/tool/src/hostos/guestos_bootstrap_image.rs[guestos_bootstrap_image.rs] for more details.
...
When the GuestOS boots, it checks for available removable media devices (i.e. the bootstrap config image). If such a device is found, the media must contain a VFAT filesystem.

GuestOS then reads from the bootstrap config image (link:../components/guestos/init/init-config/init-config.sh[see init-config.sh]). For backward compatibility with older HostOS versions, it supports both:

* Legacy tar-based configuration: A single file called `ic-bootstrap.tar`
* Direct file-based configuration: Files copied directly to the bootstrap config image

Only `config.json` is unpacked and copied into `/run/config`.

=== First Boot Bootstrap

On first boot, GuestOS bootstraps the node by copying config files from the bootstrap config image. See
link:../../rs/ic_os/config/tool/README.md[rs/ic_os/config/tool].
...
3. Add interpretation of configuration file/directory to suitable place, e.g. the config tool's template rendering or init scripts.
...
Consider that values may be controlled by an attacker on boot. Bootstrapping a node with bogus values can cause the node to malfunction. Our security goal is to disallow a Node Technician to gain control over the node via code injection or similar.
...
link:../../rs/ic_os/config/tool/README.md[rs/ic_os/config/tool]. Or, you may find it easier to update *guestos_bootstrap_image.rs* and *bootstrap_ic_node.rs* directly,
...
The behavior is triggered through the presence of the following files in the bootstrap config image:

- `ic_crypto` - Crypto key material
- `ic_registry_local_store` - Initial registry state
- `ic_state` - Initial IC state

*SEV/TEE Restrictions:* When SEV (Secure Encrypted Virtualization) is active on production builds, state injection is blocked for security reasons. This prevents a potentially malicious node operator from injecting arbitrary state into a TEE-protected node. State injection is only allowed when:

- SEV is not active, OR
- The GuestOS is built with the `dev` feature flag (for testing purposes)
