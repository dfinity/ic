...
def prep_image(name, image, config):
    config_img = "/tmp/config-%s.img" % name

    subprocess.run(["truncate", "--size", "100M", config_img], check=True)
    subprocess.run(["make_ext4fs", "-T", "0", "-l", "100M", config_img, config], check=True)
    config_script = os.path.join(os.path.dirname(__file__), "write_config_part.sh")
    subprocess.run([config_script, config_img, image], check=True)


def start_machine_local(machine, disk_image, config, interactive=False):
    ...
    prep_image(name, machine_disk, config)
...
def start_machines(items, start_ssh_log_streaming=True):
    for machine, disk_image, config in items:
        start_machine_local(machine, disk_image, config)
