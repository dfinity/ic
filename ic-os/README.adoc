...
All IC-OS image build outputs are stored under `bazel-bin/ic-os/{setupos,hostos,guestos}/envs/<TARGET>` (relative to the repository root)

Example:

   $ bazel build //ic-os/guestos/envs/dev/...
   # This will output a GuestOS image in bazel-bin/ic-os/guestos/envs/dev
