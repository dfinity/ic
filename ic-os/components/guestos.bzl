...
def component_files(mode):
    return {
        ...
        # Unlike other rust binaries, we include open_rootfs here, so that it can be built into the initramfs
        (Label("//rs/ic_os/open_rootfs:open_rootfs_dev") if "dev" in mode else Label("//rs/ic_os/open_rootfs")): "/opt/ic/bin/open_rootfs",
