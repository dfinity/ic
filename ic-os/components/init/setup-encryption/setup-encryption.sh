...
umask 0077
mkdir -p /var/config
dd if=/dev/random of=/var/config/store.keyfile bs=16 count=1
...
cryptsetup luksFormat --type luks2 --pbkdf pbkdf2 --pbkdf-force-iterations 1000 /dev/vda10 /var/config/store.keyfile
