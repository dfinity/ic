...
def launch_bare_metal(name, image_zst_file):
    ...
    native.sh_binary(
        ...
        args = [
            ...
            "$(location //rs/ic_os/dev_tools/setupos-inject-configuration)",
            "--upload_img",
            "$(location " + image_zst_file + ")",
            "--idrac_script_dir",
            "$(location //ic-os/dev-tools/bare_metal_deployment:find_idrac_package_path)",
        ],
        data = [":" + binary_name, image_zst_file, "//rs/ic_os/dev_tools/setupos-inject-configuration", "//ic-os/dev-tools/bare_metal_deployment:find_idrac_package_path"],
        tags = ["manual"],
    )
