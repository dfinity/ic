...
def icos_build(
    ...
    if "extra_boot_args_template" not in image_deps:
        ...
        native.genrule(
            ...
            cmd = "export USE_TMPFS=true; $(location //toolchains/sysimage:proc_wrapper) $(location //toolchains/sysimage:verity_sign) -i $< -o $(location :partition-root.tzst) -r $(location partition-root-hash) --dflate $(location //rs/ic_os/build_tools/dflate)",
            executable = False,
        ...
        if upgrades:
            native.genrule(
                ...
                cmd = "export USE_TMPFS=true; $(location //toolchains/sysimage:proc_wrapper) $(location //toolchains/sysimage:verity_sign) -i $< -o $(location :partition-root-test.tzst) -r $(location partition-root-test-hash) --dflate $(location //rs/ic_os/build_tools/dflate)",
                tools = ["//toolchains/sysimage:proc_wrapper", "//toolchains/sysimage:verity_sign", "//rs/ic_os/build_tools/dflate"],
                tags = ["manual", "no-cache"],
            )
