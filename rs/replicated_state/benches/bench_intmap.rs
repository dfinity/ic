...
use ic_replicated_state::page_map::int_map2::IntMap as IntMap0;
...
fn bench_intmap(c: &mut Criterion<ProcessTime>) {
    ...
    for &n in BENCH_SIZES.iter() {
        ...
        group.bench_function(BenchmarkId::new("patricia0", n), |b| {
            b.iter(|| {
                let m: IntMap0<_> = (0..n).map(|x| (x * 13 % n, value(x))).collect();
                black_box(m);
            })
        });
    ...
    for &n in BENCH_SIZES.iter() {
        ...
        let patricia_map0: IntMap0<Value> = (0..n).map(kv).collect();
        ...
        group.bench_function(BenchmarkId::new("patricia0", n), |b| {
            b.iter(|| {
                for i in 0..n {
                    black_box(patricia_map0.get(i * N));
                    black_box(patricia_map0.get(i * N + 3));
                }
            });
        });
        group.bench_function(BenchmarkId::new("mpatricia", n), |b| {
    ...
    for &n in BENCH_SIZES.iter() {
        ...
        let patricia_lmap0: IntMap0<Value> = (0..n).map(|x| (x, value(x))).collect();
        let patricia_rmap0: IntMap0<Value> = (n / 2..n + n / 2).map(|x| (x, value(x))).collect();
        ...
        group.bench_function(BenchmarkId::new("patricia0", n), |b| {
            b.iter_batched(
                || (patricia_lmap0.clone(), patricia_rmap0.clone()),
                |(l, r)| {
                    black_box(l.union(r));
                },
                BatchSize::SmallInput,
    ...
    for &n in BENCH_SIZES.iter() {
        let patricia_map: IntMap<u64, Value, _> = (0..n).map(|x| (x, value(x))).collect();
        let patricia_map0: IntMap0<Value> = (0..n).map(|x| (x, value(x))).collect();
        let mpatricia_map: MutableIntMap<u64, Value, _> = (0..n).map(|x| (x, value(x))).collect();
        let btree_map: Arc<BTreeMap<u64, Value>> =
            Arc::new((0..n).map(|x| (x, value(x))).collect());
        let hash_map: Arc<HashMap<u64, Value>> = Arc::new((0..n).map(|x| (x, value(x))).collect());
