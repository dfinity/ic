...
impl MessagePool {
    ...
    /// message.
    ///
    /// Panics if the ID is known not to have been generated by the pool (i.e. if
    /// it is greater than or equal to `next_message_id`); or if a message with the
    /// same ID is already present in the pool.
    ...
    /// Removes the message with the given ID from the pool if it is present and it
    /// is a request.
    ///
