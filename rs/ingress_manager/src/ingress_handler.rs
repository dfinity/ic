    ...
    fn on_state_change(&self, pool: &T) -> ChangeSet {
        ...
        change_set.extend(unvalidated_artifacts.filter_map(|artifact| {
            ...
            match self.validate_ingress_(
                ...
                Ok(()) => Some(MoveToValidated(IngressMessageId::from(ingress_object))),
                Err(err) => {
                    warn!(
                        self.log,
                        "ingress_message_remove_unvalidated";
                        ingress_message.message_id => format!("{}", ingress_object.message_id),
                        ingress_message.reason => format!("{:?}", err),
                    );
                    Some(RemoveFromUnvalidated(IngressMessageId::from(
                        ingress_object,
                    )))
                }
            }
...
impl IngressManager {
    fn validate_ingress_(
        ...
        if ingress_object.signed_ingress.count_bytes() > settings.max_ingress_bytes_per_message {
            return Err(IngressMessageValidationError::IngressMessageTooLarge {
                max: settings.max_ingress_bytes_per_message,
                actual: ingress_object.signed_ingress.count_bytes(),
            });
        }
