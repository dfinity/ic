...
=== Dependencies
The tests start a `replica` process for each node. Thus, when running the tests (`cargo test testcase_`), a `replica` and a `nodemanager` binary needs to be on the `$PATH`.
