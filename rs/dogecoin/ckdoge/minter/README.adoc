...
TIP: To interact with the minter or ledger as exemplified below with `dfx`, make sure to locate yourself in `rs/dogecoin/ckdoge/mainnet`.
...
You now have some ckDOGE, you can enjoy the Internet Computer speed and low fees!
...
----
+
2. Call the `retrieve_btc_with_approval` endpoint with the desired DOGE destination address where you want to receive your Dogecoin.
   Replace DOGE_ADDRESS with your DOGE address (the minter supports all address formats).
   Replace AMOUNT with the amount that you transferred minus the transfer fee of 0.0000001 ckDOGE (the equivalent of 10 Satoshi).
+
----
dfx canister --network ic call minter retrieve_btc_with_approval "(record {address=\"DOGE_ADDRESS\"; amount=AMOUNT;})"
----
+
...
====
There is an example implementation of how to decode the ckDOGE ledger memos in Typescript in the https://github.com/dfinity/oisy-wallet/tree/main[Oisy Wallet] repository:

* https://github.com/dfinity/oisy-wallet/blob/384013a67a994555b6bd1cd6baeba0a83c444839/src/frontend/src/icp/utils/ckbtc-memo.utils.ts[Decode ckDOGE ledger burn and mint memo]
* https://github.com/dfinity/oisy-wallet/blob/384013a67a994555b6bd1cd6baeba0a83c444839/src/frontend/src/tests/icp/utils/ckbtc-memo.utils.spec.ts[Tests to decode ckDOGE ledger memo]
====
