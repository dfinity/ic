...
/// Similar to ic_cdk_bitcoin_canister but for Dogecoin
mod dogecoin_canister {
    ...
    pub use ic_cdk_bitcoin_canister::SendTransactionRequest;
    ...
    pub async fn dogecoin_get_utxos(arg: &GetUtxosRequest) -> CallResult<GetUtxosResponse> {
        ...
        let cycles = ic_cdk_bitcoin_canister::cost_get_utxos(arg);
    ...
    pub async fn dogecoin_get_fee_percentiles(
        ...
        let canister_id = get_dogecoin_canister_id(&into_dogecoin_network(arg.network.into()));
        // same cycles cost as for the Bitcoin canister
        let cycles = ic_cdk_bitcoin_canister::cost_get_current_fee_percentiles(arg);
        Ok(
    ...
    pub async fn dogecoin_send_transaction(arg: &SendTransactionRequest) -> CallResult<()> {
        let canister_id = get_dogecoin_canister_id(&into_dogecoin_network(arg.network.into()));
        // same cycles cost as for the Bitcoin canister
        let cycles = ic_cdk_bitcoin_canister::cost_send_transaction(arg);
    ...
    fn into_dogecoin_network(network: ic_cdk_bitcoin_canister::Network) -> Network {
        match network {
            ic_cdk_bitcoin_canister::Network::Mainnet => Network::Mainnet,
            ic_cdk_bitcoin_canister::Network::Testnet => {
                panic!("Network {network:?} is not supported!")
            }
            ic_cdk_bitcoin_canister::Network::Regtest => Network::Regtest,
        }
    }
}
