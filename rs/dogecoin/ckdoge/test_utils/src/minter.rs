...
use canlog::LogEntry;
use ic_ckdoge_minter::candid_api::WithdrawalFee;
...
impl MinterCanister {
    ...
    pub fn estimate_withdrawal_fee(&self, withdrawal_amount: u64) -> WithdrawalFee {
        let call_result = self
            .env
            .query_call(
                self.id,
        ...
        Decode!(&call_result, WithdrawalFee).unwrap()
    }
