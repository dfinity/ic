...
// - Rule order within a config is significant, as rules are applied in the order they appear in the config.
// - Adding a new config requires providing the entire list of rules and increments the version by one.
// - Each rule has two IDs: rule_id (unique identifier) and incident_id (linking the rule to an incident).
// - rule_id is autogenerated by hashing JSON-decoded `rule_raw` + `description`.
// - Each rule must be linked to some incident_id, this ID must be provided for each input rule be the caller; multiple rules can be linked to one incident_id.
// - Rules can persist across config versions if resubmitted.
// - Non-resubmitted rules are marked as "deactivated" (StorableRuleMetadata.removed_in_version = Some()).
// - Individual rules or incidents (set of rules with the same incident_id) can be disclosed. This means that a rule can be viewed by the callers with `RestrictedRead` access level.
// - Disclosing rules or incidents multiple times has no additional effect.

// Policies:
// - Existing rules cannot be modified; a new rule must be created for changes
