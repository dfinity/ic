...
impl<K: Send + Sync + Hash + Eq + Clone + 'static> ShardedRatelimiter<K> {
    pub fn new(limit: u32, dur: Duration, tti: Duration, max_shards: u64) -> Self {
        let shards = Cache::builder()
            .time_to_idle(tti)
            .max_capacity(max_shards)
            .build();

        Self { shards, limit, dur }
    }
...
mod test {
    ...
    fn test_sharded() {
        let s: ShardedRatelimiter<String> =
            ShardedRatelimiter::new(10, Duration::from_secs(1), Duration::from_secs(5), 1000);
