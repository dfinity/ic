...
#[test]
fn test_get_routing_table() -> Result<(), Error> {
    let reg = Arc::new(create_fake_registry_client(4));
    let rt_init = ArcSwapOption::const_empty();
    let mut runner = Runner::new(&rt_init, reg);

    let rt = runner.get_routing_table().unwrap();
    ...
    let nodes = vec![
        ("y7s52-3xjam-aaaaa-aaaap-2ai", "192.168.0.1", 8080),
        ("ftjgm-3pkam-aaaaa-aaaap-2ai", "192.168.0.2", 8080),
        ("ymia2-u7lam-aaaaa-aaaap-2ai", "192.168.0.3", 8080),
        ("ehgbm-kxmam-aaaaa-aaaap-2ai", "192.168.0.4", 8080),
    ];
    ...
    for i in 0..rt.subnets.len() {
        ...
        assert_eq!(
            sn.nodes[0].tls_certificate_der,
