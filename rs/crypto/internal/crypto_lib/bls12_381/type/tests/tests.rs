...
fn test_scalar_comparison() {
    ...
    for _ in 0..300 {
        let a = Scalar::from_u32(rng.random::<u32>());
        let b = Scalar::from_u32(rng.random::<u32>());
...
fn test_verify_bls_signature() {
    ...
    let message = G1Affine::hash(b"bls_signature", &rng.random::<[u8; 32]>());
...
fn with_random_new_msgs_signed_by_existing_keys(
    ...
    for _ in 0..n {
        ...
        let rand_new_msg = G1Affine::hash(b"bls_signature", &rng.random::<[u8; 32]>());
    ...
    ($batch_verification_function:ident) => {
        ...
        for num_inputs in [1, 2, 4, 8, 16, 32, 100] {
            ...
            let msgs: Vec<_> = (0..num_inputs)
                .map(|_| G1Affine::hash(b"bls_signature", &rng.random::<[u8; 32]>()))
                .collect();
            let sigs: Vec<_> = sks
                .iter()
                .zip(msgs.iter())
                ...
                for i in 0..sigs_w_new_msgs.len() {
                    let sigs_w_new_msgs: Vec<_> = sigs_w_new_msgs
                        ...
                        .map(|(j, sig)| {
                            if j == i {
                                // corrupt signature i
                                G1Affine::hash(b"bls_signature", &rng.random::<[u8; 32]>())
                            } else {
                                sig.clone()
                            }
                        })
...
fn test_verify_bls_signature_batch_with_same_msg() {
    ...
    for num_inputs in [1, 2, 4, 8, 16, 32, 100] {
        ...
        let pks: Vec<_> = sks
            ...
            .map(|sk| G2Affine::from(G2Affine::generator() * sk))
            .collect();
        let msg = G1Affine::hash(b"bls_signature", &rng.random::<[u8; 32]>());
...
impl BiasedValue for G1Projective {
    ...
    fn biased<R: RngCore + CryptoRng>(rng: &mut R) -> Self::Output {
        ...
        if coin < 10 {
            ...
            Self::hash(b"random-g1-val-for-testing", &rng.random::<[u8; 32]>())
        }
    }
}
...
impl BiasedValue for G2Projective {
    ...
    fn biased<R: RngCore + CryptoRng>(rng: &mut R) -> Self::Output {
        ...
        if coin < 10 {
            ...
            Self::hash(b"random-g2-val-for-testing", &rng.random::<[u8; 32]>())
        }
    }
}
...
test_point_operation!(serialization_round_trip, [g1, g2], {
    ...
    for _ in 1..30 {
        let orig = Projective::hash(b"serialization-round-trip-test", &rng.random::<[u8; 32]>());
...
test_point_operation!(multiply, [g1, g2, gt], {
    ...
    for _ in 1..300 {
        let lhs = rng.random::<u32>() as u64;
        let rhs = rng.random::<u32>() as u64;
...
test_point_operation!(mul_with_precompute, [g1, g2], {
    ...
    let g = Affine::hash(b"random-point-for-mul-precompute", &rng.random::<[u8; 32]>());
...
test_point_operation!(batch_mul, [g1, g2], {
    ...
    let pt = Affine::hash(b"ic-crypto-batch-mul-test", &rng.random::<[u8; 32]>());
