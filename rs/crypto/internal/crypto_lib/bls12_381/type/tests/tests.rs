...
fn test_g2_deserialize_rejects_infinity_bit_with_nonzero_x() {
    ...
    assert!(G2Affine::deserialize_public_key(&g2_bytes).is_err());
}
...
fn test_g2_deserialize_rejects_out_of_range_x_value() {
    ...
    assert!(G2Affine::deserialize_public_key(&invalid_x0).is_err());
    ...
    assert!(G2Affine::deserialize_public_key(&invalid_x1).is_err());
}
