...
fn conversion_to_individual_signature_fails_gracefully() {
    ...
    assert_matches!(
        IndividualSignature::try_from(&bad_point),
...
fn conversion_to_pop_fails_gracefully() {
    ...
    assert_matches!(
        Pop::try_from(&bad_point),
        Err(CryptoError::MalformedPop {
            algorithm: AlgorithmId::MultiBls12_381,
            ..
        })
    );
...
fn conversion_to_combined_signature_fails_gracefully() {
    ...
    assert_matches!(
        CombinedSignature::try_from(&bad_point),
        Err(CryptoError::MalformedSignature {
            algorithm: AlgorithmId::MultiBls12_381,
            ..
        })
