...
fn fs_keys_should_be_valid() {
    ...
    let key_gen_assoc_data = rng.random::<[u8; 32]>();
...
fn integrity_check_should_return_error_on_wrong_associated_data() {
    ...
    let associated_data = rng.random::<[u8; 32]>();
...
fn should_decrypt_correctly_for_cheating_dealer() {
    let epoch = Epoch::from(0);
    let rng = &mut reproducible_rng();
    let associated_data = rng.random::<[u8; 10]>();
    ...
    let mut sij = {
        ...
        for _ in 0..nodes {
            ...
            for i in 0..NUM_CHUNKS {
                ...
                let chunk = (0x8000 | rng.random::<u16>()) as isize;
                chunks[i] = chunk;
            }
    ...
    let cheating_i = rng.random::<usize>() % nodes;
    let cheating_j = std::cmp::max(1, rng.random::<usize>() % NUM_CHUNKS);

    let delta = (2 + rng.random::<usize>() % 10) as isize;
