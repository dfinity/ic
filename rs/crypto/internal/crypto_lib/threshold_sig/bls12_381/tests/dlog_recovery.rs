...
fn gt_rand<R: Rng>(rng: &mut R) -> Gt {
    let g1 = G1Affine::hash(b"ic-crypto-test-gt-random", &rng.random::<[u8; 32]>());
...
fn baby_giant_big_range() {
    ...
    for _trial in 0..30 {
        let x = Scalar::from_isize((rng.random::<u64>() % upper_bound as u64) as isize + lower_bound);
        let tgt = &base * &x;
...
fn honest_dealer_search_works_randomized_test() {
    ...
    let mut dlogs = (0..500).map(|_| rng.random::<u16>()).collect::<Vec<_>>();
