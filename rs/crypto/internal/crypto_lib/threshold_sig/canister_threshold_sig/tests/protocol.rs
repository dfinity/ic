...
fn random_subset<R: rand::Rng, T: Clone>(
    ...
    while result.len() != include {
        let key_to_add = keys[rng.random::<usize>() % keys.len()];
...
fn should_basic_signing_protocol_work() -> Result<(), CanisterThresholdError> {
    ...
    for cfg in TestConfig::all() {
        ...
        let signed_message = rng.random::<[u8; 32]>().to_vec();
        let random_beacon = Randomness::from(rng.random::<[u8; 32]>());
        ...
        let random_beacon2 = Randomness::from(rng.random::<[u8; 32]>());
...
#[test]
fn should_be_able_to_perform_taproot_signature() -> Result<(), CanisterThresholdError> {
    let mut rng = &mut reproducible_rng();
    ...
    for msg_len in msg_lens {
        ...
        let taproot_tree_root = rng.random::<[u8; 32]>().to_vec();
        ...
        let proto = Bip340SignatureProtocolExecution::new_with_taproot(
            setup,
            signed_message,
            taproot_tree_root,
            random_beacon,
