...
mod tests {
    use super::*;
    use crate::curves::bls12_381::G2Bytes;
    ...
    fn flags_have_weight_1_and_do_not_overlap() {
        ...
        assert_eq!(weight(G2Bytes::COMPRESSED_FLAG), 1);
        assert_eq!(weight(G2Bytes::INFINITY_FLAG), 1);
        assert_eq!(weight(G2Bytes::SIGN_FLAG), 1);
        assert_eq!(weight(G2Bytes::NON_FLAG_BITS), 5);
        assert_eq!(
            weight(
                G2Bytes::COMPRESSED_FLAG
                    | G2Bytes::INFINITY_FLAG
                    | G2Bytes::SIGN_FLAG
                    | G2Bytes::NON_FLAG_BITS
            ),
            8
        );
    }
    ...
    fn compression_flag_is_set_for_test_vectors() {
        ...
        {
            assert_eq!(
                hex::decode(hex).unwrap()[G2Bytes::FLAG_BYTE_OFFSET] & G2Bytes::COMPRESSED_FLAG,
                G2Bytes::COMPRESSED_FLAG
            );
        }
    }
    ...
    fn infinity_flag_is_set_for_infinity() {
        assert_eq!(
            hex::decode(INFINITY).unwrap()[G2Bytes::FLAG_BYTE_OFFSET] & G2Bytes::INFINITY_FLAG,
            G2Bytes::INFINITY_FLAG
        );
    }
    #[test]
    fn infinity_flag_is_not_set_for_finite_values() {
        for hex in FINITE_TEST_VECTORS.iter().flat_map(|x| x.iter()) {
            assert_eq!(
                hex::decode(hex).unwrap()[G2Bytes::FLAG_BYTE_OFFSET] & G2Bytes::INFINITY_FLAG,
                0
            );
        }
    }
    ...
    fn sign_flag_distinguishes_negative_from_positive() {
        for (negative, positive) in NEGATIVE_NUMBERS.iter().zip(POSITIVE_NUMBERS) {
            ...
            negated_negative[G2Bytes::FLAG_BYTE_OFFSET] ^= G2Bytes::SIGN_FLAG;
