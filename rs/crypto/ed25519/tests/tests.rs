...
fn secret_key_serialization_round_trips() {
    ...
    for _ in 0..100 {
        ...
        let via_pkcs8 = PrivateKey::deserialize_pkcs8(&key.serialize_pkcs8()).unwrap();
        assert_eq!(key, via_pkcs8);

        let via_pkcs8v1 = PrivateKey::deserialize_pkcs8(&key.serialize_pkcs8_v1()).unwrap();
        assert_eq!(key, via_pkcs8v1);

        let via_pem = PrivateKey::deserialize_pkcs8_pem(&key.serialize_pkcs8_pem()).unwrap();
