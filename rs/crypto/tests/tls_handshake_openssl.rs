...
use crate::tls_utils::test_client_openssl::{OpenSslClient, OpenSslClientBuilder};
use crate::tls_utils::test_server_openssl::{OpenSslServer, OpenSslServerBuilder};
...
mod handshakes {
    ...
    async fn should_perform_tls_handshake_if_multiple_clients_allowed() {
        ...
        let client_2 = OpenSslClient::builder(CLIENT_ID_2, SERVER_ID_1).build(registry.get());
        registry
            .add_cert(SERVER_ID_1, server.cert())
...
mod server_allowing_all_nodes {
    ...
    async fn should_perform_handshake_if_all_nodes_allowed_and_registry_contains_several_nodes() {
        ...
        let server = OpenSslServer::builder(SERVER_ID_1)
            ...
            .build(registry.get());
        let client =
            OpenSslClient::builder(CLIENT_THAT_CONNECTS, SERVER_ID_1).build(registry.get());
        registry
            .add_cert(SERVER_ID_1, server.cert())
    ...
    async fn should_return_error_if_node_record_of_connecting_client_missing() {
        ...
        let server = OpenSslServer::builder(SERVER_ID_1)
            .allow_all_nodes()
            .build(registry.get());
        let client = OpenSslClient::builder(CLIENT_THAT_CONNECTS_WITHOUT_NODE_RECORD, SERVER_ID_1)
            .build(registry.get());
        registry
            .add_cert(SERVER_ID_1, server.cert())
...
mod server {
    ...
    async fn should_return_error_if_client_not_allowed_and_allowed_clients_exist() {
        ...
        let client = OpenSslClient::builder(NOT_ALLOWED_CLIENT, SERVER_ID_1).build(registry.get());
        registry
            .add_cert(SERVER_ID_1, server.cert())
...
mod client {
    ...
    async fn should_connect_even_if_server_rejects_client_cert_but_get_error_on_read() {
        ...
        let server = OpenSslServer::builder(SERVER_ID_1)
            ...
            .build(registry.get());
        assert!(!server.allowed_clients().contains(&NOT_ALLOWED_CLIENT));
        let client = OpenSslClient::builder(NOT_ALLOWED_CLIENT, SERVER_ID_1)
    ...
    async fn should_return_error_if_server_cert_is_issued_by_other_ca_in_registry() {
        ...
        let registry = TlsRegistry::new();
        let client = OpenSslClient::builder(CLIENT_ID_1, SERVER_CA_ID).build(registry.get());
        let ca_cert_key_pair = ed25519_key_pair();
        let leaf_cert_key_pair = ed25519_key_pair();
        let leaf_cert = CertWithPrivateKey::builder()
...
) -> (OpenSslServer, OpenSslClient, TlsRegistry) {
    let registry = TlsRegistry::new();
    let (server_builder, client_builder) =
...
fn matching_server_and_client_builders(
    server_node_id: NodeId,
    client_node_id: NodeId,
) -> (OpenSslServerBuilder, OpenSslClientBuilder) {
    let server = OpenSslServer::builder(server_node_id).add_allowed_client(client_node_id);
    let client = OpenSslClient::builder(client_node_id, server_node_id);
    (server, client)
}
