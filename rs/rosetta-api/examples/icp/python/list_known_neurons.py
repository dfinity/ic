...
def main():
    ...
    if 'result' in response and 'known_neurons' in response['result']:
        ...
        if args.min_stake is not None:
            print(f"Note: Stake information not available in known_neurons response, skipping min_stake filter")
        
        # Sort based on the criteria
        if args.sort_by == "stake":
            print(f"Note: Stake information not available in known_neurons response, sorting by ID instead")
            neurons.sort(key=lambda n: n.get('id', 0))
        elif args.sort_by == "age":
            print(f"Note: Age information not available in known_neurons response, sorting by ID instead")
            neurons.sort(key=lambda n: n.get('id', 0))
