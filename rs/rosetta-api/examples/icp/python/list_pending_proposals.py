...
def main():
    parser = argparse.ArgumentParser(
        description='List pending proposals on the Internet Computer NNS')
    ...
    if 'result' in response and 'pending_proposals' in response['result']:
        ...
        try:
            proposals.sort(key=lambda p: p.get(
                'created_timestamp_seconds', 0), reverse=True)
        ...
        for i, proposal in enumerate(proposals):
            ...
            created_date = datetime.fromtimestamp(created_time).strftime(
                '%Y-%m-%d %H:%M:%S') if created_time else 'Unknown'

            # Format deadline if available
            deadline = proposal.get('deadline_timestamp_seconds', 0)
            deadline_date = datetime.fromtimestamp(deadline).strftime(
                '%Y-%m-%d %H:%M:%S') if deadline else 'No deadline'
            ...
            if proposal_summary:
                max_len = 200
                summary_display = proposal_summary if len(
                    proposal_summary) <= max_len else f"{proposal_summary[:max_len]}..."
