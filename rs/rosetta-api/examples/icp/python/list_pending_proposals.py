...
def main():
    ...
    print(f"Fetching pending proposals from the NNS...")
    ...
    if 'result' in response and 'pending_proposals' in response['result']:
        ...
        for i, proposal in enumerate(proposals):
            ...
            created_date = datetime.fromtimestamp(created_time).strftime('%Y-%m-%d %H:%M:%S') if created_time else 'Unknown'
            
            # Format deadline if available
            deadline = proposal.get('deadline_timestamp_seconds', 0)
            deadline_date = datetime.fromtimestamp(deadline).strftime('%Y-%m-%d %H:%M:%S') if deadline else 'No deadline'
            ...
            if 'status' in proposal:
                ...
                status_text = {
                    ...
                    4: "Executed",
                    5: "Failed"
                }.get(status_code, f"Status code: {status_code}")
            ...
            if 'latest_tally' in proposal:
                ...
                print(f"Votes: {yes_votes / 10**8:.2f} ICP yes ({yes_percentage:.1f}%), {no_votes / 10**8:.2f} ICP no ({no_percentage:.1f}%)")
                print(f"Total voting power: {total_votes / 10**8:.2f} ICP")
            
            # Show summary if available (truncated if too long)
            if proposal_summary:
                max_len = 200
                summary_display = proposal_summary if len(proposal_summary) <= max_len else f"{proposal_summary[:max_len]}..."
