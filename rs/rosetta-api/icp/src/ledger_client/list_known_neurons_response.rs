...
use ic_nns_governance_api::{KnownNeuron, ListKnownNeuronsResponse as Response};
    ...
    fn try_from(d: ListKnownNeuronsResponse) -> Result<ObjectMap, Self::Error> {
        match serde_json::to_value(d) {
            ...
            Ok(o) => Err(ApiError::internal_error(format!(
                "Could not convert ListKnownNeuronsResponse to ObjectMap. Expected type Object but received: {o:?}"
            ))),
            Err(err) => Err(ApiError::internal_error(format!(
                "Could not convert ListKnownNeuronsResponse to ObjectMap: {err:?}"
            ))),
        }
    }
}
    ...
    fn try_from(o: Option<ObjectMap>) -> Result<Self, Self::Error> {
        serde_json::from_value(serde_json::Value::Object(o.unwrap_or_default())).map_err(|e| {
            ApiError::internal_error(format!(
                "Could not parse a `ListKnownNeuronsResponse` from JSON object: {e}"
            ))
        })
    }
}
