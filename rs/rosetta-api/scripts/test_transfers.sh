...
for ((batch = 0; batch < $NUM_OF_BATCHES; batch++)); do
    ...
    for ((t = 0; t < $NUM_OF_TRANSFERS_PER_BATCH; t++)); do
        dfx canister call --candid "$LEDGER_DID" --network "$REPLICA_URL" "$LEDGER_CANISTER_ID" icrc1_transfer "(record {to=record {owner=principal \"$LEDGER_CANISTER_ID\"}; amount=$AMOUNT_PER_TRANSFER},)" &
    done
    wait
