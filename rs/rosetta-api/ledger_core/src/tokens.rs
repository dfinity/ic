...
/// Like [Tokens] but can go negative.
#[derive(Clone, Copy, Hash, Debug, PartialEq, Eq, PartialOrd, Ord)]
pub enum SignedTokens {
    Plus(Tokens),
    Minus(Tokens),
}

impl SignedTokens {
    const MAX128: i128 = u64::MAX as i128;
    const MIN128: i128 = -(u64::MAX as i128);

    pub const MAX: Self = Self::Plus(Tokens::MAX);
    pub const MIN: Self = Self::Minus(Tokens::MAX);
    pub const ZERO: Self = Self::Plus(Tokens::ZERO);

    /// Constructs a non-negative amount of tokens from the number of e8s.
    pub fn from_e8s(n: u64) -> Self {
        Self::Plus(Tokens::from_e8s(n))
    }

    /// Constructs a negative amount of tokens from the number of e8s.
    pub fn minus_e8s(n: u64) -> Self {
        if n == 0 {
            Self::ZERO
        } else {
            Self::Minus(Tokens::from_e8s(n))
        }
    }

    /// Returns the signed token value as number of e8s.
    pub fn to_i128(self) -> i128 {
        match self {
            Self::Plus(n) => n.get_e8s() as i128,
            Self::Minus(n) => -(n.get_e8s() as i128),
        }
    }

    /// Parses a signed token amount from an i128 value.  Returns None if the
    /// absolute value of the input does not fit into u64.
    pub fn try_from_i128(n: i128) -> Option<Self> {
        if n >= 0 {
            if n <= Self::MAX128 {
                Some(Self::from_e8s(n as u64))
            } else {
                None
            }
        } else if n >= Self::MIN128 {
            Some(Self::minus_e8s((-n) as u64))
        } else {
            None
        }
    }
...
impl Default for SignedTokens {
    fn default() -> Self {
        Self::ZERO
    }
}

impl CandidType for SignedTokens {
    ...
    {
        self.to_i128().idl_serialize(serializer)
    }
}

impl Serialize for SignedTokens {
...
impl<'de> Deserialize<'de> for SignedTokens {
        ...
        D: Deserializer<'de>,
    {
        ...
        Self::try_from_i128(n).ok_or_else(|| {
            D::Error::custom(format!("integer value {} does not fit into 64 bits", n))
        })
    }
}

#[test]
fn signed_tokens_conversion() {
    assert_eq!(SignedTokens::from_e8s(0), SignedTokens::ZERO);
    assert_eq!(SignedTokens::minus_e8s(0), SignedTokens::ZERO);

    assert_eq!(SignedTokens::from_e8s(50).to_i128(), 50);
    assert_eq!(SignedTokens::minus_e8s(50).to_i128(), -50);

    assert_eq!(SignedTokens::try_from_i128(0).unwrap(), SignedTokens::ZERO);
    assert_eq!(
        SignedTokens::try_from_i128(u64::MAX as i128).unwrap(),
        SignedTokens::MAX
    );
    assert_eq!(
        SignedTokens::try_from_i128(-(u64::MAX as i128)).unwrap(),
    ...
    assert_eq!(SignedTokens::try_from_i128(u64::MAX as i128 + 1), None);
    assert_eq!(SignedTokens::try_from_i128(-(u64::MAX as i128) - 1), None);

    assert_eq!(SignedTokens::try_from_i128(i128::MAX), None);
    assert_eq!(SignedTokens::try_from_i128(i128::MIN), None);
}
