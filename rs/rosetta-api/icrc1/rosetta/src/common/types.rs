...
pub struct TransactionBuilder {
    ...
    transaction: Option<ic_icrc1::Transaction<Tokens>>,
...
impl TransactionBuilder {
    ...
    pub fn with_transaction(mut self, transaction: ic_icrc1::Transaction<Tokens>) -> Self {
        self.transaction = Some(transaction);
        self
    }

    pub fn with_effective_fee(mut self, effective_fee: Tokens) -> Self {
    ...
    pub fn build(self) -> anyhow::Result<Transaction> {
        let transaction = self
            .transaction
            .clone()
            .context("A Transaction<Tokens> is required to build a transaction.")?;
        let currency = self
            .currency
            .clone()
        ...
        match transaction.operation {
            ic_icrc1::Operation::Mint { to, ref amount } => {
                push_operation(
                    OperationType::Mint,
                    icrc1_account_to_rosetta_accountidentifier(&to),
                    Some(format!("{}", amount.clone())),
                    None,
                );
            }
                ...
                push_operation(
                    ...
                    Some(
                        ApproveMetadata {
                            ...
                            expected_allowance: expected_allowance.clone(),
                            expires_at,
                        }
                        .into(),
                    ),
...
mod test {
    ...
    fn build_expected_transaction(
        transaction: ic_icrc1::Transaction<Tokens>,
        currency: Currency,
        effective_fee: Option<Tokens>,
    ) -> Transaction {
        let operations = match transaction.operation {
            ic_icrc1::Operation::Mint { to, ref amount } => vec![Operation::new(
                0,
                OperationType::Mint.to_string(),
                Some(icrc1_account_to_rosetta_accountidentifier(&to)),
                Some(Amount::new(amount.clone().to_string(), currency.clone())),
            )],
            ic_icrc1::Operation::Transfer {
                from,
                to,
                ...
                vec![
                    ...
                    Operation::new(
                        ...
                        Some(icrc1_account_to_rosetta_accountidentifier(&from)),
                        Some(Amount::new(format!("-{}", fee.clone()), currency.clone())),
                    ),
                ]
            }
            ic_icrc1::Operation::Burn {
    ...
    proptest! {
        ...
        fn test_transaction_builder(
            ...
            if let Some(effective_fee) = effective_fee.clone() {
            ...
            match builder.build() {
                ...
                Err(error) => {
                    let expected_error = if transaction.is_none() {
                        "A Transaction<Tokens> is required to build a transaction."
        ...
        fn test_block_transaction_response_builder(
            ...
            if let Some(block) = &block {
                ...
                if let Some(effective_fee) = block.effective_fee.clone() {
                    builder = builder.with_effective_fee(effective_fee);
                }
            }
