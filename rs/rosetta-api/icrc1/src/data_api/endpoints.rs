...
pub async fn ready(State(state): State<Arc<MultiTokenAppState>>) -> (StatusCode, Json<()>) {
    if state.token_states.values().all(|state| initial_sync_is_completed(&state.storage, state.synched.clone())) {
        (StatusCode::OK, Json(()))
    } else {
...
pub async fn network_list(
    ...
    Json(services::network_list(
        &state.token_states.values().map(|state| state.icrc1_agent.ledger_canister_id).collect::<Vec<_>>(),
    ))
}
