...
/// Uses [`GetBlocksMode::Icrc3`] for the standard ICRC-3 `icrc3_get_blocks` endpoint,
/// or [`GetBlocksMode::Legacy`] for the legacy `get_blocks` endpoint.
        ...
        for interval in fetchable_intervals(&fetched_blocks_result) {
            ...
            match get_blocks_mode {
                GetBlocksMode::Icrc3 => {
                    ...
                    for block_with_id in blocks_result.blocks.into_iter() {
                        ...
                        fetched_blocks_result.insert(
                            block_index,
                            Some(
                                RosettaBlock::from_icrc3_generic_block(
                                    block_with_id.block,
                                    block_index,
                                )
                                .map_err(|e| {
                    ...
                    for archived_blocks in blocks_result.archived_blocks {
                        ...
                        for block_with_id in arch_blocks_result.blocks.into_iter() {
                            let block_index = block_with_id.id.0.to_u64().with_context(|| {
                ...
                GetBlocksMode::Legacy => {
                    // Legacy mode: use get_blocks endpoint
                    ...
                    for (index, block) in blocks_response.blocks.into_iter().enumerate() {
                        ...
                        fetched_blocks_result.insert(
                            block_index,
                            Some(
                                RosettaBlock::from_generic_block(block, block_index).map_err(
                                    |e| {
                                        let old_context = e.to_string();
                                        e.context(format!(
                                            "Failed to parse block at index {block_index}: {old_context}"
