...
/// Creates only the essential tables needed for basic testing.
/// This is a minimal version that can be used in unit tests that don't need
/// all the production features like counters and triggers.
pub fn create_basic_tables(connection: &Connection) -> Result<()> {
    // Blocks table
    connection.execute(
        r#"
        CREATE TABLE IF NOT EXISTS blocks (
