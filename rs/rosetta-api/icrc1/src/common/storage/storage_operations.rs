...
pub fn store_blocks(
    ...
    for rosetta_block in rosetta_blocks.into_iter() {
        ...
        let transaction_created_at_time_i64 = transaction.created_at_time.map(convert_timestamp_to_db);
...
mod tests {
    ...
    fn test_store_and_read_blocks() -> anyhow::Result<()> {
        ...
        // Test storing blocks
        store_blocks(&mut connection, vec![
            block0.clone(),
            block1.clone(),
            block2.clone(),
            block3.clone(),
    ...
    fn test_hash_consistency() -> anyhow::Result<()> {
        ...
        let blocks = vec![
            ...
            create_test_approve_block(3, normal_timestamp, beyond_max_timestamp, max_timestamp, &principal1),
        ];
        
        // Record original hashes for comparison
        let original_block_hashes: Vec<_> = blocks.iter()
            .map(|block| block.clone().get_block_hash())
            .collect();
            
        let original_tx_hashes: Vec<_> = blocks.iter()
