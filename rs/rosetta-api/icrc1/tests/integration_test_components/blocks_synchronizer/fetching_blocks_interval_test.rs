...
use ic_icrc_rosetta::ledger_blocks_synchronization::blocks_synchronizer::{
    self, GetBlocksMode, RecurrencyMode, blocks_verifier,
};
...
proptest! {
    ...
    fn test_simple_start_of_synchronizing_blocks(transfer_args_batch1 in transfer_args_with_sender(*MAX_NUM_GENERATED_BLOCKS, *TEST_ACCOUNT),transfer_args_batch2 in transfer_args_with_sender(*MAX_NUM_GENERATED_BLOCKS, *TEST_ACCOUNT)) {
        ...
        rt.block_on(async {
            ...
            blocks_synchronizer::start_synching_blocks(agent.clone(), storage_client.clone(),2,Arc::new(AsyncMutex::new(vec![])), RecurrencyMode::OneShot, Box::new(|| {}), GetBlocksMode::Legacy).await.unwrap();

            // Check that the full sync of all blocks generated by the first batch of blocks is valid
            check_storage_validity(storage_client.clone(),transfer_args_batch1.len() as u64).await;
            ...
            blocks_synchronizer::sync_from_the_tip(agent.clone(), storage_client.clone(),2,Arc::new(AsyncMutex::new(vec![])), GetBlocksMode::Legacy).await.unwrap();

            // Check that the sync of all blocks generated by the second batch of blocks is valid
            check_storage_validity(storage_client.clone(),(transfer_args_batch1.len()+transfer_args_batch2.len()) as u64).await;
    ...
    fn test_fetching_from_archive(transfer_args in transfer_args_with_sender(*MAX_NUM_GENERATED_BLOCKS, *TEST_ACCOUNT)) {
        ...
        rt.block_on(async {
            ...
            check_storage_validity(storage_client.clone(),transfer_args.len() as u64).await;
    ...
    fn test_icrc3_certificate(transfer_args in transfer_args_with_sender(*MAX_NUM_GENERATED_BLOCKS, *TEST_ACCOUNT).no_shrink()) {
        ...
        async fn check_blocks_synchronization_and_certificate(agent: Arc<Icrc1Agent>, transfer_args: Vec<TransferArg>) {
            ...
            check_storage_validity(storage_client.clone(),transfer_args.len().saturating_sub(1) as u64).await;
            ...
            let tip_block = storage_client.get_block_with_highest_block_idx().await.unwrap().unwrap();
...
fn test_gaps_handling() {
    ...
    // Use a temporary directory for the on-disk database
    let temp_dir = tempfile::tempdir().unwrap();
    let db_path = temp_dir.path().join("test_gaps_handling.db");
    ...
    rt.block_on(async {
        ...
        // Check that the full sync of all blocks generated by the first batch of blocks is valid
        check_storage_validity(storage_client.clone(), NUM_BLOCKS - 1).await;
        ...
        // Create a separate connection to the same database file for direct SQL manipulation
        let connection = Connection::open(&db_path_clone).unwrap();
...
fn test_fetching_from_archive_icrc3_mode() {
    ...
    rt.block_on(async {
        ...
        // Start the synching process using ICRC-3 mode
        blocks_synchronizer::start_synching_blocks(
            agent.clone(),
            storage_client.clone(),
            10,
        ...
        check_storage_validity(storage_client.clone(), NUM_BLOCKS).await;
    });
}

/// Tests that both legacy (get_blocks) and ICRC-3 (icrc3_get_blocks) modes produce
...
fn test_icrc3_vs_legacy_mode_produce_same_results() {
    ...
    rt.block_on(async {
        ...
        let storage_client_icrc3 = Arc::new(StorageClient::new_in_memory().await.unwrap());
        blocks_synchronizer::start_synching_blocks(
            agent.clone(),
            storage_client_icrc3.clone(),
            10,
            Arc::new(AsyncMutex::new(vec![])),
            RecurrencyMode::OneShot,
            Box::new(|| {}),
        ...
        // Verify both storage clients have the same blocks
        let blocks_legacy = storage_client_legacy
            .get_blocks_by_index_range(0, NUM_BLOCKS)
            .await
            .unwrap();
        let blocks_icrc3 = storage_client_icrc3
            .get_blocks_by_index_range(0, NUM_BLOCKS)
            .await
            .unwrap();
        ...
        // Verify each block has the same hash and index
        for (legacy_block, icrc3_block) in blocks_legacy.iter().zip(blocks_icrc3.iter()) {
            assert_eq!(
                legacy_block.index, icrc3_block.index,
                "Block indices should match"
            );
            assert_eq!(
                legacy_block.clone().get_block_hash(),
                icrc3_block.clone().get_block_hash(),
                "Block hashes should match at index {}",
                legacy_block.index
            );
        }
    });
...
/// Tests that the callback method names differ between legacy and ICRC-3 responses.
/// Legacy mode uses `get_blocks` while ICRC-3 mode uses `icrc3_get_blocks`.
#[test]
fn test_icrc3_vs_legacy_callback_method_names() {
    ...
    rt.block_on(async {
        ...
        let legacy_response: GetBlocksResponse = agent
            ...
            .await
            .expect("Failed to get blocks using legacy endpoint");
        ...
        let icrc3_response: GetBlocksResult = agent
            ...
            .await
            .expect("Failed to get blocks using ICRC-3 endpoint");

        // Verify that we have archived blocks (archiving should have been triggered)
        assert!(
            !legacy_response.archived_blocks.is_empty(),
            "Legacy response should have archived blocks"
        );
        assert!(
            !icrc3_response.archived_blocks.is_empty(),
            "ICRC-3 response should have archived blocks"
        );

        for archived in &legacy_response.archived_blocks {
            assert_eq!(
                archived.callback.method, "get_blocks",
                "Legacy archive callback should use 'get_blocks' method"
            );
        }

        for archived in &icrc3_response.archived_blocks {
            assert_eq!(
                archived.callback.method, "icrc3_get_blocks",
                "ICRC-3 archive callback should use 'icrc3_get_blocks' method"
            );
        }
