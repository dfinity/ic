...
proptest! {
    ...
    fn test_updating_account_balances(args_with_caller in valid_transactions_strategy(
        ...
        *MAX_NUM_GENERATED_BLOCKS,
        ...
        run_updating_account_balances_test(args_with_caller, GetBlocksMode::Legacy);
    }

    #[test]
    fn test_updating_account_balances_icrc3_mode(args_with_caller in valid_transactions_strategy(
        MINTER_IDENTITY.clone(),
        DEFAULT_TRANSFER_FEE,
        *MAX_NUM_GENERATED_BLOCKS,
        SystemTime::now(),
    ).no_shrink()) {
        run_updating_account_balances_test(args_with_caller, GetBlocksMode::Icrc3);
    }
}

fn run_updating_account_balances_test(
    args_with_caller: Vec<ArgWithCaller>,
    ...
    rt.block_on(async {
        ...
        for idx in block_indices_iter.into_iter() {
            for account in accounts.clone().into_iter() {
                ...
                assert_eq!(
                    *current_balances.get(&account).unwrap(),
                    storage_client
                        .get_account_balance_at_block_idx(&account, idx)
                        .await
                        .unwrap()
        ...
        for account in accounts.clone().into_iter() {
            ...
            let balance_rosetta = storage_client
