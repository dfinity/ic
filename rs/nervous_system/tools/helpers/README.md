...
. ./rs/nervous_system/tools/helpers/cmd.sh set_testnet_env_variables
```
...
./rs/nervous_system/tools/helpers/upgrade-canister-to-working-tree.sh governance
```
...
```bash
# NNS:
./rs/nervous_system/tools/helpers/test-canister-upgrade.sh <CANISTER_NAME> <TARGET_VERSION>

# SNS:
# Test upgrading the specified SNS canisters from mainnet version to the
# specified version in every possible order
./rs/nervous_system/tools/helpers/test-sns-canister-upgrades.sh <TARGET_VERSION> <CANISTER_NAME> (<CANISTER_NAME>...)
# Test deploying a new SNS with the specified canister versions
./rs/nervous_system/tools/helpers/test-sns-canister-deployment.sh <TARGET_VERSION> <CANISTER_NAME> (<CANISTER_NAME>...)
```
...
```bash
./rs/nervous_system/tools/helpers/test-canister-upgrade.sh registry 1a2d86e9d66d93c4a9a9a147774577c377ce0c66
```
...
Your environment variables could be set from a previous run of recovered_mainnet_nns. In this case, exit and re-enter your shell to reset them. Rerunning `. ./rs/nervous_system/tools/helpers/cmd.sh set_testnet_env_variables` will not help as that script tries not to overwrite variables you already have.
...
do
    ./rs/nervous_system/tools/helpers/prepare-nns-upgrade-proposal-text.sh \
        $CANISTER \
        $RC \
...
do
    ./rs/nervous_system/tools/helpers/prepare-publish-sns-wasm-proposal-text.sh \
        $CANISTER \
        $RC \
...
for CANISTER in "${NNS_CANISTERS[@]}"
do
    ./rs/nervous_system/tools/helpers/submit-mainnet-nns-upgrade-proposal.sh \
        $PROPOSALS_DIR/nns-$CANISTER.md \
...
for CANISTER in "${SNS_CANISTERS[@]}"
do
    ./rs/nervous_system/tools/helpers/submit-mainnet-publish-sns-wasm-proposal.sh \
