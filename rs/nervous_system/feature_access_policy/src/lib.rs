...
//! ```
//!
//! ### Strict Policies (Panics on Empty)
//!
//! Use `allow_only()` and `deny_only()` when you're certain the collection contains items.
//! These methods panic if given an empty collection to catch configuration errors early:
//!
//! ```rust
//! use ic_nervous_system_feature_access_policy::FeatureAccessPolicy;
//!
//! // Allowlist - only these features are permitted
//! let policy = FeatureAccessPolicy::allow_only(vec!["read", "write"]);
//! assert!(policy.is_allowed(&"read"));
//! assert!(!policy.is_allowed(&"delete"));
//!
//! // Denylist - only these features are blocked
//! let policy = FeatureAccessPolicy::deny_only(vec!["delete", "admin"]);
//! assert!(policy.is_allowed(&"read"));
//! assert!(!policy.is_allowed(&"delete"));
//! ```
//!
...
//! let policy = FeatureAccessPolicy::allow_only(empty);
//! ```
//!
...
//! ## Design Decisions
//!
//! ### Why separate `_only` and non-`_only` variants?
//!
//! The distinction helps catch configuration errors:
//! - `allow_only()` / `deny_only()`: Use when items are required (e.g., hardcoded lists)
//! - `allow()` / `deny()`: Use when items might be empty (e.g., user input, config files)
//!
