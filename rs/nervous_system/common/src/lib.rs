...
/// Unwraps an Option, panicking if the Option is None. The panic message
/// includes the expression that was unwrapped, and the file and line number.
/// This is sometimes more useful for debugging than the vanilla `.unwrap()`
/// function.
///
/// Usage example:
/// ```
/// let some_value: Option<i32> = Some(42);
/// let _ = unwrap_verbose!(some_value); // 42
/// let _ = unwrap_verbose!(none_value); // src/main.rs:4 - unwrap_verbose called on a None value ("none_value")
/// ```
///
/// A format string can be passed as the second argument, which will be used
/// to format the panic message:
///
/// ```
/// let _ = unwrap_verbose!(none_value, "Failure message on line {}", line!());
/// // src/main.rs:5 - unwrap_verbose called on a None value ("none_value"): "Failure message on line 5"
/// ```
#[macro_export]
macro_rules! unwrap_verbose {
    ($expr:expr) => {
        match $expr {
            Some(val) => val,
            None => panic!("{}:{} - unwrap_verbose called on a None value ({:?})", file!(), line!(), stringify!($expr)),
        }
    };
    ($expr:expr, $($arg:tt)+) => {
        match $expr {
            Some(val) => val,
            None => {
                panic!("{}:{} - unwrap_verbose called on a None value ({:?}): {:?}", file!(), line!(),  stringify!($expr), format!($($arg)+))
            },
        }
    };
}
