...
pub enum ListAllSnsNeuronRecipesError<CallCanistersError: std::error::Error + 'static> {
    #[error(transparent)]
    CallError(#[from] CallCanistersError),
...
impl SwapCanister {
    ...
    pub async fn get_derived_state<C: CallCanisters>(
        &self,
        agent: &C,
    ) -> Result<GetDerivedStateResponse, C::CallError> {
        agent
            .call(self.canister_id, GetDerivedStateRequest {})
            .await
    }

    pub async fn get_init<C: CallCanisters>(
        &self,
        agent: &C,
    ) -> Result<GetInitResponse, C::CallError> {
    ...
    pub async fn list_sns_neuron_recipes<C: CallCanisters>(
        ...
        limit: u32,
        offset: u64,
    ) -> Result<ListSnsNeuronRecipesResponse, C::CallError> {
        agent
            .call(
                self.canister_id,
    ...
    pub async fn list_all_sns_neuron_recipes<C: CallCanisters>(
        &self,
        agent: &C,
    ) -> Result<Vec<SnsNeuronRecipe>, ListAllSnsNeuronRecipesError<C::CallError>> {
        ...
        for i in 0..num_calls {
            let new_sns_neuron_recipes = self
                ...
                .await
                .map_err(ListAllSnsNeuronRecipesError::CallError)?;
