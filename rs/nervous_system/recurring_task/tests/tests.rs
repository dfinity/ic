...
fn state_machine_for_test() -> StateMachine {
    ...
    subnet_config
        .scheduler_config
        .max_instructions_per_message_without_dts = subnet_config
        .scheduler_config
        .max_instructions_per_message_without_dts
        / instruction_divisor;
    hypervisor_config.max_query_call_graph_instructions =
...
fn set_up_canister_with_tasks(state_machine: &StateMachine, task_names: Vec<String>) -> CanisterId {
    let recurring_task_canister_wasm =
        Project::cargo_bin_maybe_from_env("recurring-task-canister", &[]);
    let canister_id = state_machine
        .install_canister(
            recurring_task_canister_wasm.bytes(),
