...
use ic_agent::Agent;
use ic_neurons_fund::{
    rescale_to_icp, rescale_to_icp_e8s, MatchedParticipationFunction, NonDecreasingFunction,
...
async fn main() -> Result<(), String> {
    ...
    let sns_governance_canister_id = swap_init
        .init
        .as_ref()
        .unwrap()
    ...
    if is_audit {
        let nns_governance_canister_id = swap_init
            .init
            .as_ref()
            .unwrap()
        ...
        let audit_info = {
            let response = agent
                ...
                .await
                .map_err(|e| e.to_string())?;
            Decode!(response.as_slice(), GetNeuronsFundAuditInfoResponse)
                .map_err(|e| e.to_string())?
        };
        ...
        println!(
            "Neurons' Fund audit info for {}: {:#?}",
            sns_name, audit_info
        );
        return Ok(());
    }

    let neurons_fund_participation_constraints = swap_init
        .init
        .unwrap()
    ...
    let grey = RGB8::new(100, 100, 100);
    let mint = RGB8::new(194, 252, 224);
    let pink = RGB8::new(255, 207, 135);
