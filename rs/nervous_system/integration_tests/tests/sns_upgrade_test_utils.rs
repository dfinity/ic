...
use ic_nervous_system_integration_tests::{
    ...
    pocket_ic_helpers,
    pocket_ic_helpers::{await_with_timeout, hash_sns_wasms, nns, sns},
};
...
/// Tests upgrading SNS canisters to the master version, using AdvanceSnsTargetVersion proposals
pub async fn test_sns_upgrade(sns_canisters_to_upgrade: Vec<SnsCanisterType>) {
    ...
    // Pass 2: attempt all requested upgrades again,
    // to ensure the SNS is still in an upgradeable state.
    eprintln!("Adding all WASMs again ...");
    let latest_sns_version = {
        let mut latest_version = latest_sns_version;

        for canister_type in &sns_canisters_to_upgrade {
            eprintln!("modify_and_add_wasm for {:?} ...", canister_type);
            latest_version =
                nns::sns_wasm::modify_and_add_wasm(&pocket_ic, latest_version, *canister_type, 2)
                    .await;
        }
