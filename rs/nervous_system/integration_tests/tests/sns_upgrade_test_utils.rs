...
impl Drop for Section {
    fn drop(&mut self) {
        eprintln!("{} took {:?}", self.name, self.start_time.elapsed());
    }
}

pub async fn test_sns_upgrade(sns_canisters_to_upgrade: Vec<SnsCanisterType>) {
    let _timer = Section::new("Testing the upgrade process");
    ...
    let create_service_nervous_system = {
        let _timer = Section::new("Creating SNS");
    ...
    let sns = {
        let _timer = Section::new("Deploying an SNS instance via proposal");
    ...
    if sns_canisters_to_upgrade.contains(&SnsCanisterType::Archive) {
        let _timer = Section::new("Testing if the Archive canister is spawned");
    ...
    for upgrade_pass in 0..2 {
        let _timer = Section::new(format!("Upgrade pass {}", upgrade_pass));

        let mut expected_upgrade_steps = vec![];
        {
            let _timer = Section::new("Adding all WASMs");
        ...
        {
            let _timer = Section::new("wait for the upgrade steps to be refreshed");
            await_with_timeout(
                &pocket_ic,
        ...
        {
            let _timer = Section::new("assert that the upgrade steps are correct");
            assert_eq!(
        ...
        {
            let _timer = Section::new("advance the target version to the latest version.");
        ...
        {
            let _timer = Section::new("wait for the upgrade to happen");
            await_with_timeout(
                &pocket_ic,
