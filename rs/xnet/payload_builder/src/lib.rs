    ...
    fn validate_slice(
        ...
        if let Some(messages) = slice.messages() {
            ...
            let signal_limit = get_signal_limit(
                slice.header().begin(),
                slice.messages().map(|messages| messages.begin()),
            );
...
///
/// - `slice_begin` is the index of the first message in the stream slice.
...
pub fn get_signal_limit(stream_begin: StreamIndex, slice_begin: Option<StreamIndex>) -> usize {
    let slice_begin = slice_begin.unwrap_or(stream_begin);
    if slice_begin < stream_begin {
        // Dishonest subnet, do not include any messages.
        return 0;
    }

    let max_slice_end_index = stream_begin.get() as usize + MAX_STREAM_MESSAGES;
    // `saturating_sub()` enables decreasing `MAX_STREAM_MESSAGES`.
    max_slice_end_index.saturating_sub(slice_begin.get() as usize)
}
