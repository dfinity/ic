...
pub async fn receive_body_without_timeout(
    ...
    let max_request_body_size = max_request_body_size.get_bytes() as usize;
    http_body_util::Limited::new(body, max_request_body_size)
        .collect()
        .await
        .map(|col| col.to_bytes())
        .map_err(|err| BodyReceiveError::TooLarge(err.to_string()))
}
