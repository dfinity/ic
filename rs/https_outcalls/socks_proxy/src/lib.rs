...
pub trait SocksProxyProvider: Send + Sync {
...
/// reported on every call, memoized or not, so a persistent one keeps being
/// visible.
pub struct RegistrySocksProxyProvider {
...
impl RegistrySocksProxyProvider {
    pub fn new(
        registry_client: Arc<dyn RegistryClient>,
        subnet_type: SubnetType,
        log: ReplicaLogger,
...
impl SocksProxyProvider for RegistrySocksProxyProvider {
    fn socks_proxy_addrs(&self) -> Vec<String> {
...
mod tests {
    ...
    fn memoizes_within_a_registry_version() {
        ...
        let provider = RegistrySocksProxyProvider::new(
            Arc::clone(&counting) as Arc<_>,
            SubnetType::Application,
            no_op_logger(),
        );

        let first = provider.socks_proxy_addrs();
        ...
        let second = provider.socks_proxy_addrs();
    ...
    fn reports_failures_on_every_call_even_when_memoized() {
        ...
        assert!(provider.socks_proxy_addrs().is_empty());
        let lookups_after_first = counting.lookups();
        assert!(provider.socks_proxy_addrs().is_empty());
    ...
    fn recomputes_when_the_registry_version_advances() {
        ...
        let before = provider.socks_proxy_addrs();
        ...
        registry.update_to_latest_version();
        let after = provider.socks_proxy_addrs();
    ...
    /// Counts how many registry reads the provider performs, so that the memo
