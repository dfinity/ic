...
use ic_types::{
    ...
    batch::{
        CanisterHttpInitialRefund, CanisterHttpPayload, CanisterHttpRefunds, ConsensusResponse,
...
impl CanisterHttpPayloadBuilderImpl {
    ...
    pub fn validate_canister_http_payload_impl(
        ...
        for response in &payload.responses {
            ...
            let computed_refund =
                utils::fully_replicated_initial_refund(&response.proof, subnet_size);
        ...
        for group in &payload.flexible_responses {
            ...
            // are derived from the registry version pinned in its
            // context.
            let CanisterHttpCommittee { committee, .. } = match self
                .membership
                .get_canister_http_committee(context.registry_version)
            {
                Ok(committee) => committee,
    ...
    )> for CanisterHttpPayloadBuilderImpl
{
    fn into_messages(
        ...
        for error in messages.flexible_errors {
            // Only errors that carry full responses (too-many-rejects) produce
