        ...
        canister_http_pool
            ...
            .filter_map(|artifact| {
                ...
                if let Err(err) = self.crypto.verify(payment_share, registry_version) {
                    error!(
                        self.log,
                        "Unable to verify signature of payment share, {}", err
                    );

                    self.metrics.shares_marked_invalid.inc();
                    return Some(CanisterHttpChangeAction::HandleInvalid(
                        share.clone(),
                        format!("Unable to verify signature of payment share, {err}"),
                    ));
                }
...
pub mod test {
            ...
            with_test_replica_logger(|log| {
                ...
                assert_matches!(
                    ...
                    CanisterHttpChangeAction::AddToValidatedAndGossipResponse(
                        ...
                        // The payment share must carry the metadata reported by the adapter
                        // and be signed by the local replica.
        ...
        ic_test_utilities::artifact_pool_config::with_test_pool_config(|pool_config| {
            with_test_replica_logger(|log| {
                ...
                shim_mock
                    .expect_try_receive()
                    .times(1)
                    .return_const(Ok((empty_response.clone(), payment_metadata)))
                    .in_sequence(&mut sequence);
                shim_mock
                    .expect_try_receive()
                    .times(1)
    ...
    /// A response share and payment share within the same artifact must agree
    /// on `(signer, callback id)`. If they don't, the entire artifact is
    ...
    fn test_share_and_payment_share_different_keys_are_invalid() {
        ic_test_utilities::artifact_pool_config::with_test_pool_config(|pool_config| {
            with_test_replica_logger(|log| {
                ...
                // The payment share uses the same signer as the response share but a
                // different callback id, so the (signer, id) keys disagree.
    ...
    /// allowance configured on the request context, the share is rejected as
    /// invalid: an honest replica cannot legitimately refund more than its
    /// allowance.
    #[test]
    fn test_refund_greater_than_replica_allowance_is_invalid() {
        ic_test_utilities::artifact_pool_config::with_test_pool_config(|pool_config| {
            with_test_replica_logger(|log| {
                ...
                // Refund (200) exceeds the per-replica allowance (100).
                let payment_share = CanisterHttpPaymentShare::fake(
                    CanisterHttpPaymentMetadata {
                        id: share.content.id,
                        receipt: CanisterHttpPaymentReceipt {
