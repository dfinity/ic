...
fn validate_content_size(
    ...
    let content_size = receipt.content_size() as u64;
    let max_content_size = context.max_http_outcall_content_size(receipt.is_reject());
        ...
        loop {
            match self.http_adapter_shim.lock().unwrap().try_receive() {
                ...
                Ok((response, payment_receipt)) => {
                    ...
                    if let Err(err) = validate_content_size(&receipt_share, context) {
                        warn!(
                            self.log,
    ...
    fn validate_shares(&self, canister_http_pool: &dyn CanisterHttpPool) -> CanisterHttpChangeSet {
        ...
        canister_http_pool
            ...
            .filter_map(|artifact| {
                ...
                if let Err(err) = validate_content_size(&share.content, context) {
                    return Some(CanisterHttpChangeAction::HandleInvalid(
                        share.clone(),
                        format!(
