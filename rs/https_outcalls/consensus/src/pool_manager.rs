...
const MINIMUM_ALLOWED_ERROR_MESSAGE_BYTES: u64 = 1024; // 1KB
const CANDID_OVERHEAD_RESERVE_BYTES: u64 = 1024; // 1KB

// Checks that the response size is within the allowed limits.
// In general, the response size must be lower than max_response_bytes, however there are two caveats:
    ...
    fn test_reject_message_valid_when_max_response_is_zero() {
        ic_test_utilities::artifact_pool_config::with_test_pool_config(|pool_config| {
            with_test_replica_logger(|log| {
                ...
                //    (i.e., less than MINIMUM_ALLOWED_ERROR_MESSAGE_BYTES), so it should pass
