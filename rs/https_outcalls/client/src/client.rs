...
impl NonBlockingChannel<CanisterHttpRequest> for CanisterHttpAdapterClientImpl {
    ...
    fn send(
        ...
        self.rt_handle.spawn(async move {
            ...
            // Process the outcall in an `async {}` (no `move`) block so that `budget`
            // remains accessible after the await, allowing us to call
            // `create_payment_receipt` regardless of whether processing succeeded.
            let payload = async {
                let max_response_size_bytes = budget.get_adapter_limits().max_response_size.get();

                let (adapter_response, elapsed) = execute_http_request(
                ...
                let transformed_payload = match &request_context.transform {
                    Some(transform) => {
                        ...
                        info!(
                            ...
                            response_time {}, reply_callback_id {}, \
            ...
            // The receipt must be computed AFTER processing the response so that any
            // resources consumed by the response (network usage, transform
            // instructions) are already accounted for in the budget tracker.
...
/// Send the HTTPS outcall via the adapter and account for the network resources
/// consumed against `budget`. Returns the raw adapter response together with the
...
) -> Result<(HttpsOutcallResponse, Duration), (RejectCode, String)> {
    let AdapterLimits {
        max_response_size,
        max_response_time,
    ...
    let (grpc_result, elapsed) =
        timeout_with_capped_metric(max_response_time, adapter_client.https_outcall(proto_req))
            ...
            .map_err(|_: tokio::time::error::Elapsed| {
                (RejectCode::SysTransient, "Deadline Exceeded".to_string())
            })?;
...
/// Convert the raw adapter response into a [`CanisterHttpResponsePayload`] and
/// validate its headers and body.
