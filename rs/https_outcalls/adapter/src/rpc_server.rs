        ...
        for ip in api_bn_ips {
            match ip.parse() {
                Ok(proxy_addr) => {
                    ...
                    new_cache.insert(ip, socks_client);
                }
                Err(e) => {
                    // TODO(mihailjianu): this might invalidate the cache very often
                    // We should consider storing two vectors (one for IPs, one for clients).
