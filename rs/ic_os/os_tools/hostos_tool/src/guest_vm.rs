...
use config::guest_vm_config::{assemble_config_media, generate_vm_config, DirectBootArgs};
...
use virt::sys::{
    virDomainState, VIR_DOMAIN_DESTROY_GRACEFUL, VIR_DOMAIN_RUNNING, VIR_DOMAIN_SHUTDOWN,
};
...
const STARTUP_WAIT_DURATION: Duration = Duration::from_secs(10);
...
impl VirtualMachine {
    ...
    pub fn new(
        ...
        let domain = Domain::create_xml(&libvirt_connect, xml_config, 0)
    ...
    pub fn is_running(&self) -> bool {
        match self.domain.get_state() {
            ...
            Err(err) => {
                warn!("Failed to get domain state: {}", err);
                false
            }
        }
    }
...
impl GuestVmService {
    ...
    async fn start_virtual_machine(&self) -> Result<VirtualMachine> {
        // Return error if VM already exists
        if Domain::lookup_by_name(&self.libvirt_connection, GUESTOS_DOMAIN_NAME).is_ok() {
            bail!("GuestOS virtual machine is already defined");
        }
        ...
        let vm_config = generate_vm_config(&self.hostos_config, config_media.path(), &None)
            .context("Failed to generate GuestOS VM config")?;

        info!("Defining GuestOS virtual machine");
        ...
        #[cfg(target_os = "linux")]
        systemd::daemon::notify(false, &[systemd::daemon::STATE_READY, "1"])?;

        // Wait before printing messages to console
        sleep(STARTUP_WAIT_DURATION).await;
    ...
    fn get_host_ipv6_address(&self) -> String {
        ...
        generated_mac
            .calculate_slaac(&ipv6_config.prefix)
            .to_string()
    }
    ...
    async fn monitor_virtual_machine(&self, vm: &VirtualMachine) -> Result<()> {
        ...
        tokio::select! {
            ...
            _ = sigterm.recv() => {
                // Stop signal received, gracefully shutdown
                info!("Received stop signal, shutting down VM");
                self.metrics_writer.write_metrics(&[Metric::with_annotation("hostos_guestos_service_graceful_shutdown",
                    1.0,
