...
pub fn generate_networkd_config(
    ...
    debug!("IPv6 config info: {ipv6_config:?}");
    debug!("IPv4 address info: {ipv4_info:?}");
    debug!(
    ...
    debug!("Networkd config contents: {networkd_config_file_contents:#?}");
...
fn is_k8s_testnet() -> Result<bool> {
    ...
    if output.contains("config") {
        info!("K8S testnet detected. Turning off DAD for tnet on k8s.");
        Ok(true)
    } else {
        Ok(false)
    }
