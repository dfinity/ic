...
use guest_disk::crypt::{
    KeyslotMetadata, LuksHeaderLocation, activate_crypt_device, backup_luks_header_to_file,
    check_passphrase, deactivate_crypt_device, format_crypt_device, open_luks2_device,
    read_keyslot_metadata, remove_assigned_tokens, write_keyslot_metadata,
};
...
/// Number of keyslots/tokens supported by LUKS2 (mirrors the private consts in `crypt.rs`).
...
impl<'a> PartitionView<'a> {
    ...
    fn open_crypt_device(&self) -> CryptDevice {
        open_luks2_device(&self.device_path, self.header_location(), false).unwrap()
    }

    fn has_attached_luks2_header(&self) -> bool {
        open_luks2_device(&self.device_path, LuksHeaderLocation::Attached, false).is_ok()
    }
    ...
    /// Reads all `ic-key-metadata` tokens from the device. Tolerates malformed tokens
    /// (matches the production reader's lenient behavior).
...
impl TestFixture {
    ...
    /// The current launch TCB version as a `u64` (the form used by SEV key derivation).
    fn launch_tcb_as_u64(&self) -> u64 {
        u64::from_le_bytes(
            self.launch_tcb
                .to_bytes_with(Generation::Milan)
                .expect("Failed to serialize launch TCB"),
        )
    }

    /// Derives the current SEV disk-encryption key for the given partition's data device,
    /// using the active slot's measurement and the current launch TCB.
    ...
    /// Sets the SEV launch TCB version (simulates a firmware version). Set before formatting or
    /// before a firmware-upgrade open.
    ...
    /// Simulates a firmware upgrade: changes the SEV launch TCB. The next partition open will
    /// rotate the keyslot metadata's TCB in place (the disk key itself is unchanged because the
    /// measurement is unchanged).
    fn firmware_upgrade_to(&mut self, new_launch_tcb: TcbVersion) {
        self.launch_tcb = new_launch_tcb;
    }
...
/// Tests that a firmware upgrade (launch TCB change) rotates the keyslot metadata's TCB in
/// place, while the disk key stays the same (same measurement) and data is preserved.
#[test]
fn test_sev_firmware_upgrade_rotates_keyslot_metadata() {
    for partition in [Partition::Store, Partition::Var] {
        ...
        let tcb1_u64 = u64::from_le_bytes(tcb1.to_bytes_with(Generation::Milan).unwrap());
        fixture.set_launch_tcb(tcb1);
        fixture.partition(partition).format().unwrap();

        let keyslot_metadata = fixture.partition(partition).read_keyslot_metadata();
        // There is one keyslot with the initial TCB version.
        assert_eq!(keyslot_metadata.len(), 1);
        assert_eq!(keyslot_metadata[0].sev_metadata.tcb_version, tcb1_u64);

        fixture.partition(partition).open().unwrap();
        fixture
            .partition(partition)
            .write_payload(b"data before upgrade");
        fixture.partition(partition).deactivate();

        // Firmware upgrade: TCB changes, measurement stays the same → key unchanged.
        let tcb2 = TcbVersion::new(None, 2, 2, 2, 2);
        let tcb2_u64 = u64::from_le_bytes(tcb2.to_bytes_with(Generation::Milan).unwrap());
        fixture.firmware_upgrade_to(tcb2);
        ...
        fixture
            .partition(partition)
            .assert_payload(b"data before upgrade");
        fixture.partition(partition).deactivate();

        let keyslot_metadata_after_upgrade = fixture.partition(partition).read_keyslot_metadata();
        // There is one keyslot with the upgraded TCB version.
        assert_eq!(keyslot_metadata_after_upgrade.len(), 1);
        assert_eq!(
            keyslot_metadata_after_upgrade[0].sev_metadata.tcb_version,
            tcb2_u64
        );
...
/// Tests a firmware upgrade followed by a GuestOS upgrade. After both, the device should have
/// both the previous and current keyslots (2 keys, supporting rollback).
#[test]
fn test_firmware_upgrade_then_guestos_upgrade() {
    ...
    let initial_tcb_u64 = u64::from_le_bytes(initial_tcb.to_bytes_with(Generation::Milan).unwrap());
    ...
    fixture
        .store_partition()
        .write_payload(b"firmware-then-guestos-upgrade");
    fixture.store_partition().deactivate();

    // 2. Firmware upgrade (TCB 1→2). Same GuestOS, same measurement.
    fixture.firmware_upgrade_to(upgraded_tcb);
    fixture
        .store_partition()
        .open()
        .expect("open after firmware upgrade should succeed");
    ...
    let after_fw_metadata = fixture.store_partition().read_keyslot_metadata();
    assert_eq!(after_fw_metadata.len(), 1);
    assert_eq!(
        after_fw_metadata[0].sev_metadata.tcb_version, upgraded_tcb_u64,
        "firmware upgrade should rotate TCB"
    );

    // 3. GuestOS upgrade (A→B). The upgrade protocol writes the previous key.
    let guestos_a_key = fixture.derive_sev_key(Partition::Store);
    fs::write(fixture.previous_key_path(), &guestos_a_key)
        .expect("Failed to write previous key for GuestOS upgrade");

    fixture.set_launch_measurement(guestos_b_measurement);
    // TCB stays at upgraded_tcb.
    ...
    // After the GuestOS upgrade, both GuestOS-A and GuestOS-B keyslots are kept.
    let final_metadata = fixture.store_partition().read_keyslot_metadata();
    assert_eq!(
        final_metadata.len(),
        2,
        "both the previous and current keyslots should be kept after a GuestOS upgrade"
    );

    // GuestOS-B's keyslot carries the upgraded TCB and B's measurement.
    let guestos_b_entry = final_metadata
        .iter()
        .find(|m| m.sev_metadata.launch_measurement_hex == hex::encode(guestos_b_measurement))
        .expect("GuestOS-B keyslot should be present");
    assert_eq!(
        guestos_b_entry.sev_metadata.tcb_version, upgraded_tcb_u64,
        "GuestOS-B keyslot should have the upgraded TCB"
    );

    // Both keys unlock: A (kept for rollback) and B (current).
    let store_header_path = fixture.store_header_path();
    check_passphrase(
        fixture.store_device_path(),
    ...
    .expect("GuestOS-A key should still unlock (kept for rollback)");
...
fn test_guestos_upgrade_then_firmware_upgrade_then_rollback() {
    ...
    fixture
        .store_partition()
        .write_payload(b"guestos-fw-rollback");
    fixture.store_partition().deactivate();

    // Snapshot GuestOS-A's detached header (frozen on its var partition).
    let header_path = fixture.store_header_path();
    fs::copy(&header_path, &guestos_a_header)
    ...
    let header_path = fixture.store_header_path();
    fs::copy(&guestos_a_header, &header_path).expect("Failed to restore GuestOS-A detached header");

    let pre_rollback_metadata = fixture.store_partition().read_keyslot_metadata();
    assert_eq!(pre_rollback_metadata.len(), 1);
    assert_eq!(
        pre_rollback_metadata[0].sev_metadata.tcb_version, initial_tcb_u64,
        "frozen GuestOS-A header should still have the old TCB"
    );

    // GuestOS-A boots: same measurement as before, firmware at TCB=2.
    fixture.set_launch_measurement(guestos_a_measurement);
    // launch_tcb is still upgraded_tcb from step 3.
    ...
    let post_rollback_metadata = fixture.store_partition().read_keyslot_metadata();
    assert_eq!(post_rollback_metadata.len(), 1);
    assert_eq!(
        post_rollback_metadata[0].sev_metadata.tcb_version, upgraded_tcb_u64,
...
/// Tests that a GuestOS upgrade migrates a legacy store (no SEV metadata) to SEV,
/// preserving data and writing fresh SEV metadata.
#[test]
fn test_guestos_upgrade_migrates_store_without_existing_sev_metadata() {
    ...
    let pre_migration_metadata = fixture.store_partition().read_keyslot_metadata();
    assert!(
        pre_migration_metadata.is_empty(),
        "pre-migration Store should have no SEV metadata tokens"
    );

    // Write some data before migration.
    activate_crypt_device(
        fixture.store_device_path(),
        LuksHeaderLocation::Attached,
        "store-crypt",
    ...
    fixture
        .store_partition()
        .write_payload(b"migration payload");
    fixture.store_partition().deactivate();
    fixture.backup_store_header();

    // Open via the upgrade path (previous key → SEV key migration).
    fixture
        .store_partition()
        .open()
        .expect("GuestOS upgrade migration should succeed");

    fixture
        .store_partition()
        .assert_payload(b"migration payload");
    assert!(!fixture.previous_key_path().exists());
    ...
    // The previous key should no longer unlock.
    let store_header_path = fixture.store_header_path();
    check_passphrase(
        fixture.store_device_path(),
        LuksHeaderLocation::Detached(&store_header_path),
    ...
    .expect_err("previous key should no longer unlock after migration");

    // The new SEV key should unlock.
    let sev_key = fixture.derive_sev_key(Partition::Store);
    check_passphrase(
        fixture.store_device_path(),
        LuksHeaderLocation::Detached(&fixture.store_header_path()),
    ...
    .expect("new SEV key should unlock after migration");
}

/// SEV: can_open_store errors when all metadata tokens are malformed.
#[test]
fn test_can_open_store_errors_when_all_tokens_are_malformed() {
    let fixture = TestFixture::new_sev();
    fixture.store_partition().format().unwrap();

    let keyslot = fixture.store_partition().read_keyslot_metadata()[0]
        .keyslot()
        .unwrap();
...
/// SEV: can_open_store ignores malformed tokens when at least one valid token works.
#[test]
