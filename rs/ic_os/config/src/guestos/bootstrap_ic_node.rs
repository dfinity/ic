...
fn bootstrap_ic_node_impl(bootstrap_dir: &Path, root: &Path) -> Result<()> {
    ...
    process_bootstrap(bootstrap_dir, &config_root, &state_root)?;
...
fn process_bootstrap(bootstrap_dir: &Path, config_root: &Path, state_root: &Path) -> Result<()> {
    copy_bootstrap_files(bootstrap_dir, config_root, state_root)?;
...
mod tests {
    ...
    fn test_process_bootstrap_success() {
        ...
        let result = bootstrap_ic_node_impl(bootstrap_dir.path(), test_root.root_path());
        assert!(result.is_ok());
