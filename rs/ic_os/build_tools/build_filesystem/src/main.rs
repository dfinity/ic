...
pub(crate) fn build_filesystem(args: Args) -> Result<()> {
    ...
    if !args.strip_paths.is_empty() && args.subdir.is_some() {
        ...
        bail!("Cannot use --strip-paths and --subdir together");
    }
    ...
    let (output_path, _) = if needs_compression {
        let output_path = std::env::temp_dir().join(rand::random::<u64>().to_string());
        // Wrap output_path in TempPath so it's cleaned up when we exit the scope
        (output_path.clone(), Some(TempPath::from_path(output_path)))
    } else {
