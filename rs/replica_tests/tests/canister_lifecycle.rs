    ...
    utils::canister_test(|test| {
        ...
        assert_matches!(
            ...
            Ok(WasmResult::Reply(res)) => {
                let actual = CanisterStatusResultV2::decode(&res).unwrap();
                // The creation timestamp depends on when the test runs, so it is
                // taken from the actual status; it must be set, though.
                assert!(actual.canister_creation_timestamp().is_some());
                assert_eq!(actual, CanisterStatusResultV2::new(
                    CanisterStatusType::Running,
                    false,
                    1,
                    actual.canister_creation_timestamp(),
                    0,
                    None,
                    canister_a.get(),
                    vec![canister_a.get()],
