...
const REQUEST_SIZE_BYTES: u64 = 512;
        ...
        b.to_async(&rt).iter(|| async {
            ...
            for _ in 0..PARALLEL_REQUESTS {
                ...
                js_outer.spawn(async move {
                    ...
                    while let Some(a) = js_inner.join_next().await {
                        a.unwrap().unwrap();
                    }
                });
            }
            while let Some(a) = js_outer.join_next().await {
                a.unwrap();
            }
        })
    });
