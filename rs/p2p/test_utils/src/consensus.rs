...
use ic_interfaces::p2p::consensus::{
    ArtifactMutation, ArtifactWithOpt, ChangeResult, ChangeSetProducer, FilterFnFactory,
    FilterValue, MutablePool, UnvalidatedArtifact, ValidatedPoolReader,
};
...
impl FilterFnFactory<U64Artifact, TestConsensus<U64Artifact>> for TestConsensus<U64Artifact> {
    fn get_filter_function(
        &self,
        _pool: &TestConsensus<U64Artifact>,
    ) -> ic_interfaces::p2p::consensus::FilterFn<<U64Artifact as IdentifiableArtifact>::Id> {
        Box::new(|_| FilterValue::Wants)
    }
}
