...
// TODO(NNS1-2962) remove this constant once it's published
...
impl Governance {
    ...
    async fn perform_upgrade_to_next_sns_version(
        ...
        if target_is_root {
            ...
            // TODO(NNS1-2962) remove this code once it's published
            let dragginz_governance = CanisterId::unchecked_from_principal(
                PrincipalId::from_str("zqfso-syaaa-aaaaq-aaafq-cai").unwrap(),
            );
            let upgrade_args = if canister_type_to_upgrade == SnsCanisterType::Ledger
                && self.env.canister_id() == dragginz_governance
            {
                let ledger_upgrade_args = LedgerArgument::Upgrade(Some(UpgradeArgs {
                    metadata: Some(vec![(
                        "icrc1:logo".to_string(),
                        MetadataValue::Text(DRAGGINZ_LOGO.to_string()),
                    )]),
                    token_name: Some("Draggin Karma Points".to_string()),
                    token_symbol: Some("DKP".to_string()),
                    ..Default::default()
                }));
