...
#[derive(Debug)]
enum SlowUpgradeControlFlow {
    Stop,
    Continue,
}

pub(crate) struct SlowUpgrader {
...
impl SlowUpgrader {
    ...
    pub async fn upgrade_loop(
        ...
        loop {
            match tokio::time::timeout(liveness_timeout, self.check_for_upgrade()).await {
                Ok(Ok(SlowUpgradeControlFlow::Stop)) => cancellation_token.cancel(),
                Ok(Ok(SlowUpgradeControlFlow::Continue)) => backoff.reset(),
                e => warn!(
                    &self.logger,
                    "Check for slow GuestOS upgrade failed: {:?}", e
                ),
            }
    ...
    async fn check_for_upgrade(&mut self) -> OrchestratorResult<SlowUpgradeControlFlow> {
            ...
            && self.guestos_version != node_guestos_version
        {
            ...
            info!(
                self.logger,
                "Starting slow GuestOS upgrade at registry version {}: {} -> {}",
    ...
    async fn execute_upgrade(&mut self, version: &ReplicaVersion) -> UpgradeResult<Rebooting> {
        ...
        for release_package_url in release_package_urls.iter() {
            let result = self
                .try_run_slow_upgrade(version, release_package_url, &hash)
                .await;
            ...
            match result {
                ...
                Err(e) => {
                    info!(
                        &self.logger,
                        "Slow upgrade failed using: '{release_package_url}'"
                    );
                    error = e;
                }
            }
    ...
    async fn try_run_slow_upgrade(
        ...
        if let Err(e) = download_result {
            ...
            return Err(UpgradeError::from(e).into());
        } else {
        ...
        if !out.status.success() {
            ...
            return Err(UpgradeError::GenericError("upgrade-install failed".to_string()).into());
        }
