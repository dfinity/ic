...
fn get_delegation_with_flat_canister_ranges(criterion: &mut Criterion) {
    ...
    let mut bench_function = |canister_id_ranges_count| {
        let canister_id_ranges = (0..canister_id_ranges_count)
            ...
            .collect();
        let certificate = create_fake_certificate(&canister_id_ranges);
...
fn create_fake_certificate(canister_id_ranges: &Vec<(CanisterId, CanisterId)>) -> Certificate {
    ...
    let canister_ranges_subnet_0_subtree = LabeledTree::SubTree(FlatMap::from_key_values(
        canister_id_ranges
            ...
            .map(|(start, end)| {
                (
                    Label::from(start.to_string()),
                    LabeledTree::Leaf(serialize_to_cbor(&vec![(*start, *end)])),
                )
            })
            .collect(),
    ));
    let canister_ranges_subtree = LabeledTree::SubTree(flatmap![
        Label::from(SUBNET_0.get_ref().to_vec()) => canister_ranges_subnet_0_subtree,
    ]);
