...
The canister currently uses 924 MiB, so a 1 GiB allocation leaves 100 MiB of headroom (1.11x current usage). Block archiving is currently disabled, so the archive receives no new blocks and its usage is static. The allocation only needs to cover the canister as it stands, including upgrades: the archive has no `pre_upgrade` hook, and its `post_upgrade` merely reads the stable-structure headers, so an upgrade allocates essentially nothing.
