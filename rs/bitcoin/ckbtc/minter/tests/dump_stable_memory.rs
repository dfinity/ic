...
use pocket_ic::{call_candid, PocketIc, PocketIcBuilder};
...
fn read_events_file(file_name: &str) -> GetEventsResult {
    ...
    let file = File::open(path_to_events_file(file_name)).unwrap();
...
fn test_minter_dump_stable_mem_mainnet() {
    ...
    mem_path.set_extension("mem");
    std::fs::write(mem_path, mem).unwrap();
}

#[test]
fn test_minter_event_upload_mainnet() {
    let file_name = "mainnet_events.gz";
    let network = Network::Mainnet;
    let setup = Setup::new(network);
    upload_events(&setup, file_name);
