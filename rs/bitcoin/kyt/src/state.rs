...
// Max number entries in the cache is set to 10_000. Since the average transaction size
// is about 400 bytes, the estimated storage space for the cache to in the order of 10s of MB.
...
impl<T> FetchTxCache<T> {
    ...
    pub fn set_status_with(&mut self, txid: Txid, status: T, now: Timestamp) {
    ...
    pub fn clear_status(&mut self, txid: Txid) {
...
// Mock the time syscall when running tests.
#[cfg(test)]
fn time() -> u64 {
    0
}
