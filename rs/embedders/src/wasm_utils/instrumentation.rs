...
fn export_table(mut module: orca_wasm::Module) -> orca_wasm::Module {
    ...
    for export in module.exports.iter_mut() {
        if let orca_wasm::wasmparser::ExternalKind::Table = export.kind {
    ...
    if !table_already_exported && !module.tables.is_empty() {
        module.exports.add_export(
            TABLE_STR.to_string(),
            0,
            orca_wasm::wasmparser::ExternalKind::Table,
        );
    }
...
fn update_memories(
    ...
    for export in module.exports.iter_mut() {
        if let orca_wasm::wasmparser::ExternalKind::Memory = export.kind {
    ...
    if !memory_already_exported && !module.memories.is_empty() {
        module.exports.add_export(
            WASM_HEAP_MEMORY_NAME.to_string(),
            0,
            orca_wasm::wasmparser::ExternalKind::Memory,
        );
    }
    ...
    if write_barrier == FlagStatus::Enabled && !module.memories.is_empty() {
        // TODO: Make sure this doesn't replace an existing memory.
        module.add_local_memory(orca_wasm::wasmparser::MemoryType {
            memory64: false,
            shared: false,
        ...
        module.exports.add_export(
            WASM_HEAP_BYTEMAP_MEMORY_NAME.to_string(),
            1,
            orca_wasm::wasmparser::ExternalKind::Memory,
        );
    }

    let stable_index = module.memories.iter().count() as u32;
    module.add_local_memory(orca_wasm::wasmparser::MemoryType {
        memory64: true,
        shared: false,
        initial: 0,
...
fn export_mutable_globals<'a>(
    ...
    for export in module.exports.iter() {
        if let orca_wasm::wasmparser::ExternalKind::Global = export.kind {
...
fn export_additional_symbols<'a>(
    ...
    .into_iter()
    .map(|op| orca_wasm::ir::types::Instruction::new(op))
    .collect();
    ...
    let ty_id = module.types.add_func_type(
    ...
    if let Some(index) = module.start.map(|s| s.0) {
        ...
        module.exports.add_export(
            CANISTER_START_STR.to_string(),
            index,
            orca_wasm::wasmparser::ExternalKind::Func,
        )
    }
...
fn injections(
    code: &[orca_wasm::ir::types::Instruction],
    mem_type: WasmMemoryType,
) -> Vec<InjectionPoint> {
    let mut res = Vec::new();
    use orca_wasm::wasmparser::Operator::*;
...
fn inject_metering(
    ...
    let mut elems: Vec<orca_wasm::ir::types::Instruction> = Vec::new();
...
fn calculate_api_indexes(module: &orca_wasm::Module<'_>) -> BTreeMap<SystemApiFunc, u32> {
    module
        .imports
        .iter()
        .filter(|imp| matches!(imp.ty, orca_wasm::wasmparser::TypeRef::Func(_)))
        .enumerate()
...
fn replace_system_api_functions(
    ...
    for (api, (param, ret, body)) in replacement_functions(
        ...
        if let Some(old_index) = api_indexes.get(&api) {
            let type_id = module.types.add_func_type(&param, &ret);
            let fun = LocalFunction::new(type_id, FunctionID(0), body, param.len());
            let new_index = module.functions.add_local_func(fun, None).0;
...
fn get_data(
    data_section: &mut Vec<orca_wasm::DataSegment>,
) -> Result<Segments, WasmInstrumentationError> {
    let res = data_section
        .iter()
        .filter_map(|segment| {
            let offset = match &segment.kind {
                orca_wasm::ir::types::DataSegmentKind::Active {
                    memory_index: _,
                    offset_expr,
                } => match offset_expr.instructions() {
                    [orca_wasm::Instructions::Value(orca_wasm::ir::types::Value::I32(value))] => *value as usize,
                    [orca_wasm::Instructions::Value(orca_wasm::ir::types::Value::I64(value))] => *value as usize,
                ...
                orca_wasm::ir::types::DataSegmentKind::Passive => return None,
            };
    ...
    for index in (0..data_section.len()).rev() {
        ...
        match kind {
            orca_wasm::DataSegmentKind::Passive => ends_with_passive_segment = true,
            orca_wasm::DataSegmentKind::Active { .. } => {
                if ends_with_passive_segment {
                    data_section[index] = orca_wasm::DataSegment {
                        kind: kind.clone(),
                        data: vec![],
                    };
                } else {
