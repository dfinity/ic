...
fn inject_helper_functions(
    ...
    //     if let wirm::wasmparser::TypeRef::Func(i) = import.ty {
...
fn update_memories(
    ...
    if write_barrier == FlagStatus::Enabled && !module.memories.is_empty() {
        ...
        let heap_bytemap_index = module.add_local_memory(wirm::wasmparser::MemoryType {
            memory64: false,
            shared: false,
...
) -> (InjectedLocalGlobals, wirm::Module<'a>) {
...
pub(super) fn instrument(
    ...
    //     match imp.ty {
    //         wirm::wasmparser::TypeRef::Func(_) => {
    //             num_imported_functions += 1;
    //         }
    //         wirm::wasmparser::TypeRef::Global(_) => {
