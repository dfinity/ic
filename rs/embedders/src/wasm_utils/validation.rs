...
use orca_wasm::{
    Module,
    ir::{module::{LocalOrImport, module_functions::FuncKind, module_types::Types}, types::{Body, Value}}, wasmparser::{ ExternalKind, Operator, TypeRef, ValType}, DataSegment, DataSegmentKind, DataType, Instructions
};
            ...
            match &entry.ty {
                TypeRef::Func(index) => {
                    let (params, results) = if let Types::FuncType {
                        params, results, ..
                    } = &module.types.types[*index as usize]
                    {
                        (params, results)
                    } else {
        ...
        for export in module.exports.iter() {
            ...
            if ExternalKind::Func == export.kind {
                ...
                if let Some(valid_signature) = valid_exported_functions.get(func_name) {
                    let fn_id = module.exports.get_func_by_name(&export.name).unwrap();
    ...
    fn validate_segment(
        ...
        match (&s.kind, mem_type) {
            ...
            (
                ...
                [Instructions::Value(Value::I32(_))] => Ok(()),
            ...
            (
                ...
                [Instructions::Value(Value::I64(_))] => Ok(()),
...
fn validate_global_section(module: &Module, max_globals: usize) -> Result<(), WasmValidationError> {
    ...
    for global in module.globals.iter() {
        match global.kind().ty().content_type {
