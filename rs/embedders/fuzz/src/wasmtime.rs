use crate::ic_wasm::{ic_embedders_config, ICWasmModule, SystemApiModule};
...
#[inline(always)]
pub fn run_fuzzer(module: SystemApiModule) -> Corpus {
    let wasm = module.module;
    let wasm_methods: BTreeSet<WasmMethod> = module.exported_functions;

    let config = ic_embedders_config(module.memory64_enabled);
...
mod tests {
    ...
    fn test_execute_with_wasmtime_single_run() {
        ...
        let module =
            <crate::ic_wasm::SystemApiModule as Arbitrary>::arbitrary_take_rest(unstrucutred)
