...
fn run_charge_for_heap_pages(wasm_memory_type: WasmMemoryType) {
    ...
    // adjust their code to new metering): the memory tracker subtracts from the
    // counter in the SIGSEGV handler, and the only place this module checks the
    // counter is the reentrant block at function entry, before the stores. The
    // stable memory tests below cannot assert the same, because the injected
    // stable API replacements do check the counter after the tracker charged.
    let mut instance = new_instance(&wat, 100);
