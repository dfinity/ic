...
use saturating_subnet_memory_test::Metrics;
...
fn new_fixture(
...
) -> (StateMachine, Vec<CanisterId>, StateMachine, Vec<CanisterId>) {
    ...
    let wasm = Project::cargo_bin_maybe_from_env("saturating-subnet-memory-test-canister", &[]).bytes();
    ...
    (local_env, local_canister_ids, remote_env, remote_canister_ids)
}
...
fn inter_canister_traffic_respects_memory_limits() {
    ...
    let reply = local_env.query(local_canister_ids[0], "metrics", vec![]).unwrap();
