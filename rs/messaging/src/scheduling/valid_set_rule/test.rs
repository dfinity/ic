...
fn dont_induct_duplicate_messages() {
    with_test_replica_logger(|log| {
        ...
        valid_set_rule.induct_messages(&mut state, vec![msg1, msg2, msg3], Height::from(0));
...
fn canister_on_application_subnet_charges_for_ingress() {
    ...
    valid_set_rule.induct_messages(&mut state, vec![signed_ingress], Height::from(0));
...
fn canister_on_system_subnet_does_not_charge_for_ingress() {
    ...
    let msg = SignedIngressBuilder::new()
        .canister_id(canister_test_id(0))
        .build();
    valid_set_rule.induct_messages(&mut state, vec![msg], Height::from(0));
...
fn ingress_history_max_messages_impl(subnet_type: SubnetType) {
    with_test_replica_logger(|log| {
        ...
        valid_set_rule.induct_messages(&mut state, msgs, Height::from(0));
