...
fn legacy_induct_loopback_stream_reroute_response() {
    with_legacy_local_test_setup(
        ...
        |stream_handler, state, metrics| {
            ...
            push_input(
                &mut expected_state,
                message_in_stream(state.get_stream(&LOCAL_SUBNET), 21).clone(),
            );
...
#[test]
fn inducting_duplicate_best_effort_responses_does_not_raise_a_critical_error() {
    with_local_test_setup(
        btreemap![LOCAL_SUBNET => StreamConfig {
            begin: 21,
        ...
        |stream_handler, state, metrics| {
            fn assert_one_message_inducted(state: &ReplicatedState, metrics: &MetricsFixture) {
                // No critical errors raised.
                metrics.assert_eq_critical_errors(CriticalErrorCounts::default());
                // Exactly one message inducted.
                assert_eq!(
                    1,
                    state
            ...
            // Clone the best effort response.
            let response = message_in_stream(state.get_stream(&LOCAL_SUBNET), 21).clone();

            // Induct the loopback stream and assert metrics.
            let mut state = stream_handler.induct_loopback_stream(state, &mut (i64::MAX / 2));
            assert_one_message_inducted(&state, &metrics);

            // Push the cloned response onto the loopback stream.
            state.modify_streams(|streams| {
                streams
                    .get_mut(&LOCAL_SUBNET)
                    .unwrap()
                    .push(response.clone())
            });
            // The Duplicate response should be dropped silently.
            let mut state = stream_handler.induct_loopback_stream(state, &mut (i64::MAX / 2));
            assert_one_message_inducted(&state, &metrics);
            ...
            // Remove the `LOCAL_CANISTER` and push the cloned response onto the
            // loopback stream.
            state.canister_states.remove(&LOCAL_CANISTER).unwrap();
            state.modify_streams(|streams| {
                streams
            ...
            // The duplicate response should be dropped silently.
            let _state = stream_handler.induct_loopback_stream(state, &mut (i64::MAX / 2));
            // No critical errors raised.
            metrics.assert_eq_critical_errors(CriticalErrorCounts::default());
        },
    );
