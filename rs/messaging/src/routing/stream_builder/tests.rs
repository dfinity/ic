...
const DESTINATION_CANISTER: CanisterId = CanisterId::from_u64(1);
...
/// `DESTINATION_CANISTER` by `COOLING_DOWN_SUBNET` (which is cooling down) and
    ...
    state.metadata.modify_network_topology(|network_topology| {
        ...
        network_topology.set_routing_table(
            RoutingTable::try_from(btreemap! {
                ...
                CanisterIdRange { start: DESTINATION_CANISTER, end: DESTINATION_CANISTER } => COOLING_DOWN_SUBNET,
...
/// Same as `new_cooling_down_fixture()`, except that `DESTINATION_CANISTER` is
/// hosted by `LOCAL_SUBNET` and it is `LOCAL_SUBNET` that is cooling down; i.e.
/// the messages to `DESTINATION_CANISTER` would go into the loopback stream.
fn new_loopback_cooling_down_fixture(
    ...
    state.metadata.modify_network_topology(|network_topology| {
        ...
        network_topology.set_routing_table(
            RoutingTable::try_from(btreemap! {
                CanisterIdRange { start: SENDER_CANISTER, end: DESTINATION_CANISTER } => LOCAL_SUBNET,
                CanisterIdRange { start: OTHER_CANISTER, end: OTHER_CANISTER } => OTHER_SUBNET,
            })
            .unwrap(),
        );
...
fn build_streams_routes_messages_to_other_subnets_while_one_is_cooling_down() {
    with_test_replica_logger(|log| {
        let retained = request_from_sender(DESTINATION_CANISTER, NO_DEADLINE, Cycles::zero());
        let routed =
            request_from_sender_with_callback(OTHER_CANISTER, NO_DEADLINE, Cycles::zero(), 2);

        let (stream_builder, mut provided_state, metrics_registry) = new_cooling_down_fixture(&log);
        ...
        assert_eq!(
            vec![retained],
            output_queue_contents(&result_state, SENDER_CANISTER, DESTINATION_CANISTER)
        );
        assert_eq!(
