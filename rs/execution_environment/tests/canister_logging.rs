...
use ic_types::{
    CanisterId, Cycles, DEFAULT_LOG_MEMORY_LIMIT, NumInstructions, ingress::WasmResult,
};
...
fn test_canister_log_stays_within_limit() {
    ...
    let (env, canister_id) = setup_with_controller(
        ...
        wat_canister()
            .update(
                "test",
                wat_fn().debug_print(&[42; DEFAULT_LOG_MEMORY_LIMIT]),
            )
            .build_wasm(),
    );
...
fn test_canister_log_in_state_stays_within_limit() {
    ...
    let (env, canister_id) = setup_with_controller(
        ...
        wat_canister()
            .update(
                "test",
                wat_fn()
                    .debug_print(&[b'd'; DEFAULT_LOG_MEMORY_LIMIT])
                    .trap_with_blob(&[b't'; DEFAULT_LOG_MEMORY_LIMIT]),
            )
            .build_wasm(),
    );
    for _ in 0..MESSAGES_NUMBER {
    ...
    assert_le!(log_size, DEFAULT_LOG_MEMORY_LIMIT);
}
