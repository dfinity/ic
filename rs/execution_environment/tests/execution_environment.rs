...
use ic_interfaces::{
    execution_environment::{
        CanisterHeartbeatError, ExecuteMessageResult, ExecutionEnvironment, MessageAcceptanceError,
...
use ic_replicated_state::{
    canister_state::testing::CanisterStateTesting, canister_state::QUEUE_INDEX_NONE,
    testing::CanisterQueuesTesting, CallContextManager, CallOrigin, CanisterState, CanisterStatus,
...
fn stopping_canister_rejects_requests() {
    with_setup(
        ...
        |exec_env, mut state, _, routing_table, subnet_records| {
            ...
            let cycles = 40;
            let icp = 100;
            let funds = Funds::new(Cycles::from(cycles), ICPTap::mint(icp));
            ...
            let mut state = exec_env.execute_subnet_message(
            ...
            assert_eq!(
                ...
                RequestOrResponse::Response(Response {
                    ...
                    refund: Funds::new(Cycles::from(cycles), ICPTap::mint(icp)),
...
fn stopped_canister_rejects_requests() {
    with_setup(
        ...
        |exec_env, _, _, routing_table, subnet_records| {
            ...
            let req = RequestBuilder::new()
                .sender(canister_test_id(13))
                .payment(Funds::new(Cycles::from(cycles), ICPTap::mint(icp)))
                .build();
            let reply_callback = req.sender_reply_callback;
            canister
                .push_input(QueueIndex::from(0), RequestOrResponse::Request(req))
...
fn execute_stop_canister_updates_ingress_history_when_called_on_already_stopped_canister() {
    with_setup(SubnetType::Application, |exec_env, mut state, _, _, _| {
        ...
        let payload = Encode!(&CanisterIdRecord::from(canister_test_id(0))).unwrap();
        let state = exec_env.execute_subnet_message(
            CanisterInputMessage::Ingress(
                IngressBuilder::new()
                    .source(user_test_id(1))
                    .method_payload(payload)
...
fn test_canister_status_helper(
    ...
    with_setup(SubnetType::Application, |exec_env, mut state, _, _, _| {
        ...
        let mut state = exec_env.execute_subnet_message(
            state.subnet_queues.pop_input().unwrap(),
            state,
            MAX_NUM_INSTRUCTIONS,
            &mut mock_random_number_generator(),
            &ProvisionalWhitelist::Set(BTreeSet::new()),
...
fn subnet_canister_request_unknown_method() {
    with_setup(SubnetType::Application, |exec_env, mut state, _, _, _| {
        ...
        assert_eq!(
            state.subnet_queues.pop_canister_output(&sender).unwrap().1,
            RequestOrResponse::Response(Response {
                originator: sender,
                respondent: receiver,
        ...
        .push_input(
            ...
            RequestOrResponse::Request(
                RequestBuilder::new()
                    ...
                    .method_name(Method::CreateCanister)
                    .method_payload(EmptyBlob::encode())
                    .payment(Funds::new(Cycles::from(cycles.get()), ICPTap::mint(icp)))
                    .build(),
            ),
        )
        .unwrap();
...
fn check_create_canister_fails(
    ...
    assert_eq!(
        ...
        RequestOrResponse::Response(Response {
            ...
            originator_reply_callback: CallbackId::new(0),
            refund: Funds::new(CANISTER_CREATION_FEE + Cycles::from(1), ICPTap::mint(7)),
            response_payload: Payload::Reject(RejectContext {
                code: RejectCode::CanisterError,
                message:
...
fn check_create_canister_succeeds(
    ...
    let response = state.subnet_queues.pop_canister_output(&sender).unwrap().1;
    match response {
        RequestOrResponse::Response(response) => {
            ...
            assert_eq!(
                response.refund,
                Funds::new(Cycles::from(0), ICPTap::mint(0))
            );
...
fn setup_initial_dkg_sender_on_nns() {
    with_test_replica_logger(|log| {
        ...
        assert_eq!(state.subnet_queues.pop_canister_output(&sender), None);
    });
}
    ...
    with_test_replica_logger(|log| {
        ...
        assert_eq!(
            ...
            RequestOrResponse::Response(Response {
                ...
                respondent: CanisterId::from(own_subnet_id),
                originator_reply_callback: CallbackId::new(0),
                refund: Funds::new(CANISTER_CREATION_FEE, ICPTap::mint(7)),
                response_payload: Payload::Reject(RejectContext {
                    code: RejectCode::CanisterError,
                    message: format!(
    ...
    with_test_replica_logger(|log| {
        ...
        assert_eq!(
            metric_vec(&[
                ...
                (
                    &[("method_name", "ic00_set_controller"), ("outcome", "error")],
                    1
                ),
                (
                    &[("method_name", "ic00_start_canister"), ("outcome", "error")],
                    1
                ),
                (
                    &[("method_name", "ic00_stop_canister"), ("outcome", "error")],
                    1
                ),
                (
                    &[
...
#[test]
fn canister_heartbeat_doesnt_run_on_application_subnet() {
    with_setup(
        SubnetType::Application,
        |exec_env, _, _, routing_table, subnet_records| {
            let canister = get_stopped_canister(canister_test_id(0));
            ...
            assert_eq!(
                ...
                Err(CanisterHeartbeatError::NotSystemSubnet {
                    subnet_type_given: SubnetType::Application,
                })
            );
        },
    );
...
fn message_to_canister_with_not_enough_balance_is_rejected() {
    with_setup(SubnetType::Application, |exec_env, _, _, _, _| {
        ...
        assert_eq!(
            ...
            Err(MessageAcceptanceError::CanisterOutOfCycles)
        );
    });
}
    ...
    with_setup(SubnetType::Application, |exec_env, _, _, _, _| {
        for receiver in [IC_00, CanisterId::from(subnet_test_id(1))].iter() {
            ...
            assert_eq!(
                ...
                Err(MessageAcceptanceError::CanisterNotFound)
            );
        }
    });
}
