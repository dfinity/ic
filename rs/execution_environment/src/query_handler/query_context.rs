...
///
/// `Send + 'static`, which `send_assertions` below pins: adding a borrowed
/// field, or anything `Rc`-based such as a `MeasurementScope`, breaks it.
...
#[cfg(test)]
// The lint is what makes a re-added lifetime parameter fail here: elided, it
// would be inferred as `'static` and satisfy `Send` for every shared field.
