...
fn smooth_heap_delta_rate_limiting() {
    ...
    let mut test = SchedulerTestBuilder::new()
        .with_scheduler_config(SchedulerConfig {
            ...
            instruction_overhead_per_message: NumInstructions::from(0),
            instruction_overhead_per_canister: NumInstructions::from(0),
            heap_delta_reserve_per_round: NumBytes::from(page_size),
            ..SchedulerConfig::application_subnet()
        })
        .with_cup_interval(10)
        .build();
...
fn smooth_heap_delta_rate_limiting_reaches_the_limit() {
    ...
    let mut test = SchedulerTestBuilder::new()
        .with_scheduler_config(SchedulerConfig {
            ...
            heap_delta_reserve_per_round: NumBytes::from(page_size * 2),
            ..SchedulerConfig::application_subnet()
        })
        .with_cup_interval(rounds)
        .build();
    let canister_id = test.create_canister();
...
#[test]
fn scheduled_heap_delta_limit_grows_linearly() {
    assert_eq!(
        NumBytes::from(0),
        current_heap_delta_reserve(0_u64.into(), None, 10_u64.into())
    );
    assert_eq!(
        NumBytes::from(0),
        current_heap_delta_reserve(0_u64.into(), None, 0_u64.into())
    );
    assert_eq!(
        NumBytes::from(110),
        current_heap_delta_reserve(0_u64.into(), Some(10_u64.into()), 11_u64.into())
    );
    assert_eq!(
        NumBytes::from(99),
        current_heap_delta_reserve(1_u64.into(), Some(10_u64.into()), 11_u64.into())
    );
    assert_eq!(
        NumBytes::from(88),
        current_heap_delta_reserve(2_u64.into(), Some(10_u64.into()), 11_u64.into())
    );
    assert_eq!(
        NumBytes::from(0),
        current_heap_delta_reserve(100_u64.into(), Some(10_u64.into()), 0_u64.into())
    );
    assert_eq!(
        NumBytes::from(0),
        current_heap_delta_reserve(10_u64.into(), Some(10_u64.into()), 11_u64.into())
    );
    assert_eq!(
        NumBytes::from(0),
        current_heap_delta_reserve(11_u64.into(), Some(10_u64.into()), 11_u64.into())
    );
}
