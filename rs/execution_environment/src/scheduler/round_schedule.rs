...
pub struct RoundSchedule {
    ...
    /// Sum of all canisters' compute allocations.
...
impl RoundSchedule {
    ...
    > {
        SchedulingOrder {
            ...
            prioritized_long_canisters: self
                .ordered_long_execution_canister_ids
                .iter()
            ...
            // message execution.
            new_canisters: self.ordered_new_execution_canister_ids.iter(),
    ...
    pub fn filter_canisters(
        ...
        let canister_next_executions: BTreeMap<_, _> = canisters
            .iter()
    ...
    pub(super) fn partition_canisters_to_cores(
        ...
        // With scheduler_cores == 1, new_execution_cores can be 0 when there
        // is a prioritized long execution occupying the single core.
        debug_assert!(self.scheduler_cores <= 1 || new_execution_cores > 0);
