...
/// As opposed to the pending refunds above, the pooled refund counts cover all
/// refunds ever pushed into the pool, so they don't go down as it is drained.
#[test]
fn replicated_state_metrics_pooled_refunds() {
    const NAME: &str = "replicated_state_pooled_refunds_total";
    const CYCLES_NAME: &str = "replicated_state_pooled_refunds_cycles_total";
    ...
    state.add_refund(canister_test_id(1), Cycles::new(29));
    let registry = observe(&state);
    assert_eq!(
        Some(1),
