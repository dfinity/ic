...
fn dts_allow_only_one_long_install_code_execution_at_a_time() {
    ...
    assert_eq!(metrics.round_inner_subnet_queue.slices.get_sample_sum(), 1.0);
    assert_eq!(metrics.round_inner_subnet_queue.messages.get_sample_sum(), 0.0);
    ...
    assert_eq!(test.state().subnet_queues().input_queues_message_count(), 0);
    let metrics = test.scheduler().metrics.as_ref();
    assert_eq!(metrics.round_inner_subnet_queue.slices.get_sample_sum(), 2.0);
    assert_eq!(metrics.round_inner_subnet_queue.messages.get_sample_sum(), 1.0);
    assert_eq!(
        metrics
            .round_advance_long_install_code
            .slices
