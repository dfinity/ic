...
impl CanisterManager {
    ...
    fn do_update_settings(
        ...
        if let Some(log_memory_limit) = settings.log_memory_limit() {
            // Preserve the setting and resize the log memory store.
            canister.system_state.log_memory_limit = log_memory_limit;
            canister
                .system_state
                .log_memory_store
                .resize(log_memory_limit.get() as usize);
        }
    ...
    pub(crate) fn get_canister_status(
        ...
        let log_memory_limit = canister.system_state.log_memory_limit.get();
