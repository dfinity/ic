...
1. Specify the new API or make the required changes to an existing one in the *Interface Specification* in the [developer-docs](https://github.com/dfinity/developer-docs) repository:
   - typically, you would want to make changes in the [following directory](https://github.com/dfinity/developer-docs/tree/main/docs/references/ic-interface-spec);
   - additionally, make sure you update the [Candid interface](https://github.com/dfinity/developer-docs/blob/main/public/references/ic.did) of the Management Canister accordingly;
...
6. Update the candid declarations in `rs/types/management_canister_types/tests/ic.did`. If a new management canister API is introduced, add a placeholder for it in `rs/types/management_canister_types/tests/candid_equality.rs`.
...
7. Implement the new API in the `execution_environment` crate. Main parts that need to be updated are:
   ...
   Note: Some management methods (e.g., `install_code` and `install_chunked_code`) include dedicated logic to handle rollback after failures.
   However, for the majority of methods, the function `ExecutionEnvironment::execute_mgmt_operation_on_canister` should be used
   which takes the actual management canister operation as a closure and handles rollback after failure.

   If a feature cannot be implemented in one PR or the API should be experimental for a time, define a feature flag in `rs/config/src/execution_environment.rs` and gate the API behind the flag. This way, the feature can be rolled out to a few select subnets via a feature release. Typically, there are tests that set the flags either way and ensure the proper behaviour in each case.
...
9. The new functionality can be enabled for testing in PocketIC (on a PocketIC instance created with `beta_features` enabled) by enabling the corresponding feature flags in `rs/pocket_ic_server/src/beta_features.rs`.
