...
pub fn registration(env: TestEnv) {
    ...
    // On SEV, SSH key setup is intentionally disabled, so skip SSH-based checks.
    if std::env::var("TRUSTED_EXECUTION_ENVIRONMENT").is_ok() {
        info!(
            logger,
            "Skipping GuestOS SSH systemd check because TRUSTED_EXECUTION_ENVIRONMENT is enabled."
        );
    } else {
