...
use ic_agent::{
    ...
    agent::{CallResponse, EnvelopeContent, RejectCode, RejectResponse},
...
impl<'a> UniversalCanister<'a> {
    ...
    pub async fn new_with_params(
        ...
        let mut create_canister = mgr.create_canister();
        if let Some(ca) = compute_allocation {
            create_canister = create_canister.with_compute_allocation(ca);
        }
