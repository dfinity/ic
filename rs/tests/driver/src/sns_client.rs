...
use ic_nns_governance_api::{
    CreateServiceNervousSystem, ManageNeuronCommandRequest, ManageNeuronRequest,
    ManageNeuronResponse, NnsFunction, MakePropoalRequest, ProposalActionRequest,
    ...
    let manage_neuron_payload = ManageNeuronRequest {
        ...
        command: Some(ManageNeuronCommandRequest::MakeProposal(Box::new(
            Proposal {
                ...
                summary: "summary".to_string(),
                url: "https://forum.dfinity.org/t/x/".to_string(),
                action: Some(ProposalActionRequest::OpenSnsTokenSwap(payload)),
            },
        ))),
    };
    let proposer = Sender::from_keypair(&TEST_NEURON_1_OWNER_KEYPAIR);
...
>>>>>>> b7cd341d02 (Fix places where ManageNeuron is incorrectly used)
