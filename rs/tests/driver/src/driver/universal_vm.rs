...
impl UniversalVm {
    ...
    pub fn start(&self, env: &TestEnv) -> Result<()> {
        block_on(self.start_async(env))
    }

    pub async fn start_async(&self, env: &TestEnv) -> Result<()> {
        let farm = Farm::from_test_env(env, "universal VM").await;
        ...
        let resource_group = allocate_resources(&farm, &res_request, env).await?;
        ...
        if InfraProvider::read_attribute(env) == InfraProvider::Farm {
            ...
            let ssh_config_img_file_spec = AttachImageSpec::new(
                farm.upload_file(
        ...
        if let Some(config) = &self.config {
            ...
            if InfraProvider::read_attribute(env) == InfraProvider::Farm {
                ...
                let upload = match farm
                    .claim_file(&pot_setup.infra_group_name, &file_id)
                    .await?
                {
