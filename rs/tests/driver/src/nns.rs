...
use ic_nns_governance_api::{
    ...
    ManageNeuron, ManageNeuronRequest, ManageNeuronResponse, NnsFunction, ProposalInfo,
    ProposalStatus, Vote, MakeProposalRequest, ProposalActionRequest, FulfillSubnetRentalRequest, ManageNeuronCommandRequest,
};
...
pub async fn execute_fulfill_subnet_rental_request(
    ...
    let proposal = MakeProposalRequest {
        title: Some("Fulfill the Subnet Rental Request".to_string()),
                     ...
                     burned down over time".to_string(),
        url: "https://forum.dfinity.org/wer-subnet".to_string(),
        action: Some(ProposalActionRequest::FulfillSubnetRentalRequest(FulfillSubnetRentalRequest {
            user,
            node_ids,
            replica_version_id,
        })),
    ...
    let neuron_id_or_subaccount = Some(NeuronIdOrSubaccount::NeuronId(
        ic_nns_common::pb::v1::NeuronId { id: TEST_NEURON_1_ID }
    ));
