...
def system_test(
    ...
    Args:
      ...
      additional_colocate_flags: additional flags to pass to the colocated test.
    ...
    run_system_test(
        ...
        env = env,
        tags = tags + ["requires-network", "system_test"] +
               ([] if "experimental_system_test_colocation" in tags else ["manual"]) + additional_colocate_flags,
        timeout = test_timeout,
        flaky = flaky,
    )
