...
fn main() -> Result<()> {
    SystemTestGroup::new()
        ...
        // One of the nodes has corrupted state and proposes a CUP share which will be invalidated
        // by the peers (in vice versa), so it's expected that the metric is increased.
