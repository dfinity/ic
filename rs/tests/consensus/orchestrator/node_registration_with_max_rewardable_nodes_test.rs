...
. Add a node reward table and one rewardable node.
...
use ic_consensus_system_test_node_registration_test_common::{
    setup, test_with_max_rewardable_nodes,
};
...
fn main() -> Result<()> {
    SystemTestGroup::new()
        .with_setup(setup)
        .add_test(systest!(test_with_max_rewardable_nodes))
        .execute_from_args()?;
