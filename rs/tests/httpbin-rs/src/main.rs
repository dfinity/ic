...
async fn main() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    match (args.cert_file, args.key_file) {
            ...
            loop {
                select! {
                    connection = listener.accept() => {
                        // CHANGE 1: Capture the remote address here
                        ...
                        tokio::spawn(async move {
                            ...
                            // CHANGE 2: Create a service that adds the ConnectInfo extension
                            let hyper_service =
                                hyper::service::service_fn(move |mut request: Request<Incoming>| {
                                    // Manually insert the ConnectInfo for the extractor to find
        ...
        _ => {
            // This part works correctly because axum::serve adds ConnectInfo automatically
