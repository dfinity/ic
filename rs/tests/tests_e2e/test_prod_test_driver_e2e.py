...
class TestRunSystemTests(unittest.TestCase):
    ...
    def _run_suite_with_early_failure(self, expected_error):
    ...
    def _run_suite_to_completion(self, suite_name, expected_exit_code, expected_suite_contract, expected_suite_result):
        # We need to keep the artifacts, otherwise the first test will erase them and consequent tests will fail.
        process = subprocess.run(
            [f"{self.script_path}", f"--suite={suite_name}", "--keep_artifacts", "--keep_tmp_dirs"], env=self.env
        )
        ...
        self.assertDictEqual(expected_suite_contract, actual_suite_contract)
        # Assert correct test-result.json content.
        actual_suite_result = self._find_parse_json_by_name(self.suite_result_wildcard)
        self.assertDictEqual(expected_suite_result, actual_suite_result)
