...
//     Recovered NNS is working successfully, is available at http://[2a0b:21c0:4003:2:5004:41ff:feea:7cef]:8080/
//     and can be interacted with using neuron ID NeuronId(5453944968753393786)
...
const PER_TEST_TIMEOUT: Duration = Duration::from_secs(50 * 60);
...
#[derive(Deserialize, Serialize)]
struct RecoveredNnsDictatorNeuron {
...
fn prepare_nns_state(env: TestEnv) -> NeuronId {
    ...
    */
    ...
    info!(logger, "Give our principal 1 million ICP");
    replay(ReplayToolArgs {
        config: Some(ic_config_file),
        canister_caller_id: None,
...
fn test_recovered_nns(env: TestEnv, neuron_id: NeuronId, nns_node: IcNodeSnapshot) {
    ...
    info!(
        logger,
        "Recovered NNS is working successfully, is available at {} and can be interacted with using neuron ID {:?}",
