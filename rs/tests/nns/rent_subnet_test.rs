...
pub fn test(env: TestEnv) {
    ...
    block_on(async move {
        ...
        assert_subnet_works(new_subnet_id, &topology_snapshot, log).await;
    });
}
...
#[must_use]
async fn execute_fulfill_subnet_rental_request(topology_snapshot: &TopologySnapshot) -> TopologySnapshot{
...
async fn assert_new_subnet(
    ...
    let final_subnets = HashSet::from_iter(
        get_subnet_list_from_registry(&registry_canister)
            .await
    );
...
///
///     7. Other principals are not allowed to create canisters in the rented
///        subnet.
///
...
async fn assert_subnet_works(
    ...
    let new_canister =
        UniversalCanister::new_with_retries(
            &agent,
            a_rented_subnet_node.effective_canister_id(),
            // DO NOT MERGE 1_000_000_000_000_000, // cycles. 1e15, i.e. 1 trillion
            log,
        )
        .await;
    let new_canister_principal_id = CanisterId::unchecked_from_principal(
        PrincipalId::from(new_canister.canister_id())
    );
    ...
    let original_cycles_balance = get_cycles_balance(
        new_canister_principal_id,
        &rented_subnet,
    ).await;
    ...
    new_canister.store_to_stable(0, UPDATE_MSG_1).await;
    ...
    let later_cycles_balance = get_cycles_balance(
        new_canister_principal_id,
        &rented_subnet,
    ).await;
    assert_eq!(
    ...
    // != is used here, bcause the operands do not implement Debug.
    assert!(*SUBNET_USER_KEYPAIR != *TEST_USER2_KEYPAIR);
    agent.set_identity(
        BasicIdentity::from_pem(std::io::Cursor::new(TEST_USER2_KEYPAIR.to_pem())).unwrap(),
    );

    // As another user, try to create a canister in the rented subnet. This is supposed to get blocked.
    let create_canister_result = UniversalCanister::try_new(
        &agent,
        a_rented_subnet_node.effective_canister_id(),
    )
    .await;
    assert!(create_canister_result.is_err());
}
...
async fn assert_canister_belongs_to_subnet(
    ...
    let new_canister_subnet_id = get_subnet_for_canister_result
        .unwrap()
        .subnet_id
        .unwrap();
    assert_eq!(
        new_canister_subnet_id,
