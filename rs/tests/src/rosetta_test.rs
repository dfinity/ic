...
pub fn test_everything(handle: IcHandle, ctx: &ic_fondue::pot::Context) {
    ...
    neuron_tests.add(
        &mut ledger_balances,
        "Test spawn neuron with partial maturity",
        rand::random(),
        |neuron| {
            neuron.dissolve_state = None;
            neuron.maturity_e8s_equivalent = 500_000_000;
    ...
    neuron_tests.add(
        &mut ledger_balances,
        "Test get neuron info with hotkey raw",
        rand::random(),
        |neuron| {
            neuron.dissolve_state = Some(
                ic_nns_governance::pb::v1::neuron::DissolveState::DissolveDelaySeconds(
    ...
    rt.block_on(async move {
        ...
        let neuron_info = neuron_tests.get_neuron_for_test("Test follow with hotkey raw");
        test_follow_with_hotkey_raw(&rosetta_api_serv, &ledger_for_governance, neuron_info).await;
