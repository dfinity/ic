...
const API_BOUNDARY_NODE_VM_PATH: &str = "vm.json";
const API_BOUNDARY_NODE_PLAYNET_PATH: &str = "playnet.json";
...
/// A builder for the initial configuration of an IC API boundary node.
...
pub struct ApiBoundaryNodeWithVm {
    pub name: String,
...
impl ApiBoundaryNodeWithVm {
    ...
    pub fn start(&self, env: &TestEnv) -> Result<()> {
        ...
        env.write_api_boundary_node_vm(
            &self.name,
            &self.allocated_vm,
        ...
        if self.has_ipv4 {
            ...
            if let Some(mut existing_playnet) = opt_existing_playnet.clone() {
                let api_boundary_node_vm = env
                    .get_deployed_api_boundary_node(&self.name.clone())
                    .unwrap()
                    .get_snapshot()
                    .unwrap();
                let ipv4_address = api_boundary_node_vm.block_on_ipv4().unwrap().to_string();
...
impl ApiBoundaryNode {
    ...
    pub fn allocate_vm(self, env: &TestEnv) -> Result<ApiBoundaryNodeWithVm> {
        let farm = Farm::from_test_env(env, "boundary node");
        let pot_setup = GroupSetup::read_attribute(env);
        let api_boundary_node_img_url = env.get_api_boundary_node_img_url()?;
        let api_boundary_node_img_sha256 = env.get_api_boundary_node_img_sha256()?;
        ...
        let create_vm_req = CreateVmRequest::new(
            ...
            match &self.boot_image {
                None => {
                    let url = api_boundary_node_img_url;
                    let sha256 = api_boundary_node_img_sha256;
...
/// in the api boundary node directories.
fn create_and_upload_config_disk_image(
    api_boundary_node: &ApiBoundaryNodeWithVm,
    env: &TestEnv,
    group_name: &str,
    farm: &Farm,
    opt_playnet_cert: Option<PlaynetCertificate>,
) -> anyhow::Result<FileId> {
    let api_boundary_node_dir = env
        .base_path()
        .join(API_BOUNDARY_NODE_VMS_DIR)
        .join(api_boundary_node.name.clone());
    let img_path = api_boundary_node_dir.join(CONF_IMG_FNAME);
    let script_path = env
        .get_dependency_path("ic-os/boundary-api-guestos/scripts/build-bootstrap-config-image.sh");
    let mut cmd = Command::new(script_path);
    let ssh_authorized_pub_keys_dir: PathBuf = env.get_path(SSH_AUTHORIZED_PUB_KEYS_DIR);

    cmd.arg(img_path.clone())
        .arg("--hostname")
        .arg(api_boundary_node.name.clone())
        ...
        .arg(&api_boundary_node.replica_ipv6_rule)
        .arg("--ipv4_http_ips")
        .arg("0.0.0.0/0")
        .arg("--ipv6_http_ips")
        .arg("::/0")
    ...
    if !api_boundary_node.nns_node_urls.is_empty() {
        cmd.arg("--nns_url").arg({
            let urls: Vec<&str> = api_boundary_node
                .nns_node_urls
                .iter()
                .map(|url| url.as_str())
                .collect();
    ...
    if let Some(nns_public_key) = api_boundary_node.nns_public_key.clone() {
        cmd.arg("--nns_public_key").arg(nns_public_key);
    }

    if let Some(playnet_cert) = opt_playnet_cert {
        let cert_dir = api_boundary_node_dir.join(CERT_DIR);
    ...
    let compressed_img_path = api_boundary_node_dir.join(mk_compressed_img_path());
...
impl ApiBoundaryNodeVm for TestEnv {
    fn get_deployed_api_boundary_node(&self, name: &str) -> Result<DeployedApiBoundaryNode> {
        ...
        let playnet =
            self.read_json_object(rel_api_boundary_node_dir.join(API_BOUNDARY_NODE_PLAYNET_PATH))?;
        if abs_api_boundary_node_dir.is_dir() {
            Ok(DeployedApiBoundaryNode {
                env: self.clone(),
                name: name.to_string(),
                playnet,
            })
        } else {
            bail!("Did not find deployed API boundary node '{name}'!")
        }
    }
    ...
    fn write_api_boundary_node_vm(
        ...
        self.write_json_object(
            rel_api_boundary_node_dir.join(API_BOUNDARY_NODE_VM_PATH),
            &vm,
        )?;
        self.write_json_object(
            rel_api_boundary_node_dir.join(API_BOUNDARY_NODE_PLAYNET_PATH),
            &opt_playnet,
        )
    }
}

pub struct DeployedApiBoundaryNode {
    env: TestEnv,
    name: String,
    playnet: Option<String>,
}

impl HasTestEnv for DeployedApiBoundaryNode {
    fn test_env(&self) -> TestEnv {
        self.env.clone()
    }
}

impl HasVmName for DeployedApiBoundaryNode {
...
impl DeployedApiBoundaryNode {
    ...
    pub fn get_snapshot(self) -> Result<ApiBoundaryNodeSnapshot> {
        Ok(ApiBoundaryNodeSnapshot {
            vm: self.get_vm()?,
            env: self.env,
            name: self.name,
...
pub struct ApiBoundaryNodeSnapshot {
    env: TestEnv,
    name: String,
...
impl HasTestEnv for ApiBoundaryNodeSnapshot {
    fn test_env(&self) -> TestEnv {
        self.env.clone()
    }
}

impl HasVmName for ApiBoundaryNodeSnapshot {
    fn vm_name(&self) -> String {
        self.name.clone()
    }
}

impl SshSession for ApiBoundaryNodeSnapshot {
...
#[async_trait]
impl HasPublicApiUrl for ApiBoundaryNodeSnapshot {
...
impl RetrieveIpv4Addr for ApiBoundaryNodeSnapshot {
