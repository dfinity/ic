...
pub fn test(handle: IcHandle, ctx: &ic_fondue::pot::Context) {
    ...
    rt.block_on(async move {
        ...
        .await;
        transact_icp(ctx, &ledger, &can2, 750, &can3, 750).await;
        transact_icp(ctx, &ledger, &lifeline, 859 + 2 * fee, &can4, 859 + 2 * fee).await;
        transact_icp(ctx, &ledger, &lifeline, 23 + 2 * fee, &can5, 23 + 2 * fee).await;
        transact_icp(ctx, &ledger, &lifeline, 1750, &can2, 2800 + fee * 2).await;
        //transact_icp(ctx, &ledger, &can4, 1000, &lifeline, 859).await; // not
        // possible
        transact_icp(ctx, &ledger, &can2, 800, &can5, 823 + fee * 2).await;
        transact_icp(ctx, &ledger, &can5, 0, &can5, 823 + fee).await; // self, zero
        transact_icp(ctx, &ledger, &can5, 42, &can5, 823).await; // self
        transact_icp(ctx, &ledger, &can2, 0, &can5, 823).await; // zero
        transact_icp(ctx, &ledger, &lifeline, fee, &can5, 823 + fee).await; // zero

        async fn get_uc_balance(ledger: &Canister<'_>, can: &UniversalCanister<'_>) -> Tokens {
            get_icp_balance(
                ledger,
        ...
        assert_eq!(
            ...
            funds.iter().fold(Tokens::ZERO, |s, f| (s + *f).unwrap())
        );
