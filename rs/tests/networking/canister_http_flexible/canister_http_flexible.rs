//! The scenarios exercising the `flexible_http_request` management canister
//! endpoint, shared by the system tests that run them against an application
//! subnet where HTTP outcalls are free and against one where they are paid for
//! under pay-as-you-go.
//!
//! Every scenario here has to hold either way: what a caller observes must not
//! depend on whether it was charged. Scenarios that are *about* being charged
//! live in [`add_pricing_scenarios`], which only the paying test runs.
#![allow(deprecated)]
...
/// The scenarios that must hold whether or not outcalls are paid for, set up by
/// `setup` (which decides the application subnet's cost schedule).
///
/// These all run in parallel. The caller appends its own scenarios and then
/// [`add_fault_tolerance`], which has to come last because it kills a node —
/// see [`add_pricing_scenarios`].
pub fn shared_scenarios(setup: fn(TestEnv)) -> SystemTestGroup {
    SystemTestGroup::new().with_setup(setup).add_parallel(
...
/// Appends the fault-tolerance scenario, which kills a subnet node and so has to
/// be the last thing any suite runs.
pub fn add_fault_tolerance(group: SystemTestGroup) -> SystemTestGroup {
    group.add_test(systest!(test_fault_tolerance))
}
...
/// The scenarios that only exist because the outcall is paid for, and so only
/// run against an application subnet on a normal cost schedule.
///
/// These run sequentially: [`test_charged_and_refunded`] measures the proxy
/// canister's balance across a single outcall, which any concurrent outcall from
/// the same canister would perturb.
pub fn add_pricing_scenarios(group: SystemTestGroup) -> SystemTestGroup {
    group
