//! The scenarios checking non-flexible HTTP outcalls against the spec, shared by
//! the system tests that run them under each pricing model.
//!
//! Which model applies is a property of the *request* (`pricing_version`), not of
//! the subnet, so both suites run against the same setup and differ only in what
//! [`shared_scenarios`] stamps onto every outcall.
//!
//! Almost everything here is pricing-agnostic, including the refund assertions: a
//! synchronously rejected request has its payment returned intact either way, and
//! a request that got as far as being charged refunds less than the full payment
//! either way (under pay-as-you-go the payment is consumed up front and the
//! unspent allowance is credited back to the balance instead). The scenarios that
//! do depend on the model live in [`add_legacy_pricing_scenarios`] and
//! [`add_pay_as_you_go_pricing_scenarios`].
#![allow(deprecated)]
...
/// The pricing model every outcall in this suite opts into, set once by
/// [`shared_scenarios`] and stamped onto each request by [`submit_outcall`].
///
/// This is per-binary configuration — each test target picks one model and runs
/// the whole suite under it — so it is read here rather than threaded through
/// every scenario.
static PRICING_VERSION: OnceLock<u32> = OnceLock::new();

fn pricing_version() -> u32 {
    *PRICING_VERSION
        .get()
        .expect("the pricing version is set before any scenario runs")
}
...
/// The scenarios that hold under either pricing model, with every outcall opting
/// into `pricing_version`.
///
/// The caller appends the scenarios specific to the model it picked — see
/// [`add_legacy_pricing_scenarios`] and [`add_pay_as_you_go_pricing_scenarios`].
pub fn shared_scenarios(pricing_version: u32) -> SystemTestGroup {
    PRICING_VERSION
        .set(pricing_version)
        .expect("the pricing version is set exactly once");
...
/// The scenarios that pin down legacy pricing specifically: it charges for the
/// whole of `max_response_bytes` up front, so the exact payment an outcall needs
/// is a function of that limit.
pub fn add_legacy_pricing_scenarios(group: SystemTestGroup) -> SystemTestGroup {
    group.add_parallel(
        SystemTestSubGroup::new()
...
/// The scenarios that pin down pay-as-you-go pricing specifically: only a base
/// fee is charged up front, the rest of the payment becomes a per-replica
/// allowance, and what goes unspent is credited back to the caller's balance.
///
/// These run sequentially: [`test_pay_as_you_go_charges_and_refunds`] measures the
/// proxy canister's balance across a single outcall, which any concurrent outcall
/// from the same canister would perturb.
pub fn add_pay_as_you_go_pricing_scenarios(group: SystemTestGroup) -> SystemTestGroup {
    group
...
fn test_max_number_of_request_headers_exceeded(env: TestEnv) {
    ...
    let (response, refunded_cycles) = block_on(submit_encoded_outcall(
        &handlers,
...
/// Sends a non-flexible outcall through the proxy canister.
///
/// Every well-formed request goes through here, so this is where the suite's
/// pricing model is applied — the scenarios themselves say nothing about it.
async fn submit_outcall(
    handlers: &Handlers<'_>,
    mut request: RemoteHttpRequest,
) -> OutcallsResponseWithRefund {
    request.request.pricing_version = Some(pricing_version());
    submit_encoded_outcall(handlers, request).await
}

/// Sends an arbitrarily shaped payload to the proxy canister, for the scenarios
/// that deliberately send something other than a well-formed request.
///
/// Such a payload carries no pricing version of its own, so it decodes to the
/// default model rather than the suite's. That is immaterial to those scenarios,
/// which are rejected before pricing comes into play.
async fn submit_encoded_outcall<Request>(
    handlers: &Handlers<'_>,
    request: Request,
) -> OutcallsResponseWithRefund
where
...
/// Base arguments for a plain `GET` outcall. The pricing model is stamped on by
/// [`submit_outcall`], so it is deliberately left unset here.
fn plain_get_args(url: String) -> UnvalidatedCanisterHttpRequestArgs {
...
fn test_pay_as_you_go_charges_and_refunds(env: TestEnv) {
    ...
    block_on(async {
        ...
        let (response, _refunded) = submit_outcall(
            &handlers,
            RemoteHttpRequest {
                request: plain_get_args(format!("https://[{webserver_ipv6}]/ascii/priced")),
                cycles: HTTP_REQUEST_CYCLE_PAYMENT,
            },
        )
        .await;
...
fn test_pay_as_you_go_out_of_cycles(env: TestEnv) {
    ...
    let (response, _refunded) = block_on(submit_outcall(
        &handlers,
        RemoteHttpRequest {
            request: plain_get_args(format!("https://[{webserver_ipv6}]/bytes/{BODY_SIZE}")),
            cycles: PAYMENT,
        },
    ));
