...
pub async fn access_control_test(env: TestEnv) {
    ...
    for ident in [&root_ident_a, &root_ident_b] {
        ...
        match rm_allowed_principal(agent.clone(), ident.clone(), cid, principal_b).await {
            ModifyAllowedPrincipalResponse::Err(ModifyAllowedPrincipalError::UnexpectedError(
                e,
...
pub async fn registration_test(env: TestEnv) {
    let logger = env.logger();
    ...
    .await
    {
        CreateRegistrationResponse::Err(CreateRegistrationError::Duplicate(id)) => {
            if registration_a_id != id {
    ...
    match update_registration(
        ...
        cid,
        registration_a_id.clone(),
        UpdateType::State(State::PendingAcmeApproval),
    )
    .await
    {
        UpdateRegistrationResponse::Ok(()) => {}
    ...
    match update_registration(
        ...
        cid,
        registration_a_id.clone(),
        UpdateType::Canister(canister_b),
    )
    .await
    {
        UpdateRegistrationResponse::Ok(()) => {}
    ...
    match update_registration(
        ...
        cid,
        registration_b_id.clone(),
        UpdateType::Canister(canister_a),
    )
    .await
    {
        UpdateRegistrationResponse::Err(UpdateRegistrationError::Unauthorized) => {}
        v => panic!(
            "updateRegistration failed: {v:?}, expected UpdateRegistrationError::Unauthorized"
        ),
    };
    ...
    match update_registration(
        ...
        cid,
        registration_b_id.clone(),
        UpdateType::State(State::Failed(BoundedString::<127>::from("Test"))),
    )
    .await
    {
        UpdateRegistrationResponse::Ok(()) => {}
    ...
    if let RemoveRegistrationResponse::Err(err) = remove_registration(
        agent.clone(),
        ident_a.clone(),
        cid,
        registration_b_id.clone(),
    ...
    .await
    {
        ...
        RemoveRegistrationResponse::Err(err) => {
            panic!("removeRegistration failed: {err:?} expected RemoveRegistrationError::NotFound")
        }
        RemoveRegistrationResponse::Ok(()) => panic!(
            "removeRegistration failed: got Ok(), expected RemoveRegistrationError::NotFound"
        ),
    };
    ...
    .await
    {
        ...
        RemoveRegistrationResponse::Err(err) => panic!(
            "removeRegistration failed: {err:?} expected RemoveRegistrationError::Unauthorized"
        ),
        RemoveRegistrationResponse::Ok(()) => panic!(
            "removeRegistration failed: got Ok(), expected RemoveRegistrationError::Unauthorized"
        ),
    };
}
...
pub async fn expiration_test(env: TestEnv) {
    let logger = env.logger();
...
pub async fn renewal_expiration_test(env: TestEnv) {
    let logger = env.logger();
...
pub async fn task_queue_test(env: TestEnv) {
    let logger = env.logger();
...
pub async fn retry_test(env: TestEnv) {
    let logger = env.logger();
...
// * try it with and without authorization
pub async fn certificate_export_test(env: TestEnv) {
    let logger = env.logger();
    ...
    .await
    {
        ...
        v => {
            panic!("uploadCertificate failed: {v:?}, expected UploadCertificateError::Unauthorized")
        }
    };
