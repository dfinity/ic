...
for env_var in "${runtime_dep_env_vars[@]}"; do
    ...
    if [ "$old_dep_name" = "mtools" ]; then
        new_dep="$old_dep_name"
    fi
    old_dep_abs="$(realpath $old_dep)"
    ln -sf "$old_dep_abs" "$runtime_deps/$new_dep"

    # By default the env var points at where the dep lives for the process that
    # consumes it: $runtime_dep_base, i.e. the in-container path for colocated
    # tests and the local path otherwise. MKFS_VFAT/MTOOLS are an exception: the
    # colocated *wrapper* itself dereferences them on the host (UniversalVm::start
    # builds the driver VM's SSH config image locally), so they must resolve
    # locally. colocate_test.rs rewrites them to the in-container path in the
    # environment it forwards to the UVM container, where the inner test driver
    # consumes them.
    dep_base="$runtime_dep_base"
    case "$env_var" in
        MKFS_VFAT | MTOOLS) dep_base="$runtime_deps" ;;
    esac
