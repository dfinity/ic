...
pub fn main() -> anyhow::Result<()> {
    ...
    let old_version = match std::env::var("OLD_VERSION") {
        Ok(v) => v,
        Err(_) => read_dependency_to_string("mainnet_nns_revision.txt")
            .map_err(|_| anyhow::anyhow!("Didn't find initial version specified in `mainnet_nns_revision.txt` or in `OLD_VERSION` env variable"))?,
    };
