...
pub fn main() -> anyhow::Result<()> {
    ...
    SystemTestGroup::new()
        ...
        .add_test(ic_system_test_driver::driver::dsl::TestFunction::new(
            "qualification",
            move |env| {
                let qualifier = QualificationExecutor::new(vec![
                    ...
                    Box::new(EnsureElectedVersion {
                        ...
                        guest_launch_measurements: Some(get_guestos_initial_launch_measurements()),
                    }),
                ...
                rt.block_on(qualifier.qualify(env))
                    .expect("Failed to qualify");
            },
        ))
        .execute_from_args()
}
