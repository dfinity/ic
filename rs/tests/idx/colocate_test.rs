...
/// Rewrites the `MKFS_FAT`/`MCOPY` variables (the Bazel-built FAT tools) in a
...
/// dereferences them locally (building the driver VM's config image); the inner
/// test driver, running in the container, needs the in-container path. Only the
/// directory changes (the symlink basename is identical on both sides). Operates
/// on raw bytes so a non-UTF-8 entry elsewhere in the dump is preserved verbatim
