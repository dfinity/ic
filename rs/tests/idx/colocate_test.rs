...
/// below), and the deps land under runtime_deps/ there.
...
fn setup(env: TestEnv) {
    ...
    // variables are then forward to the docker container with --env-file.
    {
        ...
        // run_systest.sh leaves MKFS_VFAT/MTOOLS pointing at their host location
        // (the wrapper dereferences them locally, see UniversalVm::start). Inside
        // the UVM container the runtime deps live under CONTAINER_RUNTIME_DEPS_DIR,
        // so rewrite just those two variables for the env we forward to the
        // container's (inner) test driver.
...
/// Rewrites the `MKFS_VFAT`/`MTOOLS` variables (the Bazel-built FAT tools) in a
/// `KEY=VALUE`-per-line env dump so they point at the runtime-deps location
/// inside the UVM container instead of their host path. `run_systest.sh`
/// intentionally leaves them host-pathed because the colocate wrapper
/// dereferences them locally (building the driver VM's SSH config image); the
/// inner test driver, running in the container, needs the in-container path. The
/// symlink basename is identical on both sides, so only the directory changes.
/// Operates on raw bytes so a non-UTF-8 entry elsewhere in the dump is preserved
/// verbatim rather than corrupted.
fn rewrite_fat_tool_paths_for_container(env_dump: &[u8]) -> Vec<u8> {
    const PREFIXES: [&[u8]; 2] = [b"MKFS_VFAT=", b"MTOOLS="];
