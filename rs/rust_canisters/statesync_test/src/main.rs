...
lazy_static! {
    ...
    static ref NUM_CHANGED: Mutex<u64> = Mutex::new(0 as u64);
}
...
/// Returns the number of times it has been called.
#[update]
async fn expand_state(index: u32) -> Result<u64, String> {
    let mut num_changed = NUM_CHANGED
        .lock()
        .expect("Could not lock NUM_CHANGED mutex");
    let mut rng = MT19937::from_seed(index);
    ...
    for index in (0..VECTOR_LENGTH).step_by(1_048_576) {
        state[index] = rng.next_u32() as u8;
    }
    *num_changed += 1;
    Ok(*num_changed)
