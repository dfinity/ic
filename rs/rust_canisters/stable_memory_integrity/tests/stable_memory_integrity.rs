...
impl StableOperationType {
    ...
    fn random(rng: &mut impl Rng) -> Self {
        let val = rng.r#gen_range(0.0..1.0);
        if val < 0.05 {
            Self::Size
        } else if val < 0.2 {
            Self::Grow
...
impl proptest::strategy::Strategy for OperationsStrategy {
    ...
    fn new_tree(
        ...
        // Execute 20 messages.
        for _ in 0..20 {
            let allow_invalid = rng.r#gen_bool(0.2);
            let concentrate_ops = rng.r#gen_bool(0.5);
...
fn generate_random_ops(
    ...
    let count = rng.r#gen_range(0..100);
    ...
    match ty {
        ...
        StableOperationType::Read => {
            let length = if state.contents.is_empty() {
                ...
                rng.r#gen_range(0..4 * KB)
            };
            let start = rng.r#gen_range(
...
fn generate_invalid_operation(
    ...
    match ty {
        ...
        StableOperationType::Read => {
            let length = rng.r#gen_range(0..4 * KB);
            let start = rng.r#gen_range(
        ...
        StableOperationType::Write => {
            let write_size = rng.r#gen_range(0..4 * KB);
