...
fn test_message_roundtrip_with_payload_size(
    #[strategy(arb_nested_call(
        vec![canister_test_id(13), canister_test_id(17), canister_test_id(19)],
        0..=(2 * 1024 * 1024), // call_bytes_range
        0..=(2 * 1024 * 1024), // reply_bytes_range
        50,                    // best_effort_percentage
        10..=300,              // timeout_secs_range
        50,                    // make_calls_percentage
        20,                    // max_total_calls
        0..=5,                 // calls_count_range
    ))]
    call: Call,
) {
    ...
    if call.call_bytes > payload_size_bytes + 4 {
        ...
        assert_eq!(call.call_bytes, blob.len() as u32);
    } else {
        // For a requested size bytes below what is possible, there should be no padding.
        assert_eq!(payload_size_bytes + 4, blob.len() as u32);
    }
    ...
    assert_eq!(test_message, decoded_test_message);
    assert_eq!(blob_size_bytes as u32, blob_size_bytes_);
