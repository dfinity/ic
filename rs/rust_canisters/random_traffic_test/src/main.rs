...
use ic_cdk::{
    api,
    call::{Call, CallError, CallResult, ConfigurableCall, RejectCode, SendableCall},
    setup,
};
...
thread_local! {
    ...
    /// An error message generated by the canister trapping in the self-call to `pulse()` performed
    /// by the heartbeat; empty means no error recorded.
    static HEARTBEAT_TRAP_MSG: RefCell<String> = RefCell::default();
...
/// Returns the error messages of the last trap that occured in the call tree initiated by the
/// heartbeat; empty if there was no trap.
#[query]
fn heartbeat_trap_msg() -> String {
    HEARTBEAT_TRAP_MSG.with_borrow(|err_msg| err_msg.clone())
}
...
/// Calls `pulse(calls_per_heartbeat)` each round; If the call fails for any reason, records the
/// error message and stops doing anything.
#[heartbeat]
async fn heartbeat() {
    if !heartbeat_trap_msg().is_empty() {
        // Error recorded, do nothing.
        return;
    }
