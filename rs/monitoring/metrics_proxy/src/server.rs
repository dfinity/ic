...
enum ServerKind {
    ...
    PrometheusMetricsServer(ListenerSpec, Registry),
}

/// Encodes the metrics currently held in `registry` into the Prometheus text
/// exposition format.
fn render_metrics(registry: &Registry) -> (StatusCode, HeaderMap, Bytes) {
    ...
    match encoder.encode(&metric_families, &mut buf) {
        Ok(()) => {
            ...
            (StatusCode::OK, headers, Bytes::from(buf))
        }
        Err(err) => {
...
impl Server {
    ...
    /// collected during proxying, reading them from `registry`.
    pub fn for_service_metrics(listen_on: ListenerSpec, registry: Registry) -> Self {
        Server {
            config: ServerKind::PrometheusMetricsServer(listen_on, registry),
            metrics_collector: None,
        }
    }
    ...
    pub async fn serve(self) -> Result<(), StartError> {
        ...
        let listener = match &self.config {
            ...
            ServerKind::PrometheusMetricsServer(listen_on, _) => listen_on.clone(),
        };
        ...
        router = match self.config {
            ...
            ServerKind::PrometheusMetricsServer(listen_on, registry) => router.route(
                listen_on.handler.as_str(),
                get(move || {
