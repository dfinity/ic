// The deadline cacher intentionally holds these async locks across `.await`
// points (it coalesces concurrent fetches by keeping an entry's write lock held
// while the fetch future runs), which is exactly what `tokio::sync` locks are
// for. A blocking lock cannot be held across an await, so the disallowed-types
