...
// `PrivateKeyDer` does not implement `Clone`, so derive it manually using its
// `clone_key()` helper.
impl Clone for Protocol {
    fn clone(&self) -> Self {
        match self {
            Protocol::Http => Protocol::Http,
            Protocol::Https { certificate, key } => Protocol::Https {
    ...
    fn try_from(other: &ListenOn) -> Result<Self, Self::Error> {
        ...
        match scheme {
            ...
            "https" => {
                ...
                let certs_parsed: Vec<CertificateDer<'static>> =
                    match rustls_pemfile::certs(&mut certs_cursor).collect::<Result<Vec<_>, _>>() {
                        Ok(certs) => certs,
                        Err(err) => return Err(Self::Error::CertificateFileReadError(err)),
                    };
                ...
                let key = match rustls_pemfile::private_key(&mut key_cursor) {
                    Ok(Some(key)) => key,
                    Ok(None) => {
                        return Err(Self::Error::KeyFileReadError(std::io::Error::other(
                            format!(
...
pub struct LabelFilter {
    ...
    // serde_yaml 0.9 represents externally tagged enums with YAML tags
    // (`!reduce_time_resolution`), but metrics-proxy configs use the singleton
    // map form (`reduce_time_resolution: { ... }`). `singleton_map_recursive`
    // preserves that historical format.
