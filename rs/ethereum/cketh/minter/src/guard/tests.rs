...
mod deposit_registration_guard {
    use crate::guard::tests::init_state;
    use crate::guard::{
        GuardError, MAX_CONCURRENT, deposit_registration_guard, retrieve_withdraw_guard,
    };
    use candid::Principal;
    ...
    fn should_error_on_reentrant_principal() {
        init_state();
        let principal = principal_with_id(1);
        let _guard = deposit_registration_guard(principal).unwrap();

        assert_eq!(
            deposit_registration_guard(principal),
            Err(GuardError::AlreadyProcessing)
        );
    }
    ...
    fn should_allow_reentrant_principal_after_drop() {
        ...
        assert!(deposit_registration_guard(principal).is_ok());
    }
    ...
    fn should_allow_limited_number_of_principals() {
        ...
        let guards: Vec<_> = (0..MAX_CONCURRENT)
            .map(|i| deposit_registration_guard(principal_with_id(i as u64)).unwrap())
            .collect();

        assert_eq!(
            deposit_registration_guard(principal_with_id(MAX_CONCURRENT as u64)),
            Err(GuardError::TooManyConcurrentRequests)
        );

        drop(guards);
        assert!(deposit_registration_guard(principal_with_id(MAX_CONCURRENT as u64)).is_ok());
    }
    ...
    fn should_not_share_the_lock_with_withdrawals() {
        init_state();
        let principal = principal_with_id(1);
        let _deposit = deposit_registration_guard(principal).unwrap();
