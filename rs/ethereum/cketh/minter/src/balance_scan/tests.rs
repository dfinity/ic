...
fn should_have_a_min_deposit_for_every_deployed_supported_token() {
    ...
    // currently support. This is the only place a typo'd address or threshold gets caught. A
    // supported token missing from `MIN_DEPOSITS`, or one with a wrong threshold, would be scanned
    // but never (or wrongly) flagged, so its deposits would go undetected.
