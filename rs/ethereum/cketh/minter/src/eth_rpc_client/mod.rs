...
use evm_rpc_client::{CandidResponseConverter, ClientBuilder, EvmRpcClient, IcRuntime};
use evm_rpc_types::{
...
pub fn rpc_client(state: &State) -> EvmRpcClient<IcRuntime, CandidResponseConverter> {
    rpc_client_builder(state).build()
}

pub fn rpc_client_builder(state: &State) -> ClientBuilder<IcRuntime, CandidResponseConverter> {
