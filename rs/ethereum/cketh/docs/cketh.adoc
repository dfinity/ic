...
Call the link:https://etherscan.io/address/0x18901044688D3756C35Ed2b36D93e6a5B8e00E68#writeContract[minter helper contract] `depositEth` method of the helper smart contract (can be called from contract's Etherscan page) specifying

. The amount of ETH being deposited.
. The IC principal encoded as a `bytes32`.
. The IC ledger subaccount, use `0x` to use the default subaccount. Together with the IC principal, they uniquely identify an IC ledger account to which the amount of ckETH should be minted.
