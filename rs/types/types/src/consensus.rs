...
/// The main difference w.r.t. [`Option`] is that its [`Hash`] implementation ignores the `None`
...
pub struct BackwardsCompatibleOption<T, const SETTABLE: bool>(pub Option<T>);
...
impl<T, const SETTABLE: bool> From<Option<T>> for BackwardsCompatibleOption<T, SETTABLE> {
    fn from(value: Option<T>) -> Self {
        Self(value)
    }
}

impl<T> From<T> for BackwardsCompatibleOption<T, true> {
