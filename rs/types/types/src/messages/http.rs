...
// The suffix "slices" refers to the fact that this struct will soon contain `&[u8]` instead of `Vec<u8>`.
pub(crate) struct RawSignedSenderInfoSlices {
    pub info: Vec<u8>,
    pub signer: Vec<u8>,
    pub sig: Vec<u8>,
}
...
pub(crate) fn representation_independent_hash_call_or_query(
    ...
    sender: Vec<u8>,
    nonce: Option<&[u8]>,
    sender_info: Option<RawSignedSenderInfoSlices>,
) -> [u8; 32] {
