//! Defines [`MaliciousBehavior`] that allows to control malicious flags.
...
pub struct MaliciousBehavior {
    pub allow_malicious_behavior: bool,
    // No structs apart from 'allow_malicious_behavior' should be directly accessible
...
impl MaliciousBehavior {
    pub fn new(allow_malicious_behavior: bool) -> Self {
        MaliciousBehavior {
    ...
    pub fn maliciously_seg_fault(&self) -> bool {
        self.maliciously_seg_fault && self.allow_malicious_behavior
    }

    pub fn set_maliciously_seg_fault(self) -> Self {
        self.set_malicious_behavior(|mut s| {
    ...
    pub fn set_maliciously_corrupt_own_state_at_heights(self, height: u64) -> Self {
        self.set_malicious_behavior_to(
            |mut s, height| {
                s.malicious_flags
    ...
    fn set_malicious_behavior<F: FnOnce(Self) -> Self>(self, f: F) -> Self {
    ...
    fn set_malicious_behavior_to<T, F: FnOnce(Self, T) -> Self>(self, f: F, value: T) -> Self {
        if self.allow_malicious_behavior {
            f(self, value)
        } else {
            panic!(
