...
pub struct RewardsPerNodeProvider {
    ...
    pub computation_log: BTreeMap<PrincipalId, Vec<String>>,
}
...
pub fn calculate_rewards_v0(
    ...
    let mut computation_log: BTreeMap<PrincipalId, Vec<String>> = BTreeMap::new();
    ...
    for (key_string, node_operator) in node_operators.iter() {
        ...
        let np_log = computation_log.entry(node_provider_id).or_default();
