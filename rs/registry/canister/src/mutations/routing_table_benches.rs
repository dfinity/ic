...
#[bench(raw)]
fn get_routing_table_from_canister_ranges() -> BenchResult {
    let mut registry = Registry::new();

    setup_empty_routing_table(&mut registry);
    setup_subnets(&mut registry);
    let mut rng = ChaCha20Rng::seed_from_u64(0);
    migrate_canisters_to_subnets(&mut registry, 1000, &mut rng);
...
fn migrate_canisters(num_existing_canisters: u64, num_calls: u64) -> BenchResult {
    let mut registry = Registry::new();

    setup_empty_routing_table(&mut registry);
    setup_subnets(&mut registry);
    let mut rng = ChaCha20Rng::seed_from_u64(0);
    migrate_canisters_to_subnets(&mut registry, num_existing_canisters, &mut rng);
