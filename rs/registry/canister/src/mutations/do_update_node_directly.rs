...
use ic_types::{crypto::KeyPurpose, registry::IDKG_KEY_UPDATE_FREQUENCY_SECS, PrincipalId};
...
impl Registry {
    ...
    /// `IDKG_KEY_UPDATE_FREQUENCY_SECS` and the most recent key update in the node's subnet happened more
    /// than `IDKG_KEY_UPDATE_FREQUENCY_SECS / subnet_size` ago.
    ...
    fn do_update_node(
        ...
        let idkg_pk_key = make_node_record_key(node_id);
        self
            .get(idkg_pk_key.as_bytes(), self.latest_version())
            .ok_or_else(|| format!(
        ...
        let idkg_pk = make_crypto_node_key(node_id, KeyPurpose::IDkgMEGaEncryption);
        let previous_timestamp_set = match self.get(idkg_pk.as_bytes(), self.latest_version()) {
            Some(record) => {
                ...
                match pk.timestamp {
                    Some(last_update_timestamp) => {
                        if Duration::from_millis(last_update_timestamp)
                            + Duration::from_secs(IDKG_KEY_UPDATE_FREQUENCY_SECS)
        ...
        // 4. Disallow updating if the node is not on an ECDSA subnet or the most recent key update
        //    on the subnet is not old enough. If the node has no timestamp, skip all checks.
        match self.last_key_update_on_ecdsa_subnet(node_id) {
            // The node is on ECDSA subnet, but has no timestamp
            Some(_) if !previous_timestamp_set => {}
            // The node is on ECDSA subnet, but has a timestamp
            Some((subnet_size, Some(last_key_update_timestamp))) => {
                let key_update_frequency_on_subnet =
                    (IDKG_KEY_UPDATE_FREQUENCY_SECS as f64 / subnet_size as f64
                        * DELAY_COMPENSATION) as u64;
                if Duration::from_millis(last_key_update_timestamp)
                    + Duration::from_secs(key_update_frequency_on_subnet)
                    > duration_since_unix_epoch
                {
                    return Err("the ECDSA subnet had a key update recently".to_string());
                }
            }
            None => return Err("the node is not on an ECDSA subnet".to_string()),
            // The node is on an ECDSA subnet, but no node has a timestamp.
            _ => {}
        }

        // 5. Create and apply mutation for new record
        ...
        // 6. Check invariants before applying mutations
        self.maybe_apply_mutation_internal(mutations);

        Ok(())
    }

    // If the specified node belongs to an ECDSA subnet, return its size and the timestamp of the
    // last key update.
    fn last_key_update_on_ecdsa_subnet(&self, node_id: NodeId) -> Option<(usize, Option<u64>)> {
        let subnet_record = get_subnet_ids_from_subnet_list(self.get_subnet_list_record())
            .into_iter()
        ...
        let last_key_update_timestamp = subnet_record
            .membership
            .into_iter()
            .filter_map(|node_id| {
                let idkg_pk = make_crypto_node_key(
                    NodeId::from(PrincipalId::try_from(node_id.as_slice()).unwrap_or_default()),
                    KeyPurpose::IDkgMEGaEncryption,
                );
                self.get(idkg_pk.as_bytes(), self.latest_version())
            })
            .filter_map(|value| {
            ...
            .max();
        Some((subnet_size, last_key_update_timestamp))
    }
}
...
mod test {
    ...
    fn test_idkg_key_update_success() {
        ...
        // time passes ...
        now = now.add(Duration::from_secs(IDKG_KEY_UPDATE_FREQUENCY_SECS + 1));
        // generate new key
        let (keys, _node_id) = valid_node_keys_and_node_id();
        ...
        // subnet limit passes
        now = now.add(Duration::from_secs(
            IDKG_KEY_UPDATE_FREQUENCY_SECS / node_ids.len() as u64,
        ));
