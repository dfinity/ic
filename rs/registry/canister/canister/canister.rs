...
use registry_canister::{
    ...
    registry::{EncodedTimestampNanos, Registry, MAX_REGISTRY_DELTAS_SIZE},
    registry_lifecycle,
};
...
fn get_certified_changes_since() {
    over(
        ...
        |req: RegistryGetChangesSinceRequest| -> CertifiedResponse {
            ...
            let from_version = EncodedTimestampNanos::from(req.version.saturating_add(1));
            ...
            let to_version =
                EncodedTimestampNanos::from(req.version.saturating_add(max_versions as u64));
