...
use ic_registry_keys::{
    ...
    make_crypto_threshold_signing_pubkey_key, make_node_record_key, make_subnet_list_record_key,
...
fn test_accepted_proposal_with_ecdsa_gets_keys_from_other_subnet() {
    ...
    canister_test_with_config_async(
        ...
        |local_runtime| async move {
            ...
            let (init_mutate, mut node_ids) = prepare_registry(5);

            let key_1_nodes = vec![node_ids.pop().unwrap()];
            let key_2_nodes = vec![node_ids.pop().unwrap()];

            let subnet_holding_key_1 = get_subnet_holding_ecdsa_key(&key_1, key_1_nodes.clone());
            let subnet_holding_key_2 = get_subnet_holding_ecdsa_key(&key_2, key_2_nodes.clone());
            ...
            let payload = {
                ...
                payload.ecdsa_config = Some(EcdsaInitialConfig {
                    ...
                    keys: vec![
                        EcdsaKeyRequest {
                            key_id: key_1,
                            subnet_id: Some(key_1_subnet_id.get()),
                        },
                        EcdsaKeyRequest {
                            key_id: key_2,
                            subnet_id: None,
...
pub fn get_subnet_holding_ecdsa_key(
