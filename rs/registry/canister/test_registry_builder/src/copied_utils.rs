...
// These functions were copied from the registry canister
// They can't be imported as it would be a dependency cycle.
//
// Making a separate third crate also doesn't work because this depends on `ic_nns_test_utils`
// which depends on the `registry-canister` so it would create a cycle again.

pub fn prepare_mutations_with_nodes_and_operator_id(
