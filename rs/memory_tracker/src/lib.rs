...
// Number of OS pages mapped from the checkpoint file during each signal handler
// invocation. This matches the Wasm page size: 64KiB / 4KiB = 16 pages.
const OS_PAGES_TO_MAP: u64 = 16;
...
impl PageBitmap {
    ...
    fn mark(&mut self, page: PageIndex) {
        debug_assert_eq!(
            ...
            Some(false),
            "Error marking a non-existent (None) or already marked (Some(true)) page {page:?}"
        );
...
pub fn sigsegv_fault_handler_new(
    ...
    match (access_kind, tracker.dirty_page_tracking) {
        (_, DirtyPageTracking::Ignore) => {
            let faulting_range = range_from_faulting_page(faulting_page);
            let prefetch_range = map_unaccessed_pages(
                tracker,
                ProtFlags::PROT_READ | ProtFlags::PROT_WRITE,
                faulting_range.clone(),
                faulting_range.clone(),
            );
        ...
        (AccessKind::Write, DirtyPageTracking::Track) => {
            ...
            if accessed_bitmap.is_marked(faulting_page) {
                tracker
                    ...
                    .fetch_add(1, Ordering::Relaxed);
                let page_start_addr = tracker.page_start_addr_from(faulting_range.start);
                unsafe {
                    mprotect(
                        page_start_addr,
                        range_size_in_bytes(&faulting_range),
                        ProtFlags::PROT_READ | ProtFlags::PROT_WRITE,
                    )
                    .map_err(print_enomem_help)
                    .unwrap()
                ...
                tracker
                    ...
                    .mprotect_count
                    .fetch_add(1, Ordering::Relaxed);
                dirty_bitmap.mark_range(&faulting_range);
                tracker.add_dirty_pages(faulting_page, faulting_range);
            } else {
                tracker
                    .metrics
                    .direct_write_count
...
fn range_from_faulting_page(page: PageIndex) -> Range<PageIndex> {
    let floor = page.get() / OS_PAGES_TO_MAP * OS_PAGES_TO_MAP;
    PageIndex::new(floor)..PageIndex::new(floor + OS_PAGES_TO_MAP)
}
