...
use crate::idkg::metrics::{timed_call, IDkgPreSignerMetrics};
...
impl IDkgPreSignerImpl {
    ...
    fn build_transcripts(
        ...
        inputs
            ...
            .flat_map(|(_, transcript_state)| {
                ...
                let transcript_params = match transcript_state.params_ref.translate(block_reader) {
    ...
    fn purge_artifacts(
        ...
        let mut action = idkg_pool
            ...
            .map(|&id| IDkgChangeAction::RemoveTranscript(id))
            .collect();
        ret.append(&mut action);
        ...
        ic_test_utilities::artifact_pool_config::with_test_pool_config(|pool_config| {
            with_test_replica_logger(|logger| {
...

                {
                    ...
                    // be completed
                    let result = pre_signer.build_transcripts(&idkg_pool, &block_reader);
                    assert!(result.is_empty());
                }

                // add dealings
                let change_set_dealings: Vec<_> = dealings
                    .values()
                    .map(|d| IDkgChangeAction::AddToValidated(IDkgMessage::Dealing(d.clone())))
                    .collect();
                idkg_pool.apply(change_set_dealings.clone());

                {
                    // cannot aggregate empty shares
                    let result = pre_signer.crypto_aggregate_dealing_support(
                        &params,
                        &[],
                        idkg_pool.stats(),
                    );
                ...
                // add support
                let change_set_supports: Vec<_> = supports
                    .iter()
                    .map(|s| {
                        IDkgChangeAction::AddToValidated(IDkgMessage::DealingSupport(s.clone()))
                    })
                    .collect();
                idkg_pool.apply(change_set_supports.clone());

                // the transcript should be completed now
                let result = pre_signer.build_transcripts(&idkg_pool, &block_reader);
                assert_matches!(&result[..], [IDkgChangeAction::AddTranscript(t)] if t.transcript_id == tid);
                ...
                idkg_pool.apply(change_set_dealings);
                idkg_pool.apply(change_set_supports);
