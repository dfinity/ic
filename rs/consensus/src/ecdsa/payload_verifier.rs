...
fn validate_summary_payload(
    ...
    match create_summary_payload(
        ...
        Err(err) => Err(InvalidEcdsaPayloadReason::SummaryPayloadRecreationFailed(err).into()),
    }
}
...
fn validate_data_payload(
    ...
    if parent_block.payload.as_ref().as_ecdsa().is_none() {
        if data_payload.is_some() {
            return Err(InvalidEcdsaPayloadReason::UnexpectedDataPayload.into());
        } else {
            return Ok(());
        }
    }
    ...
    .map_err(InvalidEcdsaPayloadReason::DataPayloadRecreationFailed)?;
