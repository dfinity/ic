...
- The argument of the endpoint `/instances/` takes an additional optional field `allow_corrupted_state` specifying if corrupted state (e.g., resulting from not deleting a PocketIC instance gracefully) is allowed.
