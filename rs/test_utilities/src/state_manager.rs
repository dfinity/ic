...
pub struct FakeStateManager {
    ...
    /// When `Some(cap)`, `latest_certified_height()` returns `min(real, cap)`.
    /// Used in tests to simulate a slow checkpoint holding certification back.
    pub override_max_certified_height: Arc<RwLock<Option<Height>>>,
}
...
impl FakeStateManager {
    pub fn new() -> Self {
        ...
        Self {
            ...
            override_max_certified_height: Arc::new(RwLock::new(None)),
        }
    }
...
impl StateReader for FakeStateManager {
    ...
    fn latest_certified_height(&self) -> Height {
        ...
        match *self.override_max_certified_height.read().unwrap() {
