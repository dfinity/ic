//! This is a special-purpose canister to test memory migration
//! of the governance canister.  It will be cleaned up after the deployment is successful
//!
//! TODO NNS1-2357 Remove this file and associated test after deploy
...
const LOG_PREFIX: &str = "[Governance mem migration test] ";
...
const TEST_NEURON_HOTKEY_VEC_LEN: usize = 1;
...
fn canister_pre_upgrade() {
    let mut writer = BufferedStableMemWriter::new(10000);
    unsafe {
        GOVERNANCE
            .as_ref()
            .unwrap()
...
fn populate_canister_state() {
    ...
    for _ in num_settled_proposals..num_settled_proposals + TEST_NUM_NOT_YET_SETTLED_PROPOSALS {
        ...
        proto.proposals.insert(
            proposal_id,
            allocate_proposal_data(true, *topic_iter.next().unwrap(), proposal_id),
        );
        proposal_id += 1;
    }
...
fn allocate_neuron(id: u64) -> Neuron {
    Neuron {
        ...
        recent_ballots: vec![
            BallotInfo {
                proposal_id: Some(ProposalIdProto { id: 0 }),
                vote: 0,
            };
...
fn allocate_proposal_data(with_ballots: bool, topic: Topic, proposal_id: u64) -> ProposalData {
    ...
    ProposalData {
        id: Some(ProposalIdProto { id: proposal_id }),
