...
#[test]
fn test_proposal_early_adoption() {
...
#[test]
fn test_proposal_adoption_by_expiry() {
    let state_machine = state_machine_builder_for_nns_tests().build();
    let nns_init_payloads = NnsInitPayloadsBuilder::new().with_test_neurons().build();
    setup_nns_canisters(&state_machine, nns_init_payloads);
    ...
    let proposal_id = make_motion_proposal(
        ...
        *TEST_NEURON_2_OWNER_PRINCIPAL,
        NeuronId::from_u64(TEST_NEURON_2_ID),
    );

    // Because the neuron 2 has very little voting power, the proposal should not be executed.
    assert!(!is_proposal_executed(&state_machine, proposal_id));

    // Fast forward time to make the proposal expire, and tick a few times to give the proposal a
    // chance to be executed.
    state_machine.advance_time(Duration::from_secs(60 * 60 * 24 * 8));
    for _ in 0..10 {
        state_machine.tick();
    }
...
#[test]
fn test_proposal_no_early_adoption_with_voting_power_spike() {
    let state_machine = state_machine_builder_for_nns_tests().build();
    let nns_init_payloads = NnsInitPayloadsBuilder::new().with_test_neurons().build();
    setup_nns_canisters(&state_machine, nns_init_payloads);
    ...
    // However, after the expiry, the proposal should be executed.
    state_machine.advance_time(Duration::from_secs(60 * 60 * 24 * 8));
    for _ in 0..10 {
        state_machine.tick();
    }
