...
async fn advance_pocket_ic_time(pocket_ic: &PocketIc, days: u64) {
    for _ in 0..days {
        for _ in 0..DAY_HOURS {
            pocket_ic.advance_time(std::time::Duration::from_secs(BLOCK_INTERVAL_SECONDS)).await;
...
async fn test_node_metrics_collector() {
    ...
    let canister_id = pocket_ic.create_canister_on_subnet(None, None, nns_subnet).await;
    ...
    assert_eq!(blocks_proposed_day_1 - blocks_proposed_day_0, DAY_HOURS as u64);
}
