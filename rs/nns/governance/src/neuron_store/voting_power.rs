...
use crate::{
    neuron::Neuron,
    pb::v1::{Ballot, VotingPowerEconomics, VotingPowerMap, VotingPowerTotal},
...
impl From<VotingPowerSnapshot> for (VotingPowerMap, VotingPowerTotal) {
    fn from(snapshot: VotingPowerSnapshot) -> Self {
        ...
        (
            VotingPowerMap { voting_power_map },
            VotingPowerTotal {
                total_deciding_voting_power,
                total_potential_voting_power,
            },
...
impl From<(VotingPowerMap, VotingPowerTotal)> for VotingPowerSnapshot {
    fn from((voting_power_map, voting_power_total): (VotingPowerMap, VotingPowerTotal)) -> Self {
        let VotingPowerMap { voting_power_map } = voting_power_map;
