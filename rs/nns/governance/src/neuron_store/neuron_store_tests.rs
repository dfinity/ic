...
#[test]
fn test_remove_deprecated_topic() {
    // Step 1.1: initialize a neuron store where some neurons follow on the deprecated topic.
    let initial_neurons = btreemap! {
        // active
        1 => Neuron {
            cached_neuron_stake_e8s: 1,
            followees: hashmap! {
        ...
        2 =>  Neuron {
            ...
            followees: hashmap! {
                ...
                // Deprecated topic.
                Topic::SnsDecentralizationSale as i32 => Followees {
                    followees: vec![NeuronId { id: 1 }, NeuronId { id: 3 }],
                }
            },
            ..simple_neuron(2)
        }
    };
    let mut neuron_store = NeuronStore::new(initial_neurons);

    // Step 1.2: make sure the test data is set up correctly by reading them.
    neuron_store
    ...
    assert_eq!(
        neuron_store.get_followers_by_followee_and_topic(NeuronId { id: 1 }, Topic::Governance),
        vec![NeuronId { id: 2 }]
    );
    assert_eq!(
        neuron_store.get_followers_by_followee_and_topic(
    ...
    let heap_neurons = neuron_store.take_heap_neurons();
    let heap_topic_followee_index = neuron_store.take_heap_topic_followee_index();
    let neuron_store = NeuronStore::new_restored(heap_neurons, heap_topic_followee_index);

    // Step 3: read the data in the same way as step 1.2 and verify that all following with the
    // deprecated topic is gone and all following with the normal topic is kept.
    neuron_store
        .with_neuron(&NeuronId { id: 1 }, |neuron| {
            assert_eq!(
                neuron.followees,
