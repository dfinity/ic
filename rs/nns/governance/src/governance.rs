...
impl Governance {
    ...
    /// Adjust the ages of neurons whose age was lost because of ICSUP-2627.
    /// This just goes through the list of adjustments, and for each:
    /// - Makes sure that the neuron exist.
    /// - Makes sure that the age is the expected one (to account for other operations
    ///   having been done to the neuron).
    /// - Substracts the provided amount from the initial timestamp dictating from
    ///   when the neuron has been aging.
    pub fn icsup_2627_adjust_neurons_age(&mut self) {
        // Filled out with the result of the algorithm outlined in the proposal that includes
        // this code.
        // The format is: [(neuron_to_adjust, current_timestamp, source_stake, lost_age, voting_power_increase, lost_maturity); num_neurons_to_fix]
        let adjustments: Vec<(u64, u64, u128, u128, u64, u64)> = vec![
            (
                1437542781357538246,
                1640221162,
                19895000000,
        ...
        let mut lot_of_5_voting_power_before = 0;
        ...
        for neuron_id in [
            ...
            let neuron = self.get_neuron(&NeuronId { id: neuron_id });
            match neuron {
                Ok(neuron) => lot_of_5_voting_power_before += neuron.voting_power(now_seconds),
                Err(_) => {
                    println!(
                        "{}ICSUP-2627 Neuron to adjust age not found. Skipping adjusting ages: {:?}",
        ...
        for (
            neuron_id,
            expected_age_since_timestamp_seconds,
            source_stake,
            lost_age,
        ...
        ) in adjustments
        {
            ...
            let neuron = self.get_neuron_mut(&neuron_id).unwrap_or_else(|_| {
                panic!(
                    "{}ICSUP-2627 Neuron to adjust age not found: {:?}",
                    LOG_PREFIX, neuron_id
                );
            });

            if neuron.aging_since_timestamp_seconds != expected_age_since_timestamp_seconds
                && (expected_age_since_timestamp_seconds
                    < neuron.aging_since_timestamp_seconds - 10
                    || expected_age_since_timestamp_seconds
                        > neuron.aging_since_timestamp_seconds + 10)
            {
                println!(
                    "{}ICSUP-2627 Neuron to adjust age didn't have the expected age +- 10 secs: Neuron {:?}, Expected: {}, Found: {}",
                    LOG_PREFIX, neuron_id, expected_age_since_timestamp_seconds, neuron.aging_since_timestamp_seconds
                );
                continue;
            }

            let adjust: u64 =
                ((source_stake * lost_age) / neuron.cached_neuron_stake_e8s as u128) as u64;

            let previous_voting_power = neuron.voting_power(now_seconds);

            neuron.aging_since_timestamp_seconds =
                now_seconds.saturating_sub(neuron.age_seconds(now_seconds).saturating_add(adjust));

            // Do not verify the voting power increase for these neurons,
            // because the age was spread across five neurons making it
            // difficult for this code to calculate. The assessment of overall
            // correctness is done in the test.
            if neuron_id.id != 6907521781164272364
                && neuron_id.id != 13082962167835576418
                && neuron_id.id != 1282604741966449439
                && neuron_id.id != 4885915525555188282
                && neuron_id.id != 4093399714311145480
            {
                let fixed_voting_power = neuron.voting_power(now_seconds);

                let diff: i64 = fixed_voting_power as i64 - previous_voting_power as i64;
                let diff_diff: i64 = i64::abs(voting_power_increase as i64 - diff);

                if diff_diff >= 10_000 {
                    println!(
                    "{}ICSUP-2627 Voting power adjustment was out of range: Neuron {:?}, Combined Stake: {}, Source Stake: {}, Previous: {}, Fixed: {}, Diff: {}, Increase: {}, Diff of Diff: {}",
                    LOG_PREFIX, neuron_id, neuron.cached_neuron_stake_e8s, source_stake, previous_voting_power, fixed_voting_power, diff, voting_power_increase, diff_diff
                );
                    continue;
                }
            }

            neuron.maturity_e8s_equivalent += lost_maturity;
        }

        let mut lot_of_5_voting_power_after = 0;
        for neuron_id in [
            13082962167835576418,
            1282604741966449439,
            4885915525555188282,
            ...
            let neuron = self
                .get_neuron(&NeuronId { id: neuron_id })
                .unwrap_or_else(|_| {
                    panic!(
                        "{}ICSUP-2627 Neuron to adjust age not found: {:?}",
                        LOG_PREFIX, neuron_id
            ...
            lot_of_5_voting_power_after += neuron.voting_power(now_seconds);
        }

        // This value comes from the adjustments.rs file, namely the expected
        // voting power increase for these five neurons in total.
        assert!(
            (lot_of_5_voting_power_after - lot_of_5_voting_power_before) as i64 - 361934893725129
                < 10_000
        );
    }
    ...
    pub async fn spawn_neuron(
        ...
        let to_subaccount = match spawn.nonce {
            ...
            Some(nonce_val) => compute_neuron_staking_subaccount(*child_controller, nonce_val),
        };
