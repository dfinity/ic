...
use std::{cell::RefCell, collections::HashMap, fmt::Display, thread::LocalKey};
...
pub fn next_maturity_disbursement_to_finalize(
    neuron_store: &NeuronStore,
    ...
    let maturity_disbursement_in_progress = neuron_store
        ...
        .map_err(|_| FinalizeMaturityDisbursementError::NeuronNotFound(neuron_id.clone()))?
        .ok_or_else(|| FinalizeMaturityDisbursementError::NoMaturityDisbursement(neuron_id))?;
    ...
    let maturity_to_disburse_after_modulation_e8s =
        apply_maturity_modulation(amount_e8s, maturity_modulation_basis_points).map_err(
            |reason| FinalizeMaturityDisbursementError::MaturityModulationFailure {
                maturity_before_modulation_e8s: amount_e8s,
                maturity_modulation_basis_points,
                reason,
            },
        )?;

    let account = account_to_disburse_to
        .ok_or_else(|| FinalizeMaturityDisbursementError::NoAccountToDisburseTo(neuron_id))?;
...
pub async fn finalize_maturity_disbursement(
    ...
    maturity_disbursement_finalization: MaturityDisbursementFinalization,
    now_seconds: u64,
) {
    let MaturityDisbursementFinalization {
        neuron_id,
        account,
        amount_e8s,
    ...
    let Ok(mut neuron_lock) = Governance::acquire_neuron_async_lock(
        ...
        ic_cdk::eprintln!(
            "Failed to acquire neuron lock for id {:?} even though we just checked \
            the neuron is not locked",
            neuron_id
        );
        return;
    };
    ...
    // for consistency.
    ic_cdk::eprintln!(
        "Minting failed: {:?}. Reversing neuron mutation.",
        mint_error
    );
    ...
    ic_cdk::eprintln!(
        "Failed to reverse neuron mutation: {:?}. Retaining neuron lock.",
        reverse_neuron_result
    );
    neuron_lock.retain();
}
