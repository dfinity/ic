...
pub fn initialize_governance(
    ...
    let heap_governance_data = HeapGovernanceData {
        ...
        // Default to a neutral 0 permyriad so that callers of
        // `get_maturity_modulation` get a usable value immediately after init,
        // before `update_icp_xdr_rate_related_data` accumulates enough price
        // history to compute a real one. `updated_at_days_since_epoch` is left
        // `None` so the task treats this as "no prior measurement" rather than
