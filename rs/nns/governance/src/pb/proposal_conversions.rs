...
fn convert_install_code(item: &pb::InstallCode) -> pb_api::InstallCode {
    ...
    // Convert (potentially) large fields.
    let wasm_module_hash = wasm_module
        .as_ref()
        .map(|wasm_module| calculate_hash(&wasm_module).to_vec());
    let arg_hash = match arg {
        Some(arg) => {
...
fn convert_ballots(
    ...
    for neuron_id in caller_neurons.iter() {
        if let Some(v) = all_ballots.get(&neuron_id.id) {
            ballots.insert(neuron_id.id, v.clone().into());
        }
    }
    ballots
}
...
mod tests {
    ...
    fn install_code_internal_to_api() {
        ...
        for (internal, api) in test_cases {
            assert_eq!(convert_install_code(&internal), api);
        }
    }
}
