...
impl FulfillSubnetRentalRequest {
    ...
    async fn create_subnet(&self, env: &Arc<dyn Environment>) -> Result<SubnetId, GovernanceError> {
        ...
        let create_subnet_payload = Encode!(&CreateSubnetPayload {
            ...
            // TODO(NNS1-3931): Make this dynamic, because one day, this
            // will become obsolete. Perhaps, this should be decided at
            // proposal creation time, or maybe the proposer should get to
            // decide this. Seems like an extra burden that we wouldn't want
            // to impose on proposers though. OTOH, they might want such
            // control. OTOOH, maybe they shouldn't have such control.
