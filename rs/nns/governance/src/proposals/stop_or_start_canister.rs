...
impl LocallyDescribableProposalAction for StopOrStartCanister {
    ...
    fn to_self_describing_value(&self) -> SelfDescribingValue {
        ...
        let action = match action.and_then(|action| CanisterAction::try_from(action).ok()) {
            Some(CanisterAction::Stop) => Some("Stop".to_string()),
            Some(CanisterAction::Start) => Some("Start".to_string()),
            None | Some(CanisterAction::Unspecified) => None,
        };
