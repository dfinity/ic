...
impl LocallyDescribableProposalAction for InstallCode {
    ...
    fn to_self_describing_value(&self) -> SelfDescribingValue {
        ...
        let install_mode = match install_mode
            .and_then(|install_mode| CanisterInstallMode::try_from(install_mode).ok())
        {
            Some(CanisterInstallMode::Install) => Some("Install".to_string()),
            Some(CanisterInstallMode::Reinstall) => Some("Reinstall".to_string()),
            Some(CanisterInstallMode::Upgrade) => Some("Upgrade".to_string()),
            None | Some(CanisterInstallMode::Unspecified) => None,
        };
