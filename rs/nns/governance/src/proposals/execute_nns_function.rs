use crate::{
    ...
    proposals::candid_to_self_describing::candid_to_self_describing,
};
...
impl ValidExecuteNnsFunction {
    ...
    async fn to_self_describing_value(
        &self,
        ...
        candid_to_self_describing(&candid_source, method_name, &self.payload)
    }
    ...
    pub async fn to_self_describing_action(
        ...
        let value = self.to_self_describing_value(env).await.map_err(|e| {
...
impl ValidNnsFunction {
    ...
    pub(crate) fn type_description(&self) -> &'static str {
        match self {
            ...
            ValidNnsFunction::AssignNoid => {
                "Assign an identity to a node operator associating key information regarding its ownership, the jurisdiction in which it is located, and other information. The node operator is stored as a record in the registry. It contains the remaining node allowance for that node operator, that is the number of nodes the node operator can still add to the IC. When an additional node is added by the node operator, the remaining allowance is decreased."
            }
            ValidNnsFunction::DeployGuestosToAllSubnetNodes => {
                "Deploy a GuestOS version to a given subnet. The proposal changes the GuestOS version that is used on the specified subnet.<br/><br/>The version must be contained in the list of elected GuestOS versions.<br/><br/>The upgrade is completed when the subnet creates the next regular CUP."
            }
            ValidNnsFunction::ClearProvisionalWhitelist => {
                "Clears the provisional whitelist, which allows the listed principals to create Canisters with cycles. The mechanism is only needed for bootstrap and testing and must be deactivated afterward."
            }
            ValidNnsFunction::RemoveNodesFromSubnet => {
                "Remove a node from a subnet. It then becomes available for reassignment. The execution of this proposal changes an existing subnet record to remove a node. From the perspective of the NNS, this update is a simple update of the subnet record in the registry."
            }
            ValidNnsFunction::SetAuthorizedSubnetworks => {
            ...
            ValidNnsFunction::SetFirewallConfig => {
                "Change the Firewall configuration in the registry (configures which boundary nodes subnet blockchain replicas will communicate with)."
            }
            ValidNnsFunction::UpdateNodeOperatorConfig => {
            ...
            ValidNnsFunction::RerouteCanisterRanges => {
                "In the routing table in the registry, remap canister ID ranges from one subnet to a different subnet.<br/><br/>The steps of canister migration are:<ol><li>Prepare Canister Migration</li><li>Reroute Canister Ranges</li><li>Complete Canister Migration</li></ol>"
            }
            ValidNnsFunction::AddFirewallRules => {
                "Add firewall rules in the registry. Nodes use a firewall to protect themselves from network attacks."
            }
            ValidNnsFunction::RemoveFirewallRules => {
                "Remove firewall rules in the registry. Nodes use a firewall to protect themselves from network attacks."
            }
            ValidNnsFunction::UpdateFirewallRules => {
                "Update firewall rules in the registry. Nodes use a firewall to protect themselves from network attacks."
            }
            ValidNnsFunction::PrepareCanisterMigration => {
                "Insert or update canister migrations entries. Such entries specify that a migration of canister ID ranges is currently ongoing.<br/><br/>The steps of canister migration are:<ol><li>Prepare Canister Migration</li><li>Reroute Canister Ranges</li><li>Complete Canister Migration</li></ol>"
            }
            ValidNnsFunction::CompleteCanisterMigration => {
                "Remove canister migrations entries. Such entries specify that a migration of canister ID ranges is currently ongoing.<br/><br/>The steps of canister migration are:<ol><li>Prepare Canister Migration</li><li>Reroute Canister Ranges</li><li>Complete Canister Migration</li></ol>"
            }
            ...
            ValidNnsFunction::UpdateSubnetType => {
                "Add or remove a subnet type. A new subnet type can be added if it doesn't already exist. An existing subnet type can be removed if no subnets are assigned to it. Subnet types can be used to choose the kind of subnet a canister should be created on."
            }
            ValidNnsFunction::ChangeSubnetTypeAssignment => {
            ...
            ValidNnsFunction::UpdateSnsWasmSnsSubnetIds => {
                "Update the list of SNS subnet IDs that SNS-W will deploy SNS instances to."
            }
            ValidNnsFunction::InsertSnsWasmUpgradePathEntries => {
                "Insert custom upgrade path entries into SNS-W for all SNSs, or for an SNS specified by its governance canister ID."
            }
            ValidNnsFunction::ReviseElectedGuestosVersions => {
                "A proposal to change the set of elected GuestOS versions.<br/><br/>The version to elect (identified by the hash of the installation image) is added to the registry. Besides creating a record for that version, the proposal also appends that version to the list of elected versions that can be installed on nodes of a subnet.<br/><br/>Only elected GuestOS versions can be deployed."
            }
            ValidNnsFunction::BitcoinSetConfig => {
                "A proposal to set the configuration of the underlying Bitcoin Canister that powers the Bitcoin API. The configuration includes whether or not the Bitcoin Canister should sync new blocks from the network, whether the API is enabled, the fees to charge, etc."
            }
            ValidNnsFunction::HardResetNnsRootToVersion => {
                "A proposal to hard reset the NNS root canister to a specific version. This is an emergency recovery mechanism that should only be used when the NNS root canister is in an unrecoverable state."
            }
            ValidNnsFunction::AddApiBoundaryNodes => {
            ...
            ValidNnsFunction::ReviseElectedHostosVersions => {
                "A proposal to change the set of currently elected HostOS versions, by electing a new version, and/or unelecting some priorly elected versions.<br/><br/>HostOS versions are identified by the hash of the installation image.<br/><br/>The version to elect is added to the Registry, and the versions to unelect are removed from the Registry, ensuring that HostOS cannot upgrade to these versions anymore.<br/><br/>This proposal does not actually perform the upgrade; for deployment of an elected version, please refer to \"Deploy HostOS To Some Nodes\"."
            }
            ValidNnsFunction::DeployHostosToSomeNodes => {
            ...
            ValidNnsFunction::SubnetRentalRequest => {
                "A proposal to rent a subnet on the Internet Computer.<br/><br/>The Subnet Rental Canister is called when this proposal is executed, and the rental request is stored there. The user specified in the proposal needs to make a sufficient upfront payment in ICP in order for the proposal to be valid, and the subnet must be available for rent. The available rental conditions can be checked by calling the Subnet Rental Canister."
            }
            ValidNnsFunction::PauseCanisterMigrations => {
                "A proposal to instruct the migration canister to not accept any more migration requests."
            }
            ValidNnsFunction::UnpauseCanisterMigrations => {
                "A proposal to instruct the migration canister to accept migration requests again."
            }
            ValidNnsFunction::SetSubnetOperationalLevel => {
                "A proposal to set the operational level of a subnet, which can be used to take a subnet offline or bring it back online as part of subnet recovery."
            }
        }
    }
}
