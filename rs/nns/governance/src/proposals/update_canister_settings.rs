use crate::{
    ...
    proposals::{
        ...
        self_describing::{LocallyDescribableProposalAction, ValueBuilder},
...
impl From<CanisterSettings> for SelfDescribingValue {
    fn from(settings: CanisterSettings) -> Self {
        ...
        let log_visibility = match log_visibility
            .and_then(|log_visibility| LogVisibility::try_from(log_visibility).ok())
        {
            Some(LogVisibility::Controllers) => Some("Controllers".to_string()),
            Some(LogVisibility::Public) => Some("Public".to_string()),
            None | Some(LogVisibility::Unspecified) => None,
        };
