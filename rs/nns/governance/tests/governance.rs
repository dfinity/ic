...
fn test_list_neurons() {
    ...
    let principal_with_no_neurons = principal(1001);
    ...
    assert!(
        list_neurons_response.neuron_infos.is_empty(),
        "{:?}",
        list_neurons_response.neuron_infos
    );
    assert!(
        list_neurons_response.full_neurons.is_empty(),
        "{:?}",
