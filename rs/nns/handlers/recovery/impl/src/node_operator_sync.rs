...
pub async fn sync_node_operators() -> Result<(), String> {
    ...
    let new_simple_records: Vec<_> = new_simple_records
        .into_iter()
        .map(|(operator_id, nodes)| SimpleNodeOperatorRecord { operator_id, nodes })
        .collect();
    print_with_prefix(format!(
        "Updating node operators in nns with: {}",
        format_node_operators(&new_simple_records)
    ));
