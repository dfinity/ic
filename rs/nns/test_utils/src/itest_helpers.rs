...
impl NnsCanisters<'_> {
    ...
    pub async fn set_up(runtime: &'_ Runtime, init_payloads: NnsInitPayloads) -> NnsCanisters<'_> {
        ...
        let maybe_canisters: Result<Vec<Canister<'_>>, String> = join_all(
            (0..NUM_NNS_CANISTERS).map(|_| runtime.create_canister_max_cycles_with_retries()),  // DO NOT MERGE - I don't think max cycles works for subnet rental canister, because it needs to be able to hold (and burn) cycles...
        )
        .await
        .into_iter()
        .collect();
