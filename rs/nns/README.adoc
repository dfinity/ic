...
The diagram below shows how NNS backend canisters talk to one another,
giving an overview of the system.

image::docs/canister_call_graph.svg[NNS backend canisters: inter-canister call graph,width=900]

To regenerate `rs/nns/docs/canister_call_graph.svg` after editing `rs/nns/docs/canister_call_graph.dot`, run:
[code,shell]
----
dot -Tsvg rs/nns/docs/canister_call_graph.dot -o rs/nns/docs/canister_call_graph.svg
----
...
To get `dfx`, see https://internetcomputer.org/docs/current/developer-docs/getting-started/install/
...
Check elected versions:
[code,shell]
----
cargo run --bin ic-admin -- --nns-url=http://localhost:8080 get-elected-replica-versions
----
