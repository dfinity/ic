        ...
        Ok(Self {
            ...
            subnet_assigned_failure_rate: Some(
                src.subnet_assigned_failure_rate
                    .to_f64()
                    .ok_or("Conversion to f64 failed (subnet_assigned_failure_rate)")?,
            ),
            ...
            original_failure_rate: Some(
                src.original_failure_rate
                    .to_f64()
                    .ok_or("Conversion to f64 failed (original_failure_rate)")?,
            ),
            relative_failure_rate: Some(
                src.relative_failure_rate
                    .to_f64()
                    .ok_or("Conversion to f64 failed (relative_failure_rate)")?,
            ),
        })
    }
}
        ...
        match src {
            ...
            native_types::DailyNodeFailureRate::NonSubnetMember {
                ...
                extrapolated_failure_rate: Some(
                    extrapolated_failure_rate
                        .to_f64()
                        .ok_or("Conversion to f64 failed (extrapolated_failure_rate)")?,
                ),
            }),
        }
    }
        ...
        Ok(Self {
            ...
            performance_multiplier: Some(
                src.performance_multiplier
                    .to_f64()
                    .ok_or("Conversion to f64 failed (performance_multiplier)")?,
            ),
            rewards_reduction: Some(
                src.rewards_reduction
                    .to_f64()
                    .ok_or("Conversion to f64 failed (rewards_reduction)")?,
            ),
    ...
    fn try_from(src: native_types::Type3RegionBaseRewards) -> Result<Self, Self::Error> {
        Ok(Self {
            ...
            avg_coefficient: Some(
                src.avg_coefficient
                    .to_f64()
                    .ok_or("Conversion to f64 failed (avg_coefficient)")?,
            ),
            daily_xdr_permyriad: Some(
                src.daily_xdr_permyriad
                    .to_u64()
        ...
        Ok(Self {
            subnets_failure_rate: src
                ...
                .map(|(k, v)| {
                    v.to_f64()
                        .map(|v| (k, v))
                        .ok_or("Conversion to f64 failed (subnets_failure_rate)")
                })
                .collect::<Result<_, _>>()?,
