...
Provides `run_as_nobody_if_root` and the `#[as_nobody_when_root]` attribute macro so that tests asserting `PermissionDenied` remain valid when the test process runs as root (e.g. under Bazel remote execution), where `CAP_DAC_OVERRIDE` would otherwise bypass filesystem permission bits.

#### Scenario: Non-root process runs action unchanged
- **WHEN** `run_as_nobody_if_root(action)` is called and the effective uid is not `0`
- **THEN** `action` runs in-process with no forking or privilege changes

#### Scenario: Root process forks and drops to nobody
- **WHEN** `run_as_nobody_if_root(action)` is called and the effective uid is `0`
- **THEN** the process forks, redirects `TMPDIR`/`TEST_TMPDIR` to a `nobody`-owned directory, and drops supplementary groups, gid, and uid to the unprivileged `nobody` user before running `action`
- **AND** the parent mirrors the child's outcome, re-raising a panic with the same message if `action` panicked

#### Scenario: Attribute macro wraps a test body
- **WHEN** a test function is annotated with `#[ic_test_utilities_privileges::as_nobody_when_root]`
- **THEN** the macro rejects `async` functions and functions with a non-unit return type at compile time
- **AND** otherwise wraps the function body in a call to `run_as_nobody_if_root`
