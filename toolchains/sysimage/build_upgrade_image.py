...
def main():
    ...
    parser.add_argument(
        "--overlay",
        help="Optional (tzst) overlay image for the fast upgrade",
        type=str,
        default=None,
    )
    ...
    if overlay_image:
        overlay_path = os.path.join(tmpdir, "overlay.img")
        subprocess.run(
            ["tar", "xf", overlay_image, "--transform=s/partition.img/overlay.img/", "-C", tmpdir],
            check=True,
        )
