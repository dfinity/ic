...
def untar_to_fat32(tf, fs_basedir, out_file, path_transform):
    ...
    into the fat32 image pointed to by "out_file". "fs_basedir" is
...
def main():
    ...
    subprocess.run(["/usr/sbin/mkfs.fat", "-F", "32", "-i", "0", out_file], check=True)
    if image_label:
        subprocess.run(["faketime", "-f", "1970-1-1 0:0:0", "/usr/sbin/fatlabel", out_file, image_label], check=True)

    if in_file:
        with tarfile.open(in_file, mode="r|*") as tf:
            untar_to_fat32(tf, fs_basedir, out_file, path_transform)
